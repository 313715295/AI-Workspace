...
输入齐备、互不依赖的受试任务一次完整派发，各自在固定预算内完成、自存产物和简短终态后结束；不另设统筹任务，不要求 ACK，不持续轮询。用户选择稍后收集时，派发后结束主控本轮工作，等用户通知再集中收集；受试者不逐个提取消耗，收集者从精确日志一次提取配置、Token 和费用。共享 Planner 或分阶段题只在真实依赖边界处理一次交接；阻塞保留事实，不自行补提示、换模型或重跑。沿用原质量检查和停止条件，费用分别列 Planner、Worker、统筹及废弃尝试，不拿含 Planner 的路线与 Worker 自身费用混比。
...
4. 通过脱敏数据或可选日志适配器补实际turn身份与usage、计费通道及范围。再次运行同一命令，仅处理新增/变化输入：不重跑已完成机械检查，不替换首交；计量报告在计算版本和所有输出身份核验后复用，否则新目录离线重算且保留旧目录。已完成题面/模型配置若变化会提示使用新case或participant ID；未完成题面的变化生成新准备版本并保留旧版本。
...
模型/effort、轮次、费率、计费单位、截止时间、受试与统筹身份都来自配置；新增GPT-6等模型没有默认费率。记录实际model ID、通道、费率来源/日期和实际时间定义，不从旧型号推断新型号速度或价格；套餐额度差没有可靠读取时标为N/A。日志适配仅支持明确指定的文件和turn选择，不扫描账户目录或完整任务历史。详细数据格式见 [FORMAT.md](FORMAT.md)。

## 能证明什么

- 费率估算不是账单；推理tokens已含在output，不能再次相加。Codex credits与API USD分别计量；cache-write按实际通道处理，未知计数保留null。Codex credits不强加单独缓存写入价；API写入若缺实际计数或费率则费用未知。
- 响应数只计去重的可观察usage记录，不代表完整供应商请求数。配置已核验、计量完整、指定范围费用可比较分别呈现；阶段费用不冒充完整任务费用。部分已知费用以小计呈现，未知总额保持unknown。
