...
需要项目治理的请求
  ↓
Router Skill 按需导航；健康续作复用已加载内容
  ↓
需要恢复时，从项目 .ai-workspace/BOOTSTRAP.md 定位当前事实和规则入口
  ↓
...
```text
请按当前项目采用的 Framework 继续处理这项任务。需要项目治理事实时使用已接入的 Router Skill 导航，健康续作复用有效上下文；只恢复或补载当前缺失、变化的内容，遵守项目现行授权与保护边界。
```
