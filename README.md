...
- 明确责任：项目 Controller 和领域 Owner 承担长期职责；每项任务绑定结果责任，临时执行者和独立 Reviewer 按需参与。
- 控制权限：写入、测试、Review、Git、推送、设备和外部操作分别授权，不能相互推导。
- 核对工作边界：在已声明的范围内检查项目根、Git 根、路径、对象身份和保护边界；设计与实现是否符合任务目标和约束，仍需结合任务合同、验证与 Review 判断。
- 降低沟通与协作开销：健康会话连续规划、实施和测试，按实际独立性、并行、隔离需要及总成本安排委派，直接向负责的 Owner 返回可用结果，避免重复状态和非必要交接。
