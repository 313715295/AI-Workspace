...
2. For 1.15 or later, invoke the sealed `PROCESS_REQUIREMENTS_RESOLVE/DISCOVER` directly against the complete metadata catalog. Load the complete Markdown blocks returned for sealed Framework rules plus the selected effective corrections and project-policy rules. Do not use `LOAD_PLAN` to exclude catalog entries.
...
6. Before each distinct governed action, invoke `ADMIT_ACTION` with the unchanged DISCOVER receipt and actual preparation/authorization evidence.
7. Immediately before final output, invoke `FINALIZE_OUTPUT` with the actual result and delivery receipts. Complete missing safe work or return the exact blocker.
