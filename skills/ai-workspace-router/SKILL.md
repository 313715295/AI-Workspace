...
首次进入或命中 recovery invalidation/body-availability boundary 时，按当前 pin 的 `RECOVERY_CORE.md` 恢复；健康连续工作复用仍有效的 composition、decision 与完整正文。具体触发、失效和 compaction 后实际正文重读条件全部由该模块规定，本 Skill 不维护第二份清单。显式 `$ai-workspace-router` 只重新检查当前绑定，不能单独证明模型已读取正文。
