...
Project ID=`{{PROJECT_ID}}`; repo-local control plane=`.ai-workspace/`; pinned Framework=`{{FRAMEWORK_VERSION}}`. This is the only entry. Chat, legacy summaries, memory and `framework/CURRENT` are locators only, never authority.
...
4. The pinned stable version must be complete. Do not repair it from CURRENT, tag, HEAD, network, or `framework/drafts/`.
5. A complete repo-local plane outranks a compatible frozen central legacy copy. Partial, reparse, identity conflict or unknown control bytes fail closed.
