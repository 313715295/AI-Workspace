...
Assert-True ([string]$version.version -ceq '1.7.0' -and [string]$version.lifecycle -ceq 'DRAFT' -and [string]$version.releaseClass -ceq 'MINOR' -and -not [bool]$version.consumable -and [string]$version.baseline -ceq '1.6.1') 'version-draft-minor-baseline-1.6.1'
Assert-True ([string]$loadManifest.lifecycle -ceq 'DRAFT' -and @($loadManifest.topologies.PSObject.Properties.Name).Count -eq 2 -and @($loadManifest.topologies.FRAMEWORK_MAINTENANCE_SIBLING) -contains 'FRAMEWORK_MAINTENANCE.md') 'load-manifest-topology-contract'
...
Write-Output ("RESULT|" + $script:passed + "/" + $script:passed + " passed|scope=Framework-1.7.0-candidate|lifecycle=DRAFT")
