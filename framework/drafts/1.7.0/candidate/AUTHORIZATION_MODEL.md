...
只读搜索、读取、hash和非变更诊断默认不需要包，但日常恢复、搜索与Git仍须应用项目`routineExcludedPaths`；该列表不授予或拒绝source写权，写权仍由task exact/forbidden和本授权包决定。
...
普通actual schema3 `repo-local`项目继续使用schema1。checker在Git发现前拒绝非空`GIT_DIR / GIT_WORK_TREE / GIT_COMMON_DIR`进程覆盖，必须证明调用cwd按目录组件真实属于无reparse Git top，再严格读取canonical `.ai-workspace/project.json`并完整验证schema3 root字段、安全且唯一的routine exclusions，以及空capability或唯一合法`KNOWLEDGE_REFERENCE`形状。不是`schemaVersion=3 / controlPlaneLayout=repo-local / frameworkVersion=1.7.0`、配置缺失/异常或路径链含reparse时schema1拒绝。`controlPlaneLayout=framework-maintenance-sibling`只允许schema2，并在通用字段之外增加：

```json
{
  "schemaVersion": 2,
  "repositoryId": "AI_WORKSPACE_FRAMEWORK",
  "projectConfigIdentity": "<bytes|UPPER_SHA256>"
}
```

一个schema2包只绑定一个Git仓，`exactPaths`与`objectIdentities`都相对于该仓真实Git top。`repositoryId=CONTROL`的包不得包含`SOURCE_WRITE / TEST_WRITE / BROWSER_RUN / DEVICE_RUN`；目标Framework仓包不得包含`CONTROL_WRITE`。控制面与Framework源码需要连续变化时必须先释放writer，再分别签发两个包，不能用一个pathset跨越两个仓。

schema2必须额外包含`REPOSITORY_CHANGE / PROJECT_CONFIG_DRIFT`失效条件。checker复证当前Maintenance `.ai-workspace/project.json`的whole-file identity、schema4布局和配置声明的唯一target repository ID，并直接调用同版本`resolve-framework-maintenance-target.ps1`。只有resolver证明CURRENT controller、两个真实Git top、完整目标pin和目标无任何`.ai-workspace`时，CONTROL或target包才可继续预检；resolver失败、返回漂移、repository ID或配置identity变化均拒绝。Git、push、external仍由动作字段和原有用户门分别控制，schema2本身不扩大权限。

## 7. Checker证明边界

1.7.0的PROJECT_CONTROLLER包除通用字段外，必须且只额外携带`issuerControllerId`、整数`issuerControllerEpoch`和`controllerControlIdentity`。预检直接读取当前`.ai-workspace/controller.json`，复证whole-file identity、`CURRENT`状态、ID与epoch；任一不一致均拒绝为stale controller authorization。controller schema本身只允许`CURRENT`，不存在可签发或消费授权包的迁移临时状态。`DOMAIN_OWNER`包不得携带这三个字段。轮换不需要扫描或迁移旧授权包：旧包因Framework版本或controller identity/epoch不匹配自然失效。

`scripts/check-authorization.ps1`验证：字段及JSON真实类型、角色签发权限、动作、用户门、repository绑定、路径规范化、observed path子集、对象identity、actor和必需失效条件。字符串`"false"`、字符串`"1"`、标量冒充数组或非字符串数组成员均拒绝，不能依靠PowerShell强制转换把异常JSON升级为合法授权。

任何含`REVIEW_EXECUTE`的包不得同时包含`CONTROL_WRITE / SOURCE_WRITE / TEST_WRITE / GIT_STAGE / GIT_COMMIT / PUSH / EXTERNAL`。独立Reviewer可以在另行明确授权时运行只读测试、browser或device验证，但候选写入与最终审核必须分包、分writer lineage；候选冻结后由未参与候选写入的Reviewer消费纯审核包。
