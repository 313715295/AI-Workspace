...
Framework维护任务除task/owner/actions/exact/identity外还冻结`repositoryId + projectConfigIdentity`。路径只解释为所绑定Git top内的相对路径；控制仓与Framework目标仓不能出现在同一个写包。若一个阶段需要先更新控制状态再改Framework候选，owner按`CONTROL → writer release → TARGET`签发连续包，并在每次交接重新验证配置、controller epoch和另一个仓的dirty边界。

共同父目录、其他兄弟项目、consumer项目、Pocket、稳定已发布版本、root CURRENT和未授权root脚本默认forbidden。出现第三仓、目标重命名、parent变成Git/控制面、目标`.ai-workspace`，或需要跨仓原子性/自动补偿时，不扩面实现，按公共合同变化回用户。物理迁移必须退出Framework runtime并另立项目任务。
