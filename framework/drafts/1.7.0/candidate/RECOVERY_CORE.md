...
Framework 1.7.0项目在读取任务前还必须严格读取`.ai-workspace/controller.json`与`project.json`的`routineExcludedPaths/frameworkCapabilities`。`controller.json`是current PROJECT_CONTROLLER ID与epoch的唯一机器真相；聊天、热摘要和旧授权都不能替代它。日常排除不授source权限，能力字段默认空且不产生能力；当前唯一可选能力是显式配置的`KNOWLEDGE_REFERENCE`，它只增加非权威参考模块。
...
2. 优先严格验证项目根的repo-local控制面；只有它完全不存在时才允许项目定义的legacy fallback。Framework维护项目可显式使用schema4 sibling布局，但控制面仍位于Maintenance控制仓，目标Framework仓不成为第二控制面。partial、reparse、身份冲突或未知live bytes fail closed。
...
schema4布局还必须复证共同父目录没有`.git`或`.ai-workspace`、控制仓与唯一目标仓各自是非reparse真实Git top、target ID/安全单组件目录/固定版本入口以及目标仓自己的`AGENTS.md`。目标仓出现完整、部分、foreign或reparse `.ai-workspace`、出现第三目标或任何定位漂移时保持只读并fail closed；Framework恢复不提供迁移观察例外。
...
loader的base selectors为`topology + role + profile + phase + host`，并额外接收Bootstrap从严格project config派生的可选canonical capability集合；输出为公共核心与所选完整模块的有序去重并集。`topology`默认`REPO_LOCAL`；只有schema4验证通过后才选择`FRAMEWORK_MAINTENANCE_SIBLING`并强制加载`FRAMEWORK_MAINTENANCE.md`。模块内部不得要求调用者猜测另一个未加载章节；若存在硬依赖，必须在manifest声明。
