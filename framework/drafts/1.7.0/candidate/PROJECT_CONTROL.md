...
`project.json` schema3增加`routineExcludedPaths`与`frameworkCapabilities`；前者只控制日常恢复/搜索/Git不展示不操作，后者默认空且当前只允许严格的`KNOWLEDGE_REFERENCE`配置。`controller.json`独立保存current controller ID、epoch和状态。注册1.7.0必须提供ControllerId并生成starter exact9；旧CURRENT或显式1.4/1.5注册仍保持schema2兼容。
...
## 7. Framework维护sibling布局

schema4 `framework-maintenance-sibling`是显式opt-in，不改变普通项目schema3默认。Maintenance仓保存唯一动态`.ai-workspace`；同一非Git父目录下唯一兄弟AI-Workspace仓保存Framework源码、版本、脚本与静态`AGENTS.md`，常态不保存`.ai-workspace`。

恢复从Maintenance `.ai-workspace/BOOTSTRAP.md`开始，冻结project config identity后运行`resolve-framework-maintenance-target.ps1`。resolver只接受配置声明的安全单组件兄弟目录，不搜索磁盘、不接受绝对路径/`..`/通配符/reparse；它必须证明父目录无`.git`/`.ai-workspace`、两个真实Git top、目标`README.md / AGENTS.md / framework/versions/<pin>/RECOVERY_CORE.md`存在。任何冲突fail closed。

Framework 1.7.0不提供从legacy同仓布局到sibling布局的通用迁移执行器或中间controller状态。starter、resolver、FULL_COLD与authorization只接受最终稳态：Maintenance controller为CURRENT，目标仓没有canonical `.ai-workspace`。checker先拒绝Git进程环境覆盖并证明cwd属于无reparse Git top，再由完整严格验证的actual canonical project config选择授权schema：schema3 repo-local才允许schema1，schema4只允许schema2；schema2再消费同版本resolver，不能由package声明降级或只凭局部字段绕过拓扑门。

创建父目录、创建Maintenance仓、移动Framework仓和退役旧控制面是AI-Workspace项目特定的离线迁移，必须另立任务并基于当时真实对象冻结步骤、恢复与授权；不是register、upgrade、starter渲染或Framework runtime的一部分。最终稳态未建立并被fresh FULL_COLD接受前，Framework不得宣布权威已切换。
