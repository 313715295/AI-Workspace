...
发生repository/action/pathset扩展、project config/对象或owner漂移、writer交接、用户决定变化、候选冻结，或进入Review/Git/external边界时必须重新预检。loader同样只在FULL_COLD、topology/role/profile/phase/host变化或加载manifest变化时重算；普通温重绑不重复运行未变化的加载计划。

推荐把预检与实际动作放在同一受控宿主hook或wrapper中；成功默认只返回一行摘要，只有失败才返回具体原因。只在聊天中写“已授权”不构成机械门。Codex当前若没有可配置的全局pre-tool hook，Framework只能要求并测试预检脚本，不能声称宿主无法绕过。1.7.0将其明确记录为机械预检的能力边界，不冒充操作系统级不可绕过强制，也不为常态任务增加binding文件或digest缓存合同。
...
未启用、无库、空索引、条目漂移或authority冲突统一按`REFERENCE_UNAVAILABLE`处理，继续正式恢复链。每次普通查询最多消费3条CURRENT引用；任务或owner给出稳定Entry ID时逐字传给checker，不从自然语言猜ID。fixture和人工演示不计入自然查询样本。

## 7. 双仓workspace边界

Framework维护拓扑要求Codex workspace同时且只显式挂载Maintenance控制仓与其唯一兄弟Framework目标仓；共同父目录只是workspace/sandbox边界，不是Git仓或控制面。宿主必须分别解析两个真实Git top，并完整读取目标仓自己的`AGENTS.md`；不能假定父目录或兄弟仓的AGENTS规则会自动继承。

宿主获得某一仓的写权限不等于获得另一仓权限。任何文件工具、测试、Review和Git动作都使用当前schema2包的`repositoryId`选择根；未绑定、根不在workspace、需要扩大sandbox、出现第三仓或目标为reparse时保持只读并回owner。
