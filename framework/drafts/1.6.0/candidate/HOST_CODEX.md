...
这些alias只属于当前Codex adapter示例，不是跨host公共合同。adapter在每次fresh rebind把公共`phase / hostClass / contextIdentity / minimumQuality / requiredTools / continuity / latencyClass / costCeiling / concurrencyFit`解析为当前可用组合，并冻结`adapterResultIdentity`及`SUPPORTED`或`UNSUPPORTED`。task checker必须以现场phase、host class、context identity和adapter result identity逐字复证；只写失效标签而不绑定现场输入不构成资源门。不支持时回owner重选；不得把更低effort、缺工具或断裂上下文标成成功降级。
...
推荐把预检与实际动作放在同一受控宿主hook或wrapper中，并在本地静默缓存PASS digest；成功默认只返回一行摘要，只有失败才返回具体原因，详细JSON按诊断请求。只在聊天中写“已授权”不构成机械门。Codex当前若没有可配置的全局pre-tool hook，Framework只能要求并测试预检脚本，不能声称宿主无法绕过。1.6继续明确记录机械预检的能力边界，不冒充操作系统级不可绕过强制。

PROJECT_CONTROLLER包的PASS cache还必须包含authorization package identity、`controller.json` path+identity和epoch；controller对象变化立即失效。DOMAIN_OWNER cache不加入虚构controller依赖。
