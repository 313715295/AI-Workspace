...
`issuerRole=PROJECT_CONTROLLER`时条件必填：

```json
{
  "issuerProjectId": "project-id",
  "issuerControllerId": "controller-id",
  "issuerControllerEpoch": 3,
  "controllerControlLocator": ".ai-workspace/controller.json",
  "controllerControlIdentity": "123|<UPPER_SHA256>"
}
```

checker必须同时接收现场`ObservedProjectId / ObservedControllerLocator`，且locator只能是规范`.ai-workspace/controller.json`；`ControllerControlPath`必须实际以该locator结尾。checker严格读取canonical controller schema、自己复算整个文件identity，并要求`issuerProjectId == observed projectId == controller.json.projectId`、`issuer == issuerControllerId == controllerId`以及epoch/identity逐字一致。缺字段、跨项目对象、非规范locator、旧epoch、旧controller、伪造摘要或control drift统一为`STALE_CONTROLLER_AUTHORIZATION`。这五个条件字段只属于PROJECT_CONTROLLER分支；DOMAIN_OWNER包不要求、不推导、不消费它们，controller轮换也不单独撤销合法DOMAIN_OWNER包。
...
`scripts/check-authorization.ps1`验证：字段及JSON真实类型、角色签发权限、动作、用户门、路径规范化、observed path子集、对象identity、actor和必需失效条件。字符串`"false"`、字符串`"1"`、标量冒充数组或非字符串数组成员均拒绝，不能依靠PowerShell强制转换把异常JSON升级为合法授权。
...
## 7. Remote transaction授权

多remote不能从一个普通Git action隐式扩张。每个remote记录稳定`remoteId + endpointFingerprint + action + refspec + expected local/remote heads + authorization locator/identity`，checker用现场观察逐字复证；整包只有一个Git closer。部分成功后只允许FAILED/UNKNOWN项重试；任何remote/config/head/authorization漂移重新签包。补偿是新的、另授权事务，每项必须绑定不同的compensation authorization locator/identity，PRIMARY不得携带`COMPENSATE_REF`，原成功receipt永久保留。
