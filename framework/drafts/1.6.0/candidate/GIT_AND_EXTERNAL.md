...
`REMOTE_TRANSACTION_SCHEMA.json`是remote事务DTO唯一真相。endpoint只保存去凭据后的SHA-256 fingerprint；remote名称/URL文本不作为身份。每项ledger逐字冻结现场local/remote head、endpoint fingerprint、refspec、primary与可选compensation authorization locator+identity及其presence，并保存`PENDING / SUCCEEDED / FAILED / UNKNOWN / COMPENSATION_REQUIRED`、observed head、secret-safe receipt与error class。幂等键是上述冻结输入的ordered canonical JSON之SHA-256，不使用可碰撞的分隔符拼接；aggregate必须由逐项状态推导。checker必须接收并逐字比较现场观察到的primary/compensation authorization locator+identity，落实DTO真实JSON类型/长度/格式，并在retry逐项冻结两组授权字段，不能仅验证ledger自报字段的形状。

retry保持SUCCEEDED项不可变且只尝试FAILED/UNKNOWN；expected local/remote head、fingerprint、refspec、Git closer或authorization identity漂移立即停止。push/tag/delete等远端副作用不可由本地rollback撤销；PRIMARY禁止`COMPENSATE_REF`，补偿必须是另授权`COMPENSATION`事务，逐项绑定不同的compensation authorization locator+identity，失败后保留原receipt与`COMPENSATION_REQUIRED`。inventory/status/diff证据显式传入protected excludes和secret-safe evidence receipt；evidence与protected路径做双向重叠检测，祖先范围覆盖受保护后代同样拒绝，现场采集scope/excludes/receipt必须与ledger逐字一致。不能为了摘要扫描被禁止visibility/read/hash/diff/index的路径。
