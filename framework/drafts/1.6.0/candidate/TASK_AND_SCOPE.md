...
本域、可逆、无公共合同变化由领域owner闭环。跨域、公共合同、阶段路线、领域冲突或高不可逆风险回主控。产品结果、重大方案和外部动作由主控整理代价后回用户。主控不重复领域日常审核，也不同时越过领域owner指挥其执行者。
...
5. protected paths的visibility/read/hash/diff/index/write边界；
...
资源公共合同只写`minimumQuality / requiredTools / continuity / latencyClass / costCeiling / concurrencyFit`及失效条件，不写具体model alias。具体model/effort是host adapter结果；fresh rebind必须验证可用性、工具和组合支持，低于minimum或缺工具时fail closed。诊断转实施、host/phase/tool/context变化都重新判定，不能继承旧档位或静默降级。

长周期工作使用可主动回传的`INDEPENDENT_TASK`；同一活跃回合可闭合的短并行才使用`SAME_TURN_SUBAGENT`。每个持久任务记录`sourceTaskId / reportTo / waitPolicy / archivePolicy`。不轮询维持进度感；只有当前消费者依赖结果时允许一次有界wait。只有terminal状态归档，复用前重新核对项目、领域、角色、候选血缘、owner、生命周期、资源和路径冲突。

默认`DIRECT_OWNER_DEFAULT`，`sourceTaskId`不等于主控抄送。领域owner可在自己的任务中`USER_DECISION_DIRECT`，handoff必须绑定来源任务、candidate/evidence、原话或稳定turn locator和invalidators。主控用索引/current卡按需pull；普通状态不主动上报，exception才升级。同一链只有责任owner产生一次terminal用户摘要。
