...
- Resource contract: phase=IMPLEMENT; hostClass=<LOCAL_AGENT|REMOTE_AGENT|HOST_NEUTRAL>; contextIdentity=<bytes|UPPER_SHA256>; minimumQuality=<OWNER_FRONTIER|FOCUSED_HIGH|ROUTINE_BALANCED|MECHANICAL_LOW>; requiredTools=[<tool|tool或空>]; continuity=FRESH; latencyClass=<INTERACTIVE|BOUNDED|BACKGROUND>; costCeiling=<项目值>; concurrencyFit=<INLINE_ONLY|PARALLEL_OK>; adapterResult=SUPPORTED; adapterResultIdentity=<bytes|UPPER_SHA256>; selectedQuality=<实际抽象质量>; invalidatesOn=[HOST_CHANGE|PHASE_CHANGE|TOOL_CHANGE|CONTEXT_CHANGE]
- Coordination contract: mode=INLINE; sourceTaskId=<TASK-ID>; reportTo=<owner>; waitPolicy=NOT_APPLICABLE; archivePolicy=TERMINAL_ONLY; userDecisionHandoff=NONE
...
- Resource contract: phase=<RECOVER|PLAN|IMPLEMENT|VERIFY|REVIEW|CONTROL|GIT|EXTERNAL|RELEASE>; hostClass=<LOCAL_AGENT|REMOTE_AGENT|HOST_NEUTRAL>; contextIdentity=<bytes|UPPER_SHA256>; minimumQuality=<OWNER_FRONTIER|FOCUSED_HIGH|ROUTINE_BALANCED|MECHANICAL_LOW>; requiredTools=[<tool|tool或空>]; continuity=<FRESH|WARM>; latencyClass=<INTERACTIVE|BOUNDED|BACKGROUND>; costCeiling=<项目值>; concurrencyFit=<INLINE_ONLY|PARALLEL_OK>; adapterResult=<SUPPORTED|UNSUPPORTED>; adapterResultIdentity=<bytes|UPPER_SHA256>; selectedQuality=<抽象质量>; invalidatesOn=[HOST_CHANGE|PHASE_CHANGE|TOOL_CHANGE|CONTEXT_CHANGE]
- Coordination contract: mode=<INLINE|SAME_TURN_SUBAGENT|INDEPENDENT_TASK>; sourceTaskId=<source task>; reportTo=<唯一owner/下游owner>; waitPolicy=<NOT_APPLICABLE|CURRENT_TURN_ONLY|NO_POLL|SINGLE_BOUNDED_IF_CONSUMER_DEPENDS>; archivePolicy=TERMINAL_ONLY; userDecisionHandoff=<NONE|BOUND>
...
若`issuerRole=PROJECT_CONTROLLER`，同一JSON再加入`issuerProjectId / issuerControllerId / issuerControllerEpoch / controllerControlLocator=.ai-workspace/controller.json / controllerControlIdentity`，并在调用checker时传现场`ObservedProjectId / ObservedControllerLocator`与唯一`ControllerControlPath`。若为`DOMAIN_OWNER`，不得为了格式统一添加或消费controller依赖。

若`userDecisionHandoff=BOUND`，增加一行：

```markdown
- User decision handoff: sourceTaskId=<必须与Coordination contract一致的来源任务>; candidateEvidence=<绑定对象>; turnLocator=<原话或稳定turn>; invalidatesOn=[CANDIDATE_CHANGE|USER_DECISION_CHANGE|LOCATOR_CHANGE|<其他>]
```
...
- forbidden / protected visibility-read-hash-diff-index-write：<...>
...
## 6. Remote transaction对象

多remote任务把唯一ledger写成`REMOTE_TRANSACTION_SCHEMA.json`定义的对象。任务卡只保存ledger locator/identity、唯一Git closer、authorization locator、protected excludes、当前aggregate与下一动作，不复制逐remote第二状态。retry或compensation必须绑定新/当前ledger；已SUCCEEDED项及receipt不可变。
