...
1.6.0 repo-local恢复还必须读取schema 3 project.json和controller.json。整个project.json identity绑定protected policy；整个controller.json identity绑定唯一current controller/epoch。热摘要、聊天与旧任务不能替代这两个机器对象。
...
执行/测试/browser/device/Review类包必须绑定由`resolve-resource-binding.ps1`生成的current短期binding。PROJECT_CONTROLLER包还必须绑定current controller ID、epoch与controller文件identity；DOMAIN_OWNER包保持独立。资源、controller或其他失效键变化时旧PASS不可复用。
...
Framework Git inventory/status/diff/index必须经protected-safe入口消费固定project.json与bounded allowlist；安全摘要失败时对应事实为`UNVERIFIED`，禁止扩大扫描重试。多remote普通push使用一次尝试的RemoteBatch，不提供自动retry或compensation。
...
默认执行者→owner、Reviewer→candidate owner、领域owner直达；routine terminal状态不抄送主控、不要求ACK。主控主动pull current卡，只有例外升级。旧controller epoch的routine排队消息只作审计，旧decision/authorization/ACK拒绝。
