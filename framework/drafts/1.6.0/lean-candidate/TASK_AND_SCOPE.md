...
普通任务只写`Resource requirement: DEFAULT`。非默认时只写单行JSON的`minimumQuality / requiredTools / continuity`；latency/cost/concurrency只有项目明确设置时才另行持久化。host在dispatch/fresh rebind时调用`resolve-resource-binding.ps1`，短期binding覆盖需求identity、adapter policy、现场capability、selected quality/tools、continuity、phase和context。任一输入变化都重算，不能复用旧PASS或静默降级。

### 8.2 默认协调与例外

默认协调不写入任务卡：`INLINE / DIRECT_OWNER_DEFAULT / NO_POLL / TERMINAL_ONLY / userDecisionHandoff=NONE`。只有独立任务、有界事件等待、非默认report target或用户决定handoff写`Coordination exception`差异。

用户决定handoff必须携带`sourceTaskId + bound candidate/evidence + user原话或稳定turn locator + invalidatesOn`。询问可行性、时间或建议是`QUESTION_NOT_DECISION`，只允许调查和回答，不形成采用、排期变化、CONTROL_WRITE或writer授权。
