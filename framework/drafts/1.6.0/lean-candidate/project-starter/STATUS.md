...
- current controller：`.ai-workspace/controller.json`（只保存locator，不在本文件复制identity/epoch）
- 保护策略：`.ai-workspace/project.json.protectedPaths`（整个project.json为唯一机器identity）
- 初始化结论：`NEEDS_INPUT`
...
- 当前主控：从`.ai-workspace/controller.json`读取；领域owner：`UNVERIFIED`
- 当前写入者：NONE
