...
任何包含`SOURCE_WRITE / TEST_WRITE / TEST_RUN / BROWSER_RUN / DEVICE_RUN / REVIEW_EXECUTE`的包，都必须增加`resourceBindingLocator / resourceBindingIdentity / resourceBindingDigest`，并包含`RESOURCE_BINDING_CHANGE`失效项。

binding只能由`resolve-resource-binding.ps1`根据current任务需求、adapter policy和现场capability生成。checker读取locator、复算整个文件identity和canonical digest；任务卡与授权包不复制完整资源矩阵，低于minimum、缺工具或continuity不满足时fail closed。

## 5.2 PROJECT_CONTROLLER条件绑定

`issuerRole=PROJECT_CONTROLLER`的包必须增加`issuerControllerId / issuerControllerEpoch / controllerControlIdentity`和`CONTROLLER_EPOCH_CHANGE`失效项。checker通过唯一`ControllerControlPath`实际读取controller.json，要求`issuer == issuerControllerId == current controllerId`且epoch、整个文件identity完全一致；否则统一为`STALE_CONTROLLER_AUTHORIZATION`。

`DOMAIN_OWNER`分支不要求、推导或消费上述controller字段。controller轮换不会把DOMAIN_OWNER包转换成主控包，也不自动授予successor旧主控权限。
