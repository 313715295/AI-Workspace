...
1.6.0 A-C+G审核还必须穿透以下有效键：RemoteBatch逐remote authorization/head/refspec/fingerprint与真实结果；整个project.json identity及protected-safe allowlist；资源requirement/policy/capability/quality/tools/continuity/phase/context；controller ID/epoch/whole-file identity、current task identity与原始message class。只检查字段存在或自报摘要不能关闭这些门。

RemoteBatch direct PASS不证明真实remote动作；protected PlanOnly不证明真实Git；资源binding PASS不证明所选模型的实际质量；controller route PASS不证明轮换事务已完成。证据上限必须逐项披露。
