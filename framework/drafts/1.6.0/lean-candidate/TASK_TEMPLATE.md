...
- Resource requirement: <DEFAULT或单行JSON minimumQuality/requiredTools/continuity>
- Coordination exception: <仅非默认时写单行JSON；否则省略本行>
...
```authorization-package
{
  ...
  "resourceBindingLocator": ".tmp/resource-bindings/<TASK-ID>.resource-binding.json",
  "resourceBindingIdentity": "<bytes|UPPER_SHA256>",
  "resourceBindingDigest": "<UPPER_SHA256>",
  "exactPaths": ["<a>", "<b>"],
  "objectIdentities": [
    {"path": "<a>", "identity": "<bytes|UPPER_SHA256>"},
    {"path": "<b>", "identity": "NEW"}
  ],
  "invalidatesOn": ["TASK_CHANGE", "OWNER_CHANGE", "GRANTEE_CHANGE", "ACTION_CHANGE", "PATHSET_CHANGE", "OBJECT_DRIFT", "USER_DECISION_CHANGE", "RESOURCE_BINDING_CHANGE"]
}
```
...
`issuerRole=PROJECT_CONTROLLER`时授权包条件增加`issuerControllerId / issuerControllerEpoch / controllerControlIdentity`，并在`invalidatesOn`增加`CONTROLLER_EPOCH_CHANGE`；DOMAIN_OWNER包不添加这些字段。只有独立任务、有界等待、非默认report target或用户决定handoff才保留`Coordination exception`，默认协调不展开。
