...
repo-local 1.6.0还必须严格读取`.ai-workspace/controller.json`，按`CONTROLLER_SCHEMA.json`验证唯一current controller与单调epoch，并计算整个文件identity。STATUS、tasks index和聊天只能保存locator或热摘要，不能复制第二个controller/epoch权威。
...
- 不stage/commit/push；
- 1.6.0要求明确ControllerId，写入schema 3 project.json、空`protectedPaths`、空`frameworkCapabilities`和epoch 1 controller.json；project config落盘前不扫描source工作树，只检查控制面目标allowlist；
...
- schema 2→3必须消费采用任务冻结的`protectedPathsMigration`及其identity；无法证明时保护相关Git事实保持`UNVERIFIED`。同一可恢复事务写入project、managed Bootstrap、epoch 1 controller和legacy主控授权revocation ledger；每个legacy locator必须现场复算identity。
- target从冻结old material派生；事务准备前、每次replace前复证preimage。未知live bytes不得被吸收成合法old preimage或覆盖。
...
## 7. 默认路由与controller epoch

- 默认`DIRECT_OWNER_DEFAULT / NO_POLL / TERMINAL_ONLY`：执行者回owner、Reviewer回candidate owner、领域owner直接交接；普通READY/PASS/HOLD/CONSUMED/HANDOFF_SENT不上报主控。
- 主控通过任务索引/current卡主动pull；任务未持久化current状态时fail closed。只有共享writer、控制/授权损坏、保护路径、跨域合同死锁、项目Git/external冲突或用户明确要求协调才exception escalation。
- 用户产品决定由对应领域owner直接询问并绑定candidate/evidence、原话或稳定turn locator和失效条件；主控只整合跨域/公共合同/阶段门。
- controller轮换在successor FULL_COLD ACCEPT后原子切换identity并递增epoch；旧主控只有read-only grace。旧epoch routine只作`STALE_QUEUED / AUDIT_ONLY`，旧decision/authorization/ACK拒绝；旧exception基于current task复证后最多重路由一次，不能复用旧授权或ACK链。
- current epoch内以project+epoch+task+candidate/state+responsible reporter去重；同一链只有责任owner在需要用户复玩、项目裁决或整链终结时主动报告一次。
