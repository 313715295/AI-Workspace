...
具体model alias不写入Framework任务卡。dispatch/fresh rebind时读取`Resource requirement`，把current adapter policy、capability、phase和context交给`resolve-resource-binding.ps1`；只有authorization checker复证binding locator+identity+digest后才缓存PASS。任一输入变化重算，不能静默降级。
...
默认report target是唯一owner，routine终态不上报主控且不形成ACK链。主控需要状态时从tasks index/current卡主动pull；只有共享writer、控制损坏、保护路径、跨域合同死锁、项目Git/external冲突或用户明确要求协调才发exception。
