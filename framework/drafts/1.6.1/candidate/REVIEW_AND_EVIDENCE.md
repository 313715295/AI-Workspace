...
同类finding在一个候选路线中再次出现时，不继续局部补条件链；owner必须在`SIMPLIFY / REMOVE / DEFER / REDESIGN`中明确选择并重新冻结范围。Review检查真正的producer与调用消费者：没有实际调用入口的schema/checker只算静态草案，不得作为已交付能力、集成通过或release证据。
...
2. exact/contingency/forbidden、routine exclusions及其他dirty边界；
...
当任务声明`Terminology impact=BIND / CHANGE`时，Review还要核对canonical term及authority locator真实存在，中文产品词、英文代码标识和UI文字映射无同义多真相或同名异义，旧alias有明确consumer、owner与退出条件。历史词汇只作审计定位，不得在current代码或文档中无声明复活。普通`NONE`任务不为此新增字段、报告或机械checker。
