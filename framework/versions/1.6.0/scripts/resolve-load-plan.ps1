...
if ($manifest.frameworkVersion -cne '1.6.0' -or $manifest.lifecycle -cne 'STABLE') {
    throw 'LOAD_MANIFEST_NOT_STABLE_1_6_0'
}
...
if ($AsJson) {
    ...
    Write-Output ('PASS|load-plan|role=' + $Role + '|profile=' + $Profile + '|phases=' + ($Phase -join ',') + '|host=' + $HostName + '|capabilities=' + (@($capabilitySet | Sort-Object) -join ',') + '|modules=' + $result.Count + '|paths=' + (@($result | ForEach-Object { $_.path }) -join '|') + '|bytes=' + $output.totalBytes + '|estimatedTokens=' + $output.estimatedTokens)
}
