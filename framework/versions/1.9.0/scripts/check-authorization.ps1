...
if ([string]$package.frameworkVersion -cne '1.9.0') { Add-Reason $reasons 'FRAMEWORK_VERSION' }
...
if ([int]$package.schemaVersion -eq 1) {
    try {
        ...
        if (-not (Test-JsonInteger $schema1Config.schemaVersion) -or [int]$schema1Config.schemaVersion -ne 3 -or
            ...
            -not ($schema1Config.frameworkVersion -is [string]) -or [string]$schema1Config.frameworkVersion -cne '1.9.0' -or
