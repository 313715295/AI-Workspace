...
function Render-MaintenanceStarter([string]$Starter,[string]$ControlRoot) {
    ...
    Get-ChildItem -LiteralPath $destination -Recurse -Force -File | ForEach-Object {
        ...
        $text = $text.Replace('{{FRAMEWORK_VERSION}}','1.9.0')
...
function New-AuthorizationPackage(
    ...
    $package = [ordered]@{
        schemaVersion=$Schema; frameworkVersion='1.9.0'; taskId='FIXTURE-001'; profile='STANDARD'; lifecycle='ACTIVE'
...
$expected = @(
    ...
    'scripts/check-authorization.ps1','scripts/check-knowledge-entry.ps1','scripts/check-knowledge-impact.ps1','scripts/check-task-card.ps1',
...
Assert-True ($actual.Count -eq 52 -and ($actual -join "`n") -ceq ($expected -join "`n")) 'inventory-exact52'
...
Assert-True ([string]$version.version -ceq '1.9.0' -and [string]$version.lifecycle -ceq 'STABLE' -and [string]$version.releaseClass -ceq 'MINOR' -and [bool]$version.consumable -and [string]$version.baseline -ceq '1.8.0' -and $null -eq $version.PSObject.Properties['currentEligible'] -and [bool]$version.projectPinEligible) 'version-stable-minor-baseline-1.8.0-no-global-selector-field'
...
Assert-True ($loadResolverText.Contains('LOAD_MANIFEST_NOT_STABLE_1_9_0') -and -not $loadResolverText.Contains('LOAD_MANIFEST_NOT_STABLE_1_7_0')) 'load-resolver-diagnostic-version-current'
...
try {
    $null = $normalProjectTemplate.Replace('{{PROJECT_ID_JSON}}','"fixture"').Replace('{{DISPLAY_NAME_JSON}}','"Fixture"').Replace('{{FRAMEWORK_VERSION_JSON}}','"1.9.0"') | ConvertFrom-Json
...
'@
    ...
    Write-Utf8 $standardTaskPath "# TASK-STANDARD-001 - fixture`n`n- Task schema: 1.9.0`n- Owner: owner-fixture`n- Range summary: profile=STANDARD; lifecycle=ACTIVE; expected_paths=[]; actual_paths=[]`n"
    ...
    Copy-Item -LiteralPath $candidateRoot -Destination (Join-Path $target 'framework\versions\1.9.0') -Recurse
    ...
    $installedFramework = Join-Path $target 'framework\versions\1.9.0'
    ...
    Assert-True ($installedActual.Count -eq 52 -and ($installedActual -join "`n") -ceq ($actual -join "`n")) 'maintenance-target-full-candidate-installed'
    ...
    Assert-True ($missingPin.Code -ne 0 -and $missingPin.Text.Contains('TARGET_REQUIRED_FILE_MISSING|framework/versions/1.9.0/RECOVERY_CORE.md')) 'maintenance-resolver-pin-entry-missing-rejected'
...
'@
    ...
    $repoLocalConfig = [ordered]@{schemaVersion=3;id='repo-local-fixture';displayName='Repo Local Fixture';controlPlaneLayout='repo-local';repositoryRoot='..';frameworkVersion='1.9.0';routineExcludedPaths=@('private/secret.txt');frameworkCapabilities=[pscustomobject]@{KNOWLEDGE_REFERENCE=[pscustomobject]@{enabled=$false}}}
    ...
    $criticalReviewPackage = [ordered]@{
        schemaVersion=1;frameworkVersion='1.9.0';taskId='FIXTURE-REVIEW-001';profile='CRITICAL';lifecycle='ACTIVE'
    ...
    if (-not $SkipRootMigration) {
        ...
        Copy-Item -LiteralPath $candidateRoot -Destination (Join-Path $rootFlow 'framework\versions\1.9.0') -Recurse
        Assert-True (-not (Test-Path -LiteralPath (Join-Path $rootFlow 'framework\CURRENT'))) 'root-global-version-selector-absent'

        $fixtureManifestPath=Join-Path $rootFlow 'framework\versions\1.9.0\RELEASE_MANIFEST.json'
        ...
        $pendingRegister=Invoke-Ps $register @($baseRegisterArgs + @('-FrameworkVersion','1.9.0'))
        Assert-True ($pendingRegister.Code -ne 0 -and $pendingRegister.Text.Contains('FRAMEWORK_RELEASE_NOT_SEALED|1.9.0')) 'register-pending-release-rejected-before-project-write'
        Assert-True (-not (Test-Path -LiteralPath (Join-Path $consumer '.ai-workspace'))) 'register-pending-release-zero-project-write'

        $fixtureManifest.sourceReview='APPROVED';$fixtureManifest.releaseIntegration='TEST_FIXTURE_SEALED'
        Write-Utf8 $fixtureManifestPath ($fixtureManifest|ConvertTo-Json -Depth 20)
        $fixtureVersionRoot=Split-Path -Parent $fixtureManifestPath
        $fixturePayload=@(Get-ChildItem -LiteralPath $fixtureVersionRoot -Recurse -Force -File|ForEach-Object{$_.FullName.Substring($fixtureVersionRoot.Length+1).Replace('\','/')}|Where-Object{$_-cne'RELEASE_MANIFEST.json'})
        [Array]::Sort($fixturePayload,[StringComparer]::Ordinal)
        $fixtureRows=@();$fixtureTotal=[int64]0
        foreach($relative in $fixturePayload){$full=Join-Path $fixtureVersionRoot $relative;$bytes=[IO.File]::ReadAllBytes($full);$fixtureTotal+=$bytes.Length;$fixtureRows+=($relative+'|'+$bytes.Length+'|'+(Get-Identity $full).Split('|')[1])}
        [string[]]$fixtureOrderedRows=@($fixtureRows)
        [Array]::Sort($fixtureOrderedRows,[StringComparer]::Ordinal)
        $fixtureSha=[Security.Cryptography.SHA256]::Create()
        try{$fixtureCanonical=([BitConverter]::ToString($fixtureSha.ComputeHash($utf8.GetBytes(($fixtureOrderedRows-join"`n"))))).Replace('-','')}finally{$fixtureSha.Dispose()}
        $sourceRows=@()
        ...
        $shadowUpgradeWithoutRepository=Invoke-Ps $upgrade @('-ProjectId','shadow-fixture','-ToVersion','1.9.0','-ControllerId','controller-shadow')
        ...
        $previewRegister=Invoke-Ps $register @($baseRegisterArgs + @('-FrameworkVersion','1.9.0'))
        if ($previewRegister.Code -ne 0 -or -not $previewRegister.Text.Contains('WHAT_IF')) { Write-Output ('DIAG|register-explicit-1.9.0-preview|' + $previewRegister.Code + '|' + $previewRegister.Text) }
        Assert-True ($previewRegister.Code -eq 0 -and $previewRegister.Text.Contains('WHAT_IF')) 'register-explicit-1.9.0-preview'
        $applyRegister=Invoke-Ps $register @($baseRegisterArgs + @('-FrameworkVersion','1.9.0','-Apply'))
        Assert-True ($applyRegister.Code -eq 0 -and $applyRegister.Text.Contains('CREATED')) 'register-explicit-1.9.0-apply'
        $repeatRegister=Invoke-Ps $register @($baseRegisterArgs + @('-FrameworkVersion','1.9.0'))
        Assert-True ($repeatRegister.Code -eq 0 -and $repeatRegister.Text.Contains('ALREADY_REGISTERED')) 'register-explicit-1.9.0-repeat'
        ...
        Assert-True ([string]$registeredConfig.frameworkVersion -ceq '1.9.0' -and $registeredFiles.Count -eq 9 -and $registeredBootstrap.Contains('scripts/resolve-workflow-route.ps1')) 'register-materializes-selected-starter-exact9'
        ...
        $upgradeArgs=@('-ProjectId','upgrade-fixture','-ToVersion','1.9.0','-RepositoryPath',$upgradeRepo,'-ControllerId','controller-upgrade')
        ...
        Assert-True ($upgradePreview.Code -eq 0 -and $upgradePreview.Text.Contains('Preview only')) 'upgrade-schema3-to-1.9.0-preview'
        $upgradeApply=Invoke-Ps $upgrade @($upgradeArgs+'-Apply')
        Assert-True ($upgradeApply.Code -eq 0 -and $upgradeApply.Text.Contains('Updated:')) 'upgrade-schema3-to-1.9.0-apply'
    ...
    $knowledgeConfig=[ordered]@{schemaVersion=3;id='knowledge-fixture';displayName='Knowledge Fixture';controlPlaneLayout='repo-local';repositoryRoot='..';frameworkVersion='1.9.0';routineExcludedPaths=@();frameworkCapabilities=[ordered]@{KNOWLEDGE_REFERENCE=[ordered]@{enabled=$true;indexLocator='knowledge/index.json'}}}
    ...
    if (-not $SkipBaseline) {
        ...
        $baselineRunner = Join-Path $frameworkRoot 'versions\1.8.0\tests\run-framework-tests.ps1'
        $baseline = Invoke-Ps $baselineRunner @('-SkipRootMigration')
        if ($baseline.Code -ne 0) { Write-Output ('DIAG|baseline-1.8.0|' + $baseline.Text) }
        Assert-True ($baseline.Code -eq 0 -and $baseline.Text.Contains('RESULT|')) 'baseline-1.8.0-regression-suite'
    }
...
$resultScope=if($resultSealed){'Framework-1.9.0-stable'}else{'Framework-1.9.0-candidate'}
