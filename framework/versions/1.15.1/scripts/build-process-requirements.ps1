...
try {
    ...
    if ([string]$version.version -cne '1.15.1') { throw 'VERSION_MISMATCH' }
    ...
    foreach ($entry in @($manifest.requirementFragments)) {
        ...
        if ([int]$fragment.schemaVersion -ne 2 -or [string]$fragment.frameworkVersion -cne '1.15.1' -or [string]$fragment.ownerModule -cne $owner -or -not ($fragment.requirements -is [Array])) { throw 'REQUIREMENT_FRAGMENT_VALUES' }
    ...
    $projection = [ordered]@{schemaVersion=2;frameworkVersion='1.15.1';catalogVersion='3';requirements=@($orderedRequirements)}
