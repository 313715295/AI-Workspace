...
function Render-MaintenanceStarter([string]$Starter,[string]$ControlRoot) {
    ...
    Get-ChildItem -LiteralPath $destination -Recurse -Force -File | ForEach-Object {
        ...
        $text = $text.Replace('{{FRAMEWORK_VERSION}}','1.15.1')
...
function New-AuthorizationPackage(
    ...
    Write-Utf8 $taskFull ("# $taskId - authorization fixture`n`n- Task schema: 1.15.1`n- Owner: $issuer`n- Work route: actor=$issuer; role="+$(if($DomainOwner){'DOMAIN_OWNER'}else{'CONTROLLER'})+"; phase=IMPLEMENT`n- Range summary: profile=STANDARD; lifecycle=ACTIVE; expected_paths=[]; actual_paths=[]`n")
    $taskIdentity=Get-Identity $taskFull
    $package = [ordered]@{
        schemaVersion=$Schema; frameworkVersion='1.15.1'; taskId=$taskId; taskIdentity=$taskIdentity; profile='STANDARD'; lifecycle='ACTIVE'
...
Assert-True ([int]$inventory.schemaVersion-eq1-and[string]$inventory.frameworkVersion-ceq'1.15.1'-and$inventoryFields.Count-eq6-and$declaredArray.Count-eq$declaredUnique.Count-and$actual.Count-eq88-and($actual-join"`n")-ceq($declaredArray-join"`n")) 'normative-surface-inventory-exact88-classified-once'
...
Assert-True ([string]$version.version -ceq '1.15.1' -and $versionLifecycleState -and [string]$version.releaseClass -ceq 'PATCH' -and [string]$version.baseline -ceq '1.15.0' -and $null -eq $version.PSObject.Properties['currentEligible']) 'version-lifecycle-fields-patch-baseline-1.15.0-no-global-selector-field'
...
Assert-True ($loadResolverText.Contains('LOAD_MANIFEST_NOT_STABLE_1_15_1') -and -not $loadResolverText.Contains('LOAD_MANIFEST_NOT_STABLE_1_15_0')) 'load-resolver-diagnostic-version-current'
...
Assert-True ([int]$toolchain.schemaVersion-eq1-and[string]$toolchain.frameworkVersion-ceq'1.15.1'-and[string]$toolchain.contractVersion-ceq'1'-and[string]$toolchain.projectSelectionField-ceq'frameworkToolBackend'-and@($toolchain.officialBackends).Count-eq1) 'toolchain-single-project-selected-backend'
...
if($ToolContractOnly){
    ...
    try{
        ...
        $gitConfig=[ordered]@{schemaVersion=3;id='tool-contract-git';displayName='Tool Contract Git';controlPlaneLayout='repo-local';repositoryRoot='..';frameworkVersion='1.15.1';frameworkToolBackend='powershell7';routineExcludedPaths=@();frameworkCapabilities=[ordered]@{KNOWLEDGE_REFERENCE=[ordered]@{enabled=$false}}}
    ...
    Write-Output "RESULT|$($script:passed) passed|scope=Framework-1.15.1-tool-contract"
    return
}
...
$runtimeCandidateRoot=Join-Path $temp 'sealed-runtime\1.15.1'
...
try {
    $null = $normalProjectTemplate.Replace('{{PROJECT_ID_JSON}}','"fixture"').Replace('{{DISPLAY_NAME_JSON}}','"Fixture"').Replace('{{FRAMEWORK_VERSION_JSON}}','"1.15.1"') | ConvertFrom-Json
...
Assert-True ([string]$budgetContract.baseline.version-ceq'1.15.0'-and[string]$budgetContract.baseline.catalogIdentity-ceq'48098|2B84B06C7B682CE90AE6409F6BBF23A8C1920A66E7D1F25DFE797A806AF9EB17'-and[string]$budgetContract.candidate.catalogIdentity-ceq$catalogBudgetIdentity) 'process-budget-patch-baseline-and-candidate-catalog-identities-exact'
...
Assert-True ($catalogCheck.Code-eq0-and$catalogCheck.Text.Contains('PASS|requirements=42|fragments=11')) 'process-requirements-canonical-fragments-match-generated-catalog'
...
Assert-True ($blockProjectionValid-and@($catalog.requirements).Count-eq42) 'process-requirements-metadata-only-catalog-exact-markdown-blocks-nonempty'
...
Assert-True ($evaluationText.Contains('only after a project explicitly adopts stable Framework 1.15.1')-and$evaluationText.Contains('Use normal project tasks')-and$evaluationText.Contains('originating task')-and$evaluationText.Contains('do not require a fixed sample count or time window')-and$evaluationText.Contains('does not block the stable seal')-and-not$evaluationText.Contains('exactly 20 admitted samples')-and-not$evaluationText.Contains('INSUFFICIENT_SAMPLE / NO_ADVANCE')) 'post-release-project-observation-nonblocking-no-synthetic-work'
...
'@
    ...
    Write-Utf8 $standardTaskPath "# TASK-STANDARD-001 - fixture`n`n- Task schema: 1.15.1`n- Owner: owner-fixture`n- Work route: actor=executor-fixture; role=EXECUTOR; phase=IMPLEMENT`n- Range summary: profile=STANDARD; lifecycle=ACTIVE; expected_paths=[]; actual_paths=[]`n"
    ...
    Write-Utf8 $standardTaskPath "# TASK-STANDARD-001 - fixture`n`n- Task schema: 1.15.1`n- Owner: owner-fixture`n- Work route: actor=reviewer-fixture; role=REVIEWER; phase=REVIEW`n- Range summary: profile=STANDARD; lifecycle=ACTIVE; expected_paths=[]; actual_paths=[]`n"
    ...
    Write-Utf8 $missingRouteTaskPath "# TASK-MISSING-ROUTE-001 - fixture`n`n- Task schema: 1.15.1`n- Owner: owner-fixture`n- Range summary: profile=STANDARD; lifecycle=ACTIVE; expected_paths=[]; actual_paths=[]`n"
    ...
    Copy-Item -LiteralPath $candidateRoot -Destination (Join-Path $target 'framework\versions\1.15.1') -Recurse
    $null=Seal-ReleaseFixture (Join-Path $target 'framework\versions\1.15.1') 'MAINTENANCE_TARGET_TEST_FIXTURE'
    ...
    $installedFramework = Join-Path $target 'framework\versions\1.15.1'
    ...
    Assert-True ($installedActual.Count -eq 88 -and ($installedActual -join "`n") -ceq ($actual -join "`n")) 'maintenance-target-full-sealed-fixture-installed'
    ...
    Assert-True ($missingPin.Code -ne 0 -and $missingPin.Text.Contains('TARGET_REQUIRED_FILE_MISSING|framework/versions/1.15.1/RECOVERY_CORE.md')) 'maintenance-resolver-pin-entry-missing-rejected'
...
'@
    ...
    $repoLocalConfig = [ordered]@{schemaVersion=3;id='repo-local-fixture';displayName='Repo Local Fixture';controlPlaneLayout='repo-local';repositoryRoot='..';frameworkVersion='1.15.1';frameworkToolBackend='powershell7';routineExcludedPaths=@('private/secret.txt');frameworkCapabilities=[pscustomobject]@{KNOWLEDGE_REFERENCE=[pscustomobject]@{enabled=$false}}}
    ...
    $repoLocal4Raw=$projectStarterText.Replace('{{PROJECT_ID_JSON}}','"repo-local-schema4-fixture"').Replace('{{DISPLAY_NAME_JSON}}','"Repo Local Schema4 Fixture"').Replace('{{FRAMEWORK_VERSION_JSON}}','"1.15.1"')
    ...
    Write-Utf8 $domainExternalTaskPath "# FIXTURE-EXTERNAL-001 - external authorization fixture`n`n- Task schema: 1.15.1`n- Owner: owner-fixture`n- Work route: actor=owner-fixture; role=DOMAIN_OWNER; phase=EXTERNAL`n- Range summary: profile=STANDARD; lifecycle=ACTIVE; expected_paths=[src/public.txt]; actual_paths=[]`n"
    ...
    Copy-Item -LiteralPath $candidateRoot -Destination (Join-Path $correctionFrameworkRoot 'framework\versions\1.15.1') -Recurse
    $null=Seal-ReleaseFixture (Join-Path $correctionFrameworkRoot 'framework\versions\1.15.1') 'CORRECTION_TEST_FIXTURE'
    ...
    $correctionConfig=[ordered]@{schemaVersion=3;id='correction-fixture';displayName='Correction Fixture';controlPlaneLayout='repo-local';repositoryRoot='..';frameworkVersion='1.15.1';frameworkToolBackend='powershell7';routineExcludedPaths=@();frameworkCapabilities=[ordered]@{}}
    ...
    $sealedCoveragePath=Join-Path $correctionFrameworkRoot 'framework\versions\1.15.1\CORRECTION_COVERAGE.json';$sealedCoverageOriginal=Get-Content -Raw -Encoding utf8 -LiteralPath $sealedCoveragePath
    $sealedCatalogPath=Join-Path $correctionFrameworkRoot 'framework\versions\1.15.1\PROCESS_REQUIREMENTS.json'
    ...
    $entry113=@($coverageFixture.versions|Where-Object{[string]$_.version-ceq'1.15.1'})[0]
    ...
    $correctionArgs=@('-ProjectRoot',$correctionRoot,'-FrameworkRoot',$correctionFrameworkRoot,'-TargetVersion','1.15.1','-ExpectedProjectConfigIdentity',(Get-Identity $correctionConfigPath),'-ExpectedCorrectionsIdentity',(Get-Identity $correctionPath),'-Operation','PRECHECK','-AsJson')
    ...
    $driftArgs=@('-ProjectRoot',$correctionRoot,'-FrameworkRoot',$correctionFrameworkRoot,'-TargetVersion','1.15.1','-ExpectedProjectConfigIdentity',(Get-Identity $correctionConfigPath),'-ExpectedCorrectionsIdentity',(Get-Identity $correctionPath),'-Operation','PRECHECK','-AsJson')
    ...
    $invalidMapping=$coverageFixture|ConvertTo-Json -Depth 30|ConvertFrom-Json;$invalidEntry=@($invalidMapping.versions|Where-Object{[string]$_.version-ceq'1.15.1'})[0];$invalidEntry.incorporationMappings[0].nativeCatalogIdentity='0|'+('0'*64);Write-Utf8 $sealedCoveragePath ($invalidMapping|ConvertTo-Json -Depth 30);$null=Seal-ReleaseFixture (Split-Path -Parent $sealedCoveragePath) 'INVALID_MAPPING_FIXTURE'
    ...
    $correctionDuplicateRun=Invoke-Ps $correctionChecker @('-ProjectRoot',$correctionRoot,'-FrameworkRoot',$correctionFrameworkRoot,'-TargetVersion','1.15.1','-ExpectedProjectConfigIdentity',(Get-Identity $correctionConfigPath),'-ExpectedCorrectionsIdentity',(Get-Identity $correctionPath),'-Operation','PRECHECK','-AsJson')
    ...
    $missingCorrectionRun=Invoke-Ps $correctionChecker @('-ProjectRoot',$missingCorrectionRoot,'-FrameworkRoot',$correctionFrameworkRoot,'-TargetVersion','1.15.1','-ExpectedProjectConfigIdentity',(Get-Identity $missingCorrectionConfigPath),'-Operation','PRECHECK','-AllowMissingCorrections','-AsJson')
    ...
    $processResolver=Join-Path $correctionFrameworkRoot 'framework\versions\1.15.1\scripts\resolve-process-requirements.ps1'
    $processTaskPath=Join-Path $correctionRoot '.ai-workspace\tasks\active\PROCESS-FIXTURE-001.md'
    Write-Utf8 $processTaskPath "# PROCESS-FIXTURE-001 - process fixture`n`n- Task schema: 1.15.1`n- Owner: owner-fixture`n- Work route: actor=owner-fixture; role=DOMAIN_OWNER; phase=IMPLEMENT`n- Range summary: profile=STANDARD; lifecycle=ACTIVE; expected_paths=[src/public.txt]; actual_paths=[]`n"
    ...
    $clearNoMatchDiscover=Invoke-Ps $processResolver @('-InputPath',$discoverInputPath,'-AsJson');$clearNoMatchValue=$clearNoMatchDiscover.Output[-1]|ConvertFrom-Json
    ...
    Assert-True ($clearNoMatchDiscover.Code-eq0-and-not(@($clearNoMatchValue.evidenceCeilings)-contains'INTENT_AMBIGUITY_CONSERVATIVE_LOAD')-and@($clearNoMatchValue.selectedRequirements).Count-lt@($ambiguousValue.selectedRequirements).Count) 'process-v2-clear-description-no-match-does-not-trigger-conservative-load'
    ...
    Assert-True ($runtimeGuard112Offset-ge0-and$runtimeGuard113Offset-ge0-and$runtimeGuard1141Offset-ge0-and$runtimeGuard115Offset-ge0-and$transactionRuntimeOffset-gt$runtimeGuard112Offset-and$transactionRuntimeOffset-gt$runtimeGuard113Offset-and$transactionRuntimeOffset-gt$runtimeGuard1141Offset-and$transactionRuntimeOffset-gt$runtimeGuard115Offset) 'upgrade-powershell7-guard-precedes-transaction-recovery'
    ...
    if (-not $SkipRootMigration) {
        ...
        Copy-Item -LiteralPath $candidateRoot -Destination (Join-Path $rootFlow 'framework\versions\1.15.1') -Recurse
        ...
        if($IsWindows){
            ...
            $runtimeGuardOutput=@(& $windowsPowerShell -NoProfile -NonInteractive -ExecutionPolicy Bypass -File (Join-Path $rootFlow 'scripts\upgrade-project.ps1') -ProjectId 'runtime-guard-fixture' -ToVersion '1.15.1' -RepositoryPath $runtimeGuardRepo -Apply 2>&1|ForEach-Object{[string]$_})
        ...
        $fixtureManifestPath=Join-Path $rootFlow 'framework\versions\1.15.1\RELEASE_MANIFEST.json'
        $fixtureVersionPath=Join-Path $rootFlow 'framework\versions\1.15.1\VERSION.json';$fixtureVersion=Get-Content -Raw -Encoding utf8 -LiteralPath $fixtureVersionPath|ConvertFrom-Json;$fixtureVersion.lifecycle='STABLE';$fixtureVersion.consumable=$true;$fixtureVersion.projectPinEligible=$true;Write-Utf8 $fixtureVersionPath ($fixtureVersion|ConvertTo-Json -Depth 20)
        $fixtureLoadPath=Join-Path $rootFlow 'framework\versions\1.15.1\LOAD_MANIFEST.json';$fixtureLoad=Get-Content -Raw -Encoding utf8 -LiteralPath $fixtureLoadPath|ConvertFrom-Json;$fixtureLoad.lifecycle='STABLE';Write-Utf8 $fixtureLoadPath ($fixtureLoad|ConvertTo-Json -Depth 20)
        ...
        $pendingRegister=Invoke-Ps $register @($baseRegisterArgs + @('-FrameworkVersion','1.15.1'))
        Assert-True ($pendingRegister.Code -ne 0 -and $pendingRegister.Text.Contains('FRAMEWORK_RELEASE_NOT_SEALED|1.15.1')) 'register-pending-release-rejected-before-project-write'
        ...
        $missingControllerRegister=Invoke-Ps $register @('-ProjectId','missing-controller-fixture','-DisplayName','Missing Controller Fixture','-RepositoryPath',$missingControllerRepo,'-FrameworkVersion','1.15.1')
        ...
        $unsupportedRegister=Invoke-Ps $register @('-ProjectId','unsupported-platform-fixture','-DisplayName','Unsupported Platform Fixture','-RepositoryPath',$unsupportedRegisterRepo,'-ControllerId','controller-unsupported','-FrameworkVersion','1.15.1','-Apply')
        ...
        foreach($agentsFormatCase in @(
            ...
            $formatRegister=Invoke-Ps $register @('-ProjectId',('agents-'+$agentsFormatCase.Name+'-fixture'),'-DisplayName','Agents Format Fixture','-RepositoryPath',$formatRepo,'-ControllerId','controller-agents-format','-FrameworkVersion','1.15.1','-Apply')
        ...
        $shadowUpgradeWithoutRepository=Invoke-Ps $upgrade @('-ProjectId','shadow-fixture','-ToVersion','1.15.1','-ControllerId','controller-shadow')
        ...
        $previewRegister=Invoke-Ps $register @($baseRegisterArgs + @('-FrameworkVersion','1.15.1'))
        if ($previewRegister.Code -ne 0 -or -not $previewRegister.Text.Contains('WHAT_IF')) { Write-Output ('DIAG|register-explicit-1.15.1-preview|' + $previewRegister.Code + '|' + $previewRegister.Text) }
        ...
        $registrationStartCollisionArgs=@('-ProjectId','registration-start-collision-fixture','-DisplayName','Registration Start Collision Fixture','-RepositoryPath',$registrationStartCollisionRepo,'-ControllerId','controller-registration-start-collision','-FrameworkVersion','1.15.1')
        ...
        $failureArgs=@('-ProjectId','registration-failure-fixture','-DisplayName','Registration Failure Fixture','-RepositoryPath',$failureRepo,'-ControllerId','controller-registration-failure','-FrameworkVersion','1.15.1')
        ...
        $applyRegister=Invoke-Ps $register @($baseRegisterArgs + @('-FrameworkVersion','1.15.1','-Apply'))
        Assert-True ($applyRegister.Code -eq 0 -and $applyRegister.Text.Contains('CREATED')) 'register-explicit-1.15.1-apply'
        ...
        $registrationCollisionRun=Invoke-Ps $register @($baseRegisterArgs + @('-FrameworkVersion','1.15.1','-Apply'));$registrationCollisionAfter=Get-ExactObjectRows $consumer $registrationCollisionRelatives
        ...
        $registrationIdDriftRun=Invoke-Ps $register @($baseRegisterArgs + @('-FrameworkVersion','1.15.1','-Apply'))
        ...
        $registrationPhysicalExtraRun=Invoke-Ps $register @($baseRegisterArgs + @('-FrameworkVersion','1.15.1','-Apply'))
        ...
        $registrationTraversalRun=Invoke-Ps $register @($baseRegisterArgs + @('-FrameworkVersion','1.15.1','-Apply'))
        ...
        $registrationExtraRun=Invoke-Ps $register @($baseRegisterArgs + @('-FrameworkVersion','1.15.1','-Apply'))
        ...
        $registrationResume=Invoke-Ps $register @($baseRegisterArgs + @('-FrameworkVersion','1.15.1','-Apply'))
        ...
        $registrationUnknown=Invoke-Ps $register @($baseRegisterArgs + @('-FrameworkVersion','1.15.1','-Apply'))
        ...
        [IO.File]::Copy((Join-Path $registrationActive 'new\.ai-workspace\STATUS.md'),$registrationUnknownPath,$true);$registrationCleanup=Invoke-Ps $register @($baseRegisterArgs + @('-FrameworkVersion','1.15.1','-Apply'))
        ...
        $existingMissingController=Invoke-Ps $register @('-ProjectId','explicit-fixture','-DisplayName','Explicit Fixture','-RepositoryPath',$consumer,'-FrameworkVersion','1.15.1')
        ...
        $repeatRegister=Invoke-Ps $register @($baseRegisterArgs + @('-FrameworkVersion','1.15.1'))
        Assert-True ($repeatRegister.Code -eq 0 -and $repeatRegister.Text.Contains('ALREADY_REGISTERED')) 'register-explicit-1.15.1-repeat'
        ...
        $sameVersionArgs=@('-ProjectId','explicit-fixture','-ToVersion','1.15.1','-RepositoryPath',$consumer,'-ControllerId','controller-explicit')
        $sameVersionPreview=Invoke-Ps $upgrade $sameVersionArgs
        $sameVersionApply=Invoke-Ps $upgrade @($sameVersionArgs+'-Apply')
        Assert-True ($sameVersionPreview.Code-eq0-and$sameVersionPreview.Text.Contains('WHAT_IF|from=1.15.1|to=1.15.1|objects=0|transaction=none')) 'upgrade-schema4-1.15-to-1.15-preview-validates-noop'
        ...
        $actor114Register=Invoke-Ps $register @('-ProjectId','actor-route-114-fixture','-DisplayName','Actor Route 1.14 Fixture','-RepositoryPath',$actor114Repo,'-ControllerId','controller-actor-route-114','-FrameworkVersion','1.14.0','-Apply')
        ...
        $actor114OldTaskIdentity=Get-Identity $actor114TaskPath;$actor114BaseArgs=@('-ProjectId','actor-route-114-fixture','-ToVersion','1.15.1','-RepositoryPath',$actor114Repo,'-ControllerId','controller-actor-route-114','-ActorRouteTaskPath',$actor114TaskRelative,'-ExpectedActorRouteTaskIdentity',$actor114OldTaskIdentity,'-ActorRouteActor','executor-current')
        ...
        Assert-True ($actor114MissingProcess.Code-ne0-and$actor114MissingProcess.Text.Contains('CURRENT_PIN_PROCESS_INPUT_REQUIRED')) 'upgrade-1.14-to-1.15.1-requires-current-process-input'
        ...
        Assert-True ($actor114NonBudget.Code-ne0-and$actor114NonBudget.Text.Contains('CURRENT_PIN_PROCESS_RESOLVER_NOT_EXACT_BUDGET_FAILURE')) 'upgrade-current-pin-bridge-rejects-non-budget-resolver-result'
        ...
        $actor114Preview=Invoke-Ps $upgrade $actor114Args;$actor114WriteLines=@($actor114Preview.Output|Where-Object{$_-clike'UPGRADE_WRITESET|*'});if($actor114Preview.Code-ne0-or$actor114WriteLines.Count-ne1){throw ('ASSERT_FAIL|upgrade-actor-bound-1.14-to-1.15.1-preview|code='+$actor114Preview.Code+'|output='+$actor114Preview.Text)};$actor114WriteLine=$actor114WriteLines[0];$actor114WritePaths=@($actor114WriteLine.Split('|')[1..($actor114WriteLine.Split('|').Count-1)]);$actor114PreimageMap=@{};$actor114PostimageMap=@{}
        ...
        Assert-True ($actor114WrongDecisionRun.Code-ne0-and$actor114WrongDecisionRun.Text.Contains('CURRENT_PIN_BRIDGE_USER_DECISION_DRIFT')-and-not(Test-Path -LiteralPath (Join-Path $actor114Repo '.ai-workspace/upgrade-recovery/1.15.1'))-and(Get-Identity $actor114TaskPath)-ceq$actor114OldTaskIdentity) 'upgrade-current-pin-bridge-rejects-user-decision-drift-before-write'
        ...
        $actor114Recovery=Join-Path $actor114Repo '.ai-workspace/upgrade-recovery/1.15.1';$actor114State=Get-Content -Raw -Encoding utf8 -LiteralPath (Join-Path $actor114Recovery 'state.json')|ConvertFrom-Json
        ...
        Assert-True ($actor114Register.Code-eq0-and$actor114Preview.Code-eq0-and$actor114Preview.Text.Contains('CURRENT_PIN_BUDGET_BRIDGE|from=1.14.0|reason=SELECTED_RULE_PACK_BUDGET_EXCEEDED')-and$actor114Apply.Code-eq0-and$actor114DriftRun.Code-ne0-and$actor114DriftRun.Text.Contains('MANAGED_ROUTER_DESTINATION_REPARSE')-and(Get-Identity (Join-Path $actor114Repo '.ai-workspace/project.json'))-ceq$actor114ConfigPreimage-and(Get-Identity $actor114TaskPath)-ceq$actor114OldTaskIdentity-and-not(Test-Path -LiteralPath (Join-Path $actor114External 'skills\ai-workspace-router\SKILL.md'))-and(Test-Path -LiteralPath $actor114Recovery -PathType Container)) 'upgrade-actor-bound-router-reparse-drift-retains-recovery-zero-external-write'

        $actor1141Repo=Join-Path $rootFlow 'consumer-actor-route-1141-upgrade';New-GitRepo $actor1141Repo
        $actor1141Register=Invoke-Ps $register @('-ProjectId','actor-route-1141-fixture','-DisplayName','Actor Route 1.14.1 Fixture','-RepositoryPath',$actor1141Repo,'-ControllerId','controller-actor-route-1141','-FrameworkVersion','1.14.1','-Apply');$actor1141TaskRelative='.ai-workspace/tasks/active/ACTOR-ROUTE-1141-001.md';$actor1141TaskPath=Join-Path $actor1141Repo ($actor1141TaskRelative.Replace('/','\'));Write-Utf8 $actor1141TaskPath "# ACTOR-ROUTE-1141-001 - fixture`n`n- Task schema: 1.14.1`n- Owner: owner-fixture`n- Work route: actor=executor-current; role=EXECUTOR; phase=IMPLEMENT`n- Range summary: profile=CRITICAL; lifecycle=ACTIVE; current_exact=fixture-v1; expected_paths=[]; actual_paths=[]`n- Phase gate: FALSE`n- Proportionality: existing=partial; classification=framework_gap; minimum_sufficient_fix=current-pin bridge preview; added_machinery=NONE; escalation_trigger=Framework pin changes`n"
        $actor1141BootstrapPath=Join-Path $actor1141Repo '.ai-workspace/BOOTSTRAP.md';$actor1141Bootstrap=Get-Content -Raw -Encoding utf8 -LiteralPath $actor1141BootstrapPath;$actor1141Bootstrap=[regex]::Replace($actor1141Bootstrap,'(?s)(<!-- PROJECT-CUSTOM:BEGIN -->\n).*?(\n<!-- PROJECT-CUSTOM:END -->)','$1'+('current-pin bridge fixture rule '+('x'*12288))+'$2');Write-Utf8 $actor1141BootstrapPath $actor1141Bootstrap;$actor1141TaskIdentity=Get-Identity $actor1141TaskPath
        $actor1141Intent=[ordered]@{schemaVersion=1;objective='Read-only reproduce the current-pin process requirement selection budget before framework adoption';requestedActionKind='NONE';requestedResultKind='PLAN';semanticHints=@('Framework adoption','current-pin bridge');pathHints=@($actor1141TaskRelative);capabilityHints=@();mutationHints=@();externalHints=@();ambiguityState='CLEAR'};$actor1141ProcessPath=Join-Path $actor1141Repo '.ai-workspace/tmp/current-pin-process-input.json';$actor1141Process=[ordered]@{schemaVersion=2;mode='DISCOVER';projectRoot=$actor1141Repo;frameworkRoot=$rootFlow;taskPath=$actor1141TaskPath;expectedProjectConfigIdentity=Get-Identity (Join-Path $actor1141Repo '.ai-workspace/project.json');expectedCorrectionsIdentity=Get-Identity (Join-Path $actor1141Repo '.ai-workspace/corrections.json');expectedTaskIdentity=$actor1141TaskIdentity;observedActor='executor-current';capabilities=@();exactPaths=@($actor1141TaskRelative);forbiddenPaths=@('src/','test/','assets/','docs/');protectedPaths=@('.ai-workspace/');authorizationPackagePath='NOT_REQUIRED';expectedAuthorizationIdentity='NOT_REQUIRED';userDecision='USER_FIXTURE_ACTOR_BOUND_1141_UPGRADE';recoveryState='FULL_COLD';hostEnforcementGrade='FRAMEWORK_GATED';invocationState='PROVEN_EXPLICIT';intentEnvelope=$actor1141Intent;evaluationOnly=$true};Write-Utf8 $actor1141ProcessPath ($actor1141Process|ConvertTo-Json -Depth 20);$actor1141Preview=Invoke-Ps $upgrade @('-ProjectId','actor-route-1141-fixture','-ToVersion','1.15.1','-RepositoryPath',$actor1141Repo,'-ControllerId','controller-actor-route-1141','-ActorRouteTaskPath',$actor1141TaskRelative,'-ExpectedActorRouteTaskIdentity',$actor1141TaskIdentity,'-ActorRouteActor','executor-current','-CurrentProcessInputPath',$actor1141ProcessPath,'-ExpectedCurrentProcessInputIdentity',(Get-Identity $actor1141ProcessPath))
        Assert-True ($actor1141Register.Code-eq0-and$actor1141Preview.Code-eq0-and$actor1141Preview.Text.Contains('CURRENT_PIN_BUDGET_BRIDGE|from=1.14.1|reason=SELECTED_RULE_PACK_BUDGET_EXCEEDED')) 'upgrade-current-pin-bridge-success-preview-from-1.14.1'
        ...
        $unsupportedToolchain=$fixtureToolchainOriginal|ConvertFrom-Json;$unsupportedToolchain.officialBackends[0].platforms=@('linux');Write-Utf8 $fixtureToolchainPath ($unsupportedToolchain|ConvertTo-Json -Depth 20);$null=Seal-ReleaseFixture $fixtureVersionRoot 'UNSUPPORTED_PLATFORM_TEST_FIXTURE'
        $unsupportedUpgrade=Invoke-Ps $upgrade @('-ProjectId','upgrade-fixture','-ToVersion','1.15.1','-RepositoryPath',$upgradeRepo,'-ControllerId','controller-upgrade','-Apply')
    ...
    $knowledgeConfig=[ordered]@{schemaVersion=4;id='knowledge-fixture';displayName='Knowledge Fixture';controlPlaneLayout='repo-local';repositoryRoot='..';frameworkVersion='1.15.1';frameworkToolBackend='powershell7';routineExcludedPaths=@();frameworkCapabilities=[ordered]@{KNOWLEDGE_REFERENCE=[ordered]@{enabled=$true;indexLocator='knowledge/index.json'}};processPolicy=[ordered]@{schemaVersion=1;locator='.ai-workspace/process-policy.json'}}
    ...
    if($null-ne$pwsh){
        ...
        Write-Utf8 $correctionPath $correctionDuplicate
        try{$ps7CorrectionDuplicate=Invoke-PsHost $pwsh.Source $correctionChecker @('-ProjectRoot',$correctionRoot,'-FrameworkRoot',$correctionFrameworkRoot,'-TargetVersion','1.15.1','-ExpectedProjectConfigIdentity',(Get-Identity $correctionConfigPath),'-ExpectedCorrectionsIdentity',(Get-Identity $correctionPath),'-Operation','PRECHECK','-AsJson')}
    ...
    if (-not $SkipBaseline) {
        ...
        $stableBaselineRoot=Join-Path $frameworkRoot 'versions\1.15.0'
        ...
        Assert-True ([string]$stableBaselineManifest.lifecycle-ceq'STABLE'-and[int]$stableBaselineManifest.fileCount-eq$stableBaselineFacts.FileCount-and[int64]$stableBaselineManifest.totalBytes-eq$stableBaselineFacts.TotalBytes-and[string]$stableBaselineManifest.canonical-ceq$stableBaselineFacts.Canonical-and[string]$stableBaselineManifest.canonical-ceq'1408C9555955482651DD3EFD544A8AD36797CB04DFC52990E861C151220B076F') 'baseline-1.15.0-immutable-stable-seal'
...
$resultScope=if($resultSealed){'Framework-1.15.1-stable'}else{'Framework-1.15.1-candidate'}
