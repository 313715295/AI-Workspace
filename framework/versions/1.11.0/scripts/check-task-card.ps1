...
if ($schemaMatches.Count -ne 1 -or $taskSchema -notin @('1.5','1.5.2','1.9.0','1.10.0','1.11.0')) { Add-Reason $reasons 'TASK_SCHEMA' }
...
$workRole = if ($workRouteMatches.Count -eq 1) { $workRouteMatches[0].Groups['role'].Value } else { '' }
$workPhase = if ($workRouteMatches.Count -eq 1) { $workRouteMatches[0].Groups['phase'].Value } else { '' }
...
if ($taskSchema -in @('1.9.0','1.10.0','1.11.0') -and $profile -ceq 'CRITICAL') {
...
if ($taskSchema -in @('1.5.2','1.9.0','1.10.0','1.11.0') -and $profile -ceq 'CRITICAL') {
...
$loadContext = if ($workRouteMatches.Count -eq 1) { 'DECLARED' } else { 'LEGACY_LOAD_CONTEXT' }
$routeSuffix = if ($loadContext -ceq 'DECLARED') { '|role=' + $workRole + '|phase=' + $workPhase } else { '|evidenceCeiling=ROLE_PHASE_NOT_DECLARED' }
