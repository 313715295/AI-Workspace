...
function Assert-DirectoryChainWithoutReparse([string]$Root,[string]$Child) {
    ...
    if (-not $childPath.Equals($rootPath,[StringComparison]::OrdinalIgnoreCase) -and
        -not $childPath.StartsWith($prefix,[StringComparison]::OrdinalIgnoreCase)) { throw 'GIT_TOP_DOES_NOT_CONTAIN_CWD' }
    $cursor = $rootPath
    ...
    if ($childPath.Equals($rootPath,[StringComparison]::OrdinalIgnoreCase)) { return }
...
if ([string]$package.frameworkVersion -cne '1.11.0') { Add-Reason $reasons 'FRAMEWORK_VERSION' }
...
if ([int]$package.schemaVersion -eq 1) {
    try {
        ...
        $schema1Fields = @('schemaVersion','id','displayName','controlPlaneLayout','repositoryRoot','frameworkVersion','routineExcludedPaths','frameworkCapabilities')
        ...
        if (-not (Test-JsonInteger $schema1Config.schemaVersion) -or [int]$schema1Config.schemaVersion -ne 3 -or
            ...
            -not ($schema1Config.frameworkVersion -is [string]) -or [string]$schema1Config.frameworkVersion -cne '1.11.0' -or
...
if ([int]$package.schemaVersion -eq 2) {
    ...
    if ([string]::IsNullOrWhiteSpace($ProjectConfigPath) -or [string]::IsNullOrWhiteSpace($ExpectedProjectConfigIdentity)) {
        ...
        try {
            ...
            $configFields = @('schemaVersion','id','displayName','controlPlaneLayout','repositoryRoot','frameworkVersion','routineExcludedPaths','frameworkCapabilities','frameworkTarget')
            ...
            if (-not (Test-JsonInteger $config.schemaVersion) -or [int]$config.schemaVersion -ne 4 -or
                ...
                -not ($config.frameworkVersion -is [string]) -or [string]$config.frameworkVersion -cne '1.11.0' -or
