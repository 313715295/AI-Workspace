...
try {
    ...
    try{
        $coveragePath=Resolve-ChildFile $framework @('framework','versions','1.11.0','CORRECTION_COVERAGE.json') 'COVERAGE'
        ...
        if(-not(Test-JsonInteger $coverage.schemaVersion)-or[int]$coverage.schemaVersion-ne1-or[string]$coverage.releaseVersion-cne'1.11.0'-or-not($coverage.versions-is[System.Array])){throw 'COVERAGE_VALUES'}
