...
function Render-MaintenanceStarter([string]$Starter,[string]$ControlRoot) {
    ...
    Get-ChildItem -LiteralPath $destination -Recurse -Force -File | ForEach-Object {
        ...
        $text = $text.Replace('{{FRAMEWORK_VERSION}}','1.11.0')
...
function New-AuthorizationPackage(
    ...
    $package = [ordered]@{
        schemaVersion=$Schema; frameworkVersion='1.11.0'; taskId='FIXTURE-001'; profile='STANDARD'; lifecycle='ACTIVE'
...
Assert-True ([string]$version.version -ceq '1.11.0' -and ($candidateVersionState-or$stableVersionState) -and [string]$version.releaseClass -ceq 'MINOR' -and [string]$version.baseline -ceq '1.10.0' -and $null -eq $version.PSObject.Properties['currentEligible']) 'version-candidate-or-stable-minor-baseline-1.10.0-no-global-selector-field'
...
Assert-True ($loadResolverText.Contains('LOAD_MANIFEST_NOT_STABLE_1_11_0') -and -not $loadResolverText.Contains('LOAD_MANIFEST_NOT_STABLE_1_7_0')) 'load-resolver-diagnostic-version-current'
...
try {
    $null = $normalProjectTemplate.Replace('{{PROJECT_ID_JSON}}','"fixture"').Replace('{{DISPLAY_NAME_JSON}}','"Fixture"').Replace('{{FRAMEWORK_VERSION_JSON}}','"1.11.0"') | ConvertFrom-Json
...
'@
    ...
    Write-Utf8 $standardTaskPath "# TASK-STANDARD-001 - fixture`n`n- Task schema: 1.11.0`n- Owner: owner-fixture`n- Work route: role=EXECUTOR; phase=IMPLEMENT`n- Range summary: profile=STANDARD; lifecycle=ACTIVE; expected_paths=[]; actual_paths=[]`n"
    ...
    Write-Utf8 $standardTaskPath "# TASK-STANDARD-001 - fixture`n`n- Task schema: 1.11.0`n- Owner: owner-fixture`n- Work route: role=REVIEWER; phase=REVIEW`n- Range summary: profile=STANDARD; lifecycle=ACTIVE; expected_paths=[]; actual_paths=[]`n"
    ...
    Write-Utf8 $missingRouteTaskPath "# TASK-MISSING-ROUTE-001 - fixture`n`n- Task schema: 1.11.0`n- Owner: owner-fixture`n- Range summary: profile=STANDARD; lifecycle=ACTIVE; expected_paths=[]; actual_paths=[]`n"
    ...
    Copy-Item -LiteralPath $candidateRoot -Destination (Join-Path $target 'framework\versions\1.11.0') -Recurse
    ...
    $installedFramework = Join-Path $target 'framework\versions\1.11.0'
    ...
    Assert-True ($missingPin.Code -ne 0 -and $missingPin.Text.Contains('TARGET_REQUIRED_FILE_MISSING|framework/versions/1.11.0/RECOVERY_CORE.md')) 'maintenance-resolver-pin-entry-missing-rejected'
...
'@
    ...
    $repoLocalConfig = [ordered]@{schemaVersion=3;id='repo-local-fixture';displayName='Repo Local Fixture';controlPlaneLayout='repo-local';repositoryRoot='..';frameworkVersion='1.11.0';routineExcludedPaths=@('private/secret.txt');frameworkCapabilities=[pscustomobject]@{KNOWLEDGE_REFERENCE=[pscustomobject]@{enabled=$false}}}
    ...
    $criticalReviewPackage = [ordered]@{
        schemaVersion=1;frameworkVersion='1.11.0';taskId='FIXTURE-REVIEW-001';profile='CRITICAL';lifecycle='ACTIVE'
    ...
    Copy-Item -LiteralPath $candidateRoot -Destination (Join-Path $correctionFrameworkRoot 'framework\versions\1.11.0') -Recurse
    $null=Seal-ReleaseFixture (Join-Path $correctionFrameworkRoot 'framework\versions\1.11.0') 'CORRECTION_TEST_FIXTURE'
    ...
    $sealedCoveragePath=Join-Path $correctionFrameworkRoot 'framework\versions\1.11.0\CORRECTION_COVERAGE.json';$sealedCoverageOriginal=Get-Content -Raw -Encoding utf8 -LiteralPath $sealedCoveragePath
    ...
    $nonCumulative=$sealedCoverageOriginal|ConvertFrom-Json;$entry110=@($nonCumulative.versions|Where-Object{[string]$_.version-ceq'1.11.0'})[0];$entry110.incorporatedCorrectionIds=@($entry110.incorporatedCorrectionIds|Where-Object{[string]$_-cne'COMPACT_NON_INTERRUPT_DELIVERY'});Write-Utf8 $sealedCoveragePath ($nonCumulative|ConvertTo-Json -Depth 20);$null=Seal-ReleaseFixture (Split-Path -Parent $sealedCoveragePath) 'NONCUMULATIVE_TEST_FIXTURE'
    $nonCumulativeRun=Invoke-Ps $correctionChecker @($correctionArgs[0..4]+@('1.11.0')+$correctionArgs[6..($correctionArgs.Count-1)]);$nonCumulativeValue=$nonCumulativeRun.Output[-1]|ConvertFrom-Json
    ...
    $overlapCoverage=$sealedCoverageOriginal|ConvertFrom-Json;$entry110=@($overlapCoverage.versions|Where-Object{[string]$_.version-ceq'1.11.0'})[0];$entry110.conflictingCorrectionIds=@('OWNER_FIRST_DIRECT_DOMAIN_ROUTE');Write-Utf8 $sealedCoveragePath ($overlapCoverage|ConvertTo-Json -Depth 20);$null=Seal-ReleaseFixture (Split-Path -Parent $sealedCoveragePath) 'OVERLAP_TEST_FIXTURE'
    $overlapRun=Invoke-Ps $correctionChecker @($correctionArgs[0..4]+@('1.11.0')+$correctionArgs[6..($correctionArgs.Count-1)]);$overlapValue=$overlapRun.Output[-1]|ConvertFrom-Json
    ...
    if (-not $SkipRootMigration) {
        ...
        Copy-Item -LiteralPath $candidateRoot -Destination (Join-Path $rootFlow 'framework\versions\1.11.0') -Recurse
        Assert-True (-not (Test-Path -LiteralPath (Join-Path $rootFlow 'framework\CURRENT'))) 'root-global-version-selector-absent'

        $fixtureManifestPath=Join-Path $rootFlow 'framework\versions\1.11.0\RELEASE_MANIFEST.json'
        $fixtureVersionPath=Join-Path $rootFlow 'framework\versions\1.11.0\VERSION.json'
        ...
        $pendingRegister=Invoke-Ps $register @($baseRegisterArgs + @('-FrameworkVersion','1.11.0'))
        Assert-True ($pendingRegister.Code -ne 0 -and $pendingRegister.Text.Contains('FRAMEWORK_RELEASE_NOT_SEALED|1.11.0')) 'register-pending-release-rejected-before-project-write'
        ...
        $shadowUpgradeWithoutRepository=Invoke-Ps $upgrade @('-ProjectId','shadow-fixture','-ToVersion','1.11.0','-ControllerId','controller-shadow')
        ...
        $previewRegister=Invoke-Ps $register @($baseRegisterArgs + @('-FrameworkVersion','1.11.0'))
        if ($previewRegister.Code -ne 0 -or -not $previewRegister.Text.Contains('WHAT_IF')) { Write-Output ('DIAG|register-explicit-1.11.0-preview|' + $previewRegister.Code + '|' + $previewRegister.Text) }
        Assert-True ($previewRegister.Code -eq 0 -and $previewRegister.Text.Contains('WHAT_IF')) 'register-explicit-1.11.0-preview'
        $applyRegister=Invoke-Ps $register @($baseRegisterArgs + @('-FrameworkVersion','1.11.0','-Apply'))
        Assert-True ($applyRegister.Code -eq 0 -and $applyRegister.Text.Contains('CREATED')) 'register-explicit-1.11.0-apply'
        $repeatRegister=Invoke-Ps $register @($baseRegisterArgs + @('-FrameworkVersion','1.11.0'))
        Assert-True ($repeatRegister.Code -eq 0 -and $repeatRegister.Text.Contains('ALREADY_REGISTERED')) 'register-explicit-1.11.0-repeat'
        ...
        Assert-True ([string]$registeredConfig.frameworkVersion -ceq '1.11.0' -and $registeredFiles.Count -eq 10 -and $registeredBootstrap.Contains('scripts/resolve-workflow-route.ps1') -and $registeredBootstrap.Contains('PROJECT-CORRECTIONS:BEGIN') -and [string]$registeredCorrections.projectId -ceq 'explicit-fixture' -and [string]$registeredCorrections.contractVersion -ceq '1.10.0' -and @($registeredCorrections.corrections).Count -eq 0) 'register-materializes-1.11-starter-exact10-with-corrections'
        ...
        $upgradeArgs=@('-ProjectId','upgrade-fixture','-ToVersion','1.11.0','-RepositoryPath',$upgradeRepo,'-ControllerId','controller-upgrade')
        ...
        $conflictEntry=@($conflictCoverage.versions|Where-Object{[string]$_.version-ceq'1.11.0'})[0]
        ...
        Assert-True ($upgradePreview.Code -eq 0 -and $upgradePreview.Text.Contains('Preview only') -and $upgradePreview.Text.Contains('incorporated=2') -and $upgradePreview.Text.Contains('conflicts=0')) 'upgrade-schema3-to-1.11.0-preview-with-correction-report'
        $upgradeApply=Invoke-Ps $upgrade @($upgradeArgs+'-Apply')
        if($upgradeApply.Code-ne0-or-not$upgradeApply.Text.Contains('Updated:')){Write-Output ('DIAG|upgrade-schema3-to-1.11.0-apply|code='+$upgradeApply.Code+'|output='+$upgradeApply.Text)}
        Assert-True ($upgradeApply.Code -eq 0 -and $upgradeApply.Text.Contains('Updated:')) 'upgrade-schema3-to-1.11.0-apply'
        ...
        Assert-True ($downgradeApply.Code-eq0-and[string]$downgradedConfig.frameworkVersion-ceq'1.9.0'-and$downgradedBootstrap.Contains('PROJECT-CORRECTIONS:BEGIN')-and$downgradedBootstrap.Contains('framework/versions/1.11.0/scripts/check-project-corrections.ps1')-and(Get-Identity $upgradeCorrectionsPath)-ceq$upgradeCorrectionsBefore) 'downgrade-preserves-overlay-records-and-correction-bytes'
    }
    ...
    $knowledgeConfig=[ordered]@{schemaVersion=3;id='knowledge-fixture';displayName='Knowledge Fixture';controlPlaneLayout='repo-local';repositoryRoot='..';frameworkVersion='1.11.0';routineExcludedPaths=@();frameworkCapabilities=[ordered]@{KNOWLEDGE_REFERENCE=[ordered]@{enabled=$true;indexLocator='knowledge/index.json'}}}
    ...
    if (-not $SkipBaseline) {
        ...
        $baselineRunner = Join-Path $frameworkRoot 'versions\1.10.0\tests\run-framework-tests.ps1'
        $baseline = Invoke-Ps $baselineRunner @('-SkipRootMigration')
        if ($baseline.Code -ne 0) { Write-Output ('DIAG|baseline-1.10.0|' + $baseline.Text) }
        Assert-True ($baseline.Code -eq 0 -and $baseline.Text.Contains('RESULT|')) 'baseline-1.10.0-regression-suite'
    }
...
$resultScope=if($resultSealed){'Framework-1.11.0-stable'}else{'Framework-1.11.0-candidate'}
