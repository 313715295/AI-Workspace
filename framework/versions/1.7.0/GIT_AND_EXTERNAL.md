...
项目schema3的`routineExcludedPaths`是日常恢复、搜索和Git操作的唯一排除列表。它只承诺普通状态、diff建议和批量操作不展示或操作这些路径，不是机密性、操作系统权限、零Git内部metadata观察或source写授权；source权限仍由current task与authorization决定。

Git inventory/status/diff/index调用版本内`invoke-protected-safe-git.ps1`，传入冻结的project config identity、枚举操作和bounded positive pathset。入口自动附加literal exclusions并关闭rename/copy detection；排除对象不得进入输出。命令失败、配置漂移或排除无法可靠应用时返回不含locator的`UNVERIFIED`，禁止扩大扫描、结果净化后冒充完整事实或统计重试。

用户明确要求处理某个日常排除对象时，current owner另签fresh exact单路径Git包，调用入口的exact override；override的每个allow path必须逐字等于项目排除项，不能用父目录或整仓路径扩大。完成后立即恢复默认排除。项目不得用`assume-unchanged`或`skip-worktree`代替该合同。
...
Maintenance布局调用safe-Git时必须传入控制仓Git top、冻结project config identity和`CONTROL`或配置声明的target repository ID。入口先用resolver复证父目录非Git/无控制面、两个真实Git top、目标固定pin入口与目标仓无常态`.ai-workspace`，再只在所选仓执行bounded Git读取并使用该仓自己的routine exclusions。

两个仓分别拥有HEAD、index、dirty和Git closer；一个仓clean不能证明另一个仓clean。禁止在共同父目录运行Git、跨仓pathspec、一个包stage两个仓或把两次提交描述为原子事务。物理迁移、stable投影、CURRENT、两仓commit/push和consumer pin采用仍是独立授权动作。
...
创建前评估dirty diff复制/应用成本、磁盘、依赖安装/缓存、合并冲突、日常排除与集成顺序，并指定唯一集成负责人。独立Review通常只需读取stable candidate，不以独立worktree作为独立性的形式门。

## 3. External动作分层

### 3.1 简单多远程阶段

同一stable candidate需要推送多个远程时，任务直接冻结唯一Git closer、remote/ref清单和一次阶段授权。closer对每个remote最多尝试一次并诚实记录`SUCCEEDED / FAILED / UNKNOWN`；重试必须是新的授权阶段。Framework不为此建立DTO、ledger、自动retry或补偿事务，也不把一个remote成功推定为其他remote成功。
