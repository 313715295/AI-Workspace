...
switch ($operation) {
    ...
    'ROUTE' {
        $fields = @('operation','projectMatch','cwdGitTopMatch','outcomeMatch','ownerRoleMatch','lineageMatch','resourceRouteMatch','protectionBoundaryMatch','gitDeviceExternalMatch','publicDecisionMatch','requiresDistinctOutcome','standingCreateAuthorized')
        ...
        $boundaryMatch = [bool]$inputObject.projectMatch -and [bool]$inputObject.cwdGitTopMatch -and [bool]$inputObject.ownerRoleMatch -and
            [bool]$inputObject.lineageMatch -and [bool]$inputObject.resourceRouteMatch -and [bool]$inputObject.protectionBoundaryMatch -and
        ...
        elseif ([bool]$inputObject.requiresDistinctOutcome -or -not [bool]$inputObject.outcomeMatch) {
            $decision = 'MUST_NEW'
            $standingCreate = [bool]$inputObject.standingCreateAuthorized
            $reason = $(if ($standingCreate) { 'STANDING_CREATE_AUTHORIZED' } else { 'CREATE_AUTHORIZATION_REQUIRED' })
        }
        else {
            $decision = 'REUSE'
            $standingCreate = $false
    ...
    'TERMINAL' {
        Assert-ExactFields $inputObject $inputMemberNames @('operation','terminalStatus','reportChannelAvailable')
        ...
        $result = [ordered]@{
            operation = $operation
            status = $(if ($available) { [string]$inputObject.terminalStatus } else { 'REPORT_CHANNEL_UNAVAILABLE' })
            delivery = $(if ($available) { 'PROACTIVE' } else { 'UNAVAILABLE' })
            ackRequired = $false
            polling = $false
        }
    }
