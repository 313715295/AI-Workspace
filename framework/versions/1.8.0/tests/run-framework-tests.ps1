...
function Render-MaintenanceStarter([string]$Starter,[string]$ControlRoot) {
    ...
    Get-ChildItem -LiteralPath $destination -Recurse -Force -File | ForEach-Object {
        ...
        $text = $text.Replace('{{FRAMEWORK_VERSION}}','1.8.0')
...
function New-AuthorizationPackage(
    ...
    $package = [ordered]@{
        schemaVersion=$Schema; frameworkVersion='1.8.0'; taskId='FIXTURE-001'; profile='STANDARD'; lifecycle='ACTIVE'
...
Assert-True ($actual.Count -eq 51 -and ($actual -join "`n") -ceq ($expected -join "`n")) 'inventory-exact51'
...
Assert-True ([string]$version.version -ceq '1.8.0' -and [string]$version.lifecycle -ceq 'STABLE' -and [string]$version.releaseClass -ceq 'MINOR' -and [bool]$version.consumable -and [string]$version.baseline -ceq '1.7.0' -and $null -eq $version.PSObject.Properties['currentEligible'] -and [bool]$version.projectPinEligible) 'version-stable-minor-baseline-1.7.0-no-global-selector-field'
...
Assert-True ($selectorHits.Count -eq 0) 'version-selector-terminology-absent-from-live-root-and-1.8-runtime'
...
try {
    $null = $normalProjectTemplate.Replace('{{PROJECT_ID_JSON}}','"fixture"').Replace('{{DISPLAY_NAME_JSON}}','"Fixture"').Replace('{{FRAMEWORK_VERSION_JSON}}','"1.8.0"') | ConvertFrom-Json
...
$temp = Join-Path ([IO.Path]::GetTempPath()) ('aiw-framework-180-' + [guid]::NewGuid().ToString('N'))
...
try {
    ...
    $routeBase = [ordered]@{operation='ROUTE';projectMatch=$true;cwdGitTopMatch=$true;outcomeMatch=$true;ownerRoleMatch=$true;lineageMatch=$true;resourceRouteMatch=$true;protectionBoundaryMatch=$true;gitDeviceExternalMatch=$true;publicDecisionMatch=$true;requiresDistinctOutcome=$false;standingCreateAuthorized=$false}
    ...
    $terminalDelivered = Invoke-WorkflowCase $workflowResolver $temp 'terminal-delivered' ([ordered]@{operation='TERMINAL';terminalStatus='COMPLETE';reportChannelAvailable=$true})
    $terminalUnavailable = Invoke-WorkflowCase $workflowResolver $temp 'terminal-unavailable' ([ordered]@{operation='TERMINAL';terminalStatus='BLOCKED';reportChannelAvailable=$false})
    ...
    Copy-Item -LiteralPath $candidateRoot -Destination (Join-Path $target 'framework\versions\1.8.0') -Recurse
    ...
    $installedFramework = Join-Path $target 'framework\versions\1.8.0'
    ...
    Assert-True ($installedActual.Count -eq 51 -and ($installedActual -join "`n") -ceq ($actual -join "`n")) 'maintenance-target-full-candidate-installed'
    ...
    Assert-True ($missingPin.Code -ne 0 -and $missingPin.Text.Contains('TARGET_REQUIRED_FILE_MISSING|framework/versions/1.8.0/RECOVERY_CORE.md')) 'maintenance-resolver-pin-entry-missing-rejected'
...
'@
    ...
    $repoLocalConfig = [ordered]@{schemaVersion=3;id='repo-local-fixture';displayName='Repo Local Fixture';controlPlaneLayout='repo-local';repositoryRoot='..';frameworkVersion='1.8.0';routineExcludedPaths=@('private/secret.txt');frameworkCapabilities=[pscustomobject]@{KNOWLEDGE_REFERENCE=[pscustomobject]@{enabled=$false}}}
    ...
    if (-not $SkipRootMigration) {
        ...
        Copy-Item -LiteralPath $candidateRoot -Destination (Join-Path $rootFlow 'framework\versions\1.8.0') -Recurse
        ...
        $pendingRegister=Invoke-Ps $register @($baseRegisterArgs + @('-FrameworkVersion','1.8.0'))
        Assert-True ($pendingRegister.Code -ne 0 -and $pendingRegister.Text.Contains('FRAMEWORK_RELEASE_NOT_SEALED|1.8.0')) 'register-pending-release-rejected-before-project-write'
        Assert-True (-not (Test-Path -LiteralPath (Join-Path $consumer '.ai-workspace'))) 'register-pending-release-zero-project-write'

        $fixtureManifestPath=Join-Path $rootFlow 'framework\versions\1.8.0\RELEASE_MANIFEST.json'
        ...
        $shadowUpgradeWithoutRepository=Invoke-Ps $upgrade @('-ProjectId','shadow-fixture','-ToVersion','1.8.0','-ControllerId','controller-shadow')
        ...
        $previewRegister=Invoke-Ps $register @($baseRegisterArgs + @('-FrameworkVersion','1.8.0'))
        if ($previewRegister.Code -ne 0 -or -not $previewRegister.Text.Contains('WHAT_IF')) { Write-Output ('DIAG|register-explicit-1.8.0-preview|' + $previewRegister.Code + '|' + $previewRegister.Text) }
        Assert-True ($previewRegister.Code -eq 0 -and $previewRegister.Text.Contains('WHAT_IF')) 'register-explicit-1.8.0-preview'
        $applyRegister=Invoke-Ps $register @($baseRegisterArgs + @('-FrameworkVersion','1.8.0','-Apply'))
        Assert-True ($applyRegister.Code -eq 0 -and $applyRegister.Text.Contains('CREATED')) 'register-explicit-1.8.0-apply'
        $repeatRegister=Invoke-Ps $register @($baseRegisterArgs + @('-FrameworkVersion','1.8.0'))
        Assert-True ($repeatRegister.Code -eq 0 -and $repeatRegister.Text.Contains('ALREADY_REGISTERED')) 'register-explicit-1.8.0-repeat'
        ...
        Assert-True ([string]$registeredConfig.frameworkVersion -ceq '1.8.0' -and $registeredFiles.Count -eq 9 -and $registeredBootstrap.Contains('scripts/resolve-workflow-route.ps1')) 'register-materializes-selected-starter-exact9'
        ...
        $upgradeArgs=@('-ProjectId','upgrade-fixture','-ToVersion','1.8.0','-RepositoryPath',$upgradeRepo,'-ControllerId','controller-upgrade')
        $upgradePreview=Invoke-Ps $upgrade $upgradeArgs
        Assert-True ($upgradePreview.Code -eq 0 -and $upgradePreview.Text.Contains('Preview only')) 'upgrade-schema3-to-1.8.0-preview'
        $upgradeApply=Invoke-Ps $upgrade @($upgradeArgs+'-Apply')
        Assert-True ($upgradeApply.Code -eq 0 -and $upgradeApply.Text.Contains('Updated:')) 'upgrade-schema3-to-1.8.0-apply'
    ...
    Write-Utf8 (Join-Path $knowledgeRoot 'knowledge\reference.md') 'reference'
    Write-Utf8 (Join-Path $knowledgeRoot 'authority.md') 'authority'
    $knowledgeConfig=[ordered]@{schemaVersion=3;id='knowledge-fixture';displayName='Knowledge Fixture';controlPlaneLayout='repo-local';repositoryRoot='..';frameworkVersion='1.8.0';routineExcludedPaths=@();frameworkCapabilities=[ordered]@{KNOWLEDGE_REFERENCE=[ordered]@{enabled=$true;indexLocator='knowledge/index.json'}}}
    ...
    $validIndex=[ordered]@{schemaVersion=1;projectId='knowledge-fixture';entries=@([ordered]@{id='REF-1';state='CURRENT';title='Reference';summary='Reference only';locator='knowledge/reference.md';identity=Get-Identity (Join-Path $knowledgeRoot 'knowledge\reference.md');authorityLocator='authority.md';authorityIdentity=Get-Identity (Join-Path $knowledgeRoot 'authority.md');verifiedAt='2026-08-17T00:00:00Z';invalidatesOn=@('LOCATOR_IDENTITY_CHANGE','AUTHORITY_IDENTITY_CHANGE');tokenEstimate=10})}
    $knowledgeIndexPath=Join-Path $knowledgeRoot 'knowledge\index.json';Write-Utf8 $knowledgeIndexPath ($validIndex|ConvertTo-Json -Depth 20)
    ...
    $knowledgeArgs=@('-ProjectRoot',$knowledgeRoot,'-ExpectedProjectConfigIdentity',$knowledgeConfigIdentity,'-ExpectedIndexIdentity',(Get-Identity $knowledgeIndexPath),'-AsJson')
    $ps51Valid=Invoke-PsHost 'powershell.exe' $knowledgeChecker $knowledgeArgs
    Assert-True ($ps51Valid.Code -eq 0 -and $ps51Valid.Text.Contains('REFERENCE_AVAILABLE')) 'knowledge-ps5.1-valid-utc-string'
    $pwsh=Get-Command pwsh.exe -ErrorAction SilentlyContinue
    if($null-ne$pwsh){$ps7Valid=Invoke-PsHost $pwsh.Source $knowledgeChecker $knowledgeArgs;Assert-True ($ps7Valid.Code -eq 0 -and $ps7Valid.Text.Contains('REFERENCE_AVAILABLE')) 'knowledge-ps7-valid-utc-string'}else{Write-Output 'EVIDENCE_CEILING|POWERSHELL_7_UNAVAILABLE';Assert-True $true 'knowledge-powershell7-evidence-ceiling-recorded'}
    $typeIndex=$validIndex|ConvertTo-Json -Depth 20|ConvertFrom-Json;$typeIndex.entries[0].verifiedAt=123;Write-Utf8 $knowledgeIndexPath ($typeIndex|ConvertTo-Json -Depth 20)
    $typeArgs=@('-ProjectRoot',$knowledgeRoot,'-ExpectedProjectConfigIdentity',$knowledgeConfigIdentity,'-ExpectedIndexIdentity',(Get-Identity $knowledgeIndexPath),'-AsJson')
    ...
    $invalidIndex=$validIndex|ConvertTo-Json -Depth 20|ConvertFrom-Json;$invalidIndex.entries[0].verifiedAt='2026-08-17 00:00:00';Write-Utf8 $knowledgeIndexPath ($invalidIndex|ConvertTo-Json -Depth 20)
    $invalidArgs=@('-ProjectRoot',$knowledgeRoot,'-ExpectedProjectConfigIdentity',$knowledgeConfigIdentity,'-ExpectedIndexIdentity',(Get-Identity $knowledgeIndexPath),'-AsJson')
    ...
    if (-not $SkipBaseline) {
        ...
        $baselineRunner = Join-Path $frameworkRoot 'versions\1.7.0\tests\run-framework-tests.ps1'
        $baseline = Invoke-Ps $baselineRunner @('-SkipRootMigration')
        if ($baseline.Code -ne 0) { Write-Output ('DIAG|baseline-1.7.0|' + $baseline.Text) }
        Assert-True ($baseline.Code -eq 0 -and $baseline.Text.Contains('RESULT|')) 'baseline-1.7.0-regression-suite'
    }
...
Write-Output ("RESULT|" + $script:passed + "/" + $script:passed + " passed|scope=Framework-1.8.0-stable|lifecycle=STABLE")
