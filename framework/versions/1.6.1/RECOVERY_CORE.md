...
Framework 1.6.1项目在读取任务前还必须严格读取`.ai-workspace/controller.json`与`project.json`的`routineExcludedPaths/frameworkCapabilities`。`controller.json`是current PROJECT_CONTROLLER ID与epoch的唯一机器真相；聊天、热摘要和旧授权都不能替代它。日常排除不授source权限，能力字段默认空且不产生能力；当前唯一可选能力是显式配置的`KNOWLEDGE_REFERENCE`，它只增加非权威参考模块。
