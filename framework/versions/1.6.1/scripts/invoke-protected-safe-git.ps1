...
function Write-Unverified([string]$Reason,[bool]$Launched=$false) {
    [pscustomobject][ordered]@{ status='UNVERIFIED'; operation=$Operation; reason=$Reason; launched=$Launched } | ConvertTo-Json -Compress
    exit 3
}
...
try {
    ...
    if (-not $gitTop.Equals($root,[StringComparison]::OrdinalIgnoreCase)) { throw 'PROJECT_ROOT_NOT_GIT_TOP' }
    ...
    if (-not ($config -is [pscustomobject]) -or $names.Count -ne $expectedFields.Count -or @($expectedFields | Where-Object { $_ -cnotin $names }).Count -ne 0) { throw 'PROJECT_CONFIG_FIELDS' }
    ...
    if (-not (Test-JsonInteger $config.schemaVersion) -or [int]$config.schemaVersion -ne 3 -or
        ...
        -not ($config.frameworkVersion -is [string]) -or [string]$config.frameworkVersion -cne '1.6.1' -or
    ...
    if (-not $IncludeRoutineExcluded) {
        $pathspecs += @($excluded | Sort-Object | ForEach-Object { ':(top,exclude,literal)' + $_ })
    }
    ...
    if (-not $IncludeRoutineExcluded) {
        foreach ($line in $output) { foreach ($path in $excluded) { if ([string]$line -match [regex]::Escape($path)) { Write-Unverified 'ROUTINE_EXCLUSION_OUTPUT_DETECTED' $true } } }
    }
    [pscustomobject][ordered]@{ status='VERIFIED'; operation=$Operation; launched=$true; paths=@($allow | Sort-Object); output=@($output) } | ConvertTo-Json -Depth 5 -Compress
    exit 0
} catch {
