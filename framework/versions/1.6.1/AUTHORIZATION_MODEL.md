...
1.6.1的PROJECT_CONTROLLER包除通用字段外，必须且只额外携带`issuerControllerId`、整数`issuerControllerEpoch`和`controllerControlIdentity`。预检直接读取当前`.ai-workspace/controller.json`，复证whole-file identity、`CURRENT`状态、ID与epoch；任一不一致均拒绝为stale controller authorization。`DOMAIN_OWNER`包不得携带这三个字段。轮换不需要扫描或迁移旧授权包：旧包因Framework版本或controller identity/epoch不匹配自然失效。
