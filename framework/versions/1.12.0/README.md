...
Status: `CANDIDATE / NOT_CONSUMABLE / NOT_PIN_ELIGIBLE` until candidate freeze, three-platform evidence, independent CRITICAL Review and deterministic release seal complete.
...
- registration and upgrade write the project-level selection and fail before project mutation when `pwsh` is unavailable;
- the same normalized conformance entrypoint is intended for Windows, Ubuntu/Linux and macOS evidence.
