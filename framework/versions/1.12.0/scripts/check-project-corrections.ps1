...
function Get-Root([string]$Path,[string]$Label) {
    ...
    $full=[IO.Path]::TrimEndingDirectorySeparator([IO.Path]::GetFullPath((Resolve-Path -LiteralPath $Path).ProviderPath))
...
function Resolve-ChildFile([string]$Root,[string[]]$Components,[string]$Label,[switch]$AllowMissing) {
    ...
    foreach($component in $Components){
        ...
        if(-not(Test-Path -LiteralPath $current)){
            if($AllowMissing-and$current-ceq(Join-Path $Root ([IO.Path]::Combine([string[]]$Components)))){return $null}
...
try {
    ...
    $hasToolBackend=$null-ne$config.PSObject.Properties['frameworkToolBackend']
    $legacyConfig=[string]$config.frameworkVersion-cne'1.12.0'-and-not$hasToolBackend
    ...
    if(-not$legacyConfig){$configFields=@($configFields[0..5]+@('frameworkToolBackend')+$configFields[6..($configFields.Count-1)])}
    Assert-ExactFields $config $configFields 'PROJECT_CONFIG_FIELDS'
    Assert-Text $config.id 'PROJECT_ID'
    if(-not$legacyConfig-and(-not($config.frameworkToolBackend-is[string])-or[string]$config.frameworkToolBackend-cne'powershell7')){throw 'PROJECT_TOOL_BACKEND'}
    ...
    try{
        $coveragePath=Resolve-ChildFile $framework @('framework','versions','1.12.0','CORRECTION_COVERAGE.json') 'COVERAGE'
        ...
        if(-not(Test-JsonInteger $coverage.schemaVersion)-or[int]$coverage.schemaVersion-ne1-or[string]$coverage.releaseVersion-cne'1.12.0'-or-not($coverage.versions-is[System.Array])){throw 'COVERAGE_VALUES'}
