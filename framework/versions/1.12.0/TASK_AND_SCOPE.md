...
Every new schema 1.12.0 card declares exactly one `Work route: role=<ROLE>; phase=<PHASE>`. The task card owns this route; the task index is a compact locator/projection and never overrides it. Profile remains in the existing range summary, while host, topology, capabilities and pin remain in their existing host/project authorities. Actions remain authorization inputs and do not select workflow modules.

At initial recovery or a context discontinuity, invoke the existing loader against the task card with recovery included. When role, Work phase, profile, capability or task changes, update the authoritative card under its normal control package and rerun the loader. Compaction, pause/resume, handoff or uncertain context also reloads `RECOVER + current Work phase`. If the task, role, phase, profile, pin, host/topology/capabilities and effective corrections remain unchanged and context is continuous, keep working without reloading. A fresh authorization package alone is not a reload or FULL_COLD trigger.

Legacy cards are not bulk rewritten for adoption. Until a natural authorized task update adds the route, the caller supplies explicit role/profile/phase together with `TaskPath`; the loader reports `LEGACY_LOAD_CONTEXT`, and that inference is part of the evidence ceiling.
