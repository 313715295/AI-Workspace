...
3. On initial recovery or context discontinuity, resolve operation `LOAD_PLAN_RESOLVE` and invoke its sealed entrypoint with `-TaskPath <assigned-task> -IncludeRecovery`, `Topology=FRAMEWORK_MAINTENANCE_SIBLING`, the host and no capabilities; read every returned module completely. A legacy card without `Work route` additionally requires explicit role/profile/phase and returns `LEGACY_LOAD_CONTEXT`.
