...
Assert-True ([string]$versionMetadata.version -ceq '1.5.1' -and [string]$versionMetadata.lifecycle -ceq 'STABLE' -and [bool]$versionMetadata.consumable -and -not [bool]$versionMetadata.currentEligible -and [bool]$versionMetadata.projectPinEligible) 'version-metadata-stable-consumable'
...
$starterProjectJson = $starterProjectTemplate.Replace('{{PROJECT_ID_JSON}}','"sample-id"').Replace('{{DISPLAY_NAME_JSON}}','"Sample"').Replace('{{FRAMEWORK_VERSION_JSON}}','"1.5.1"')
...
Assert-True ($executorPlan.frameworkVersion -ceq '1.5.1' -and $executorPlan.lifecycle -ceq 'STABLE') 'load-plan-stable-version'
...
$tempRoot = Join-Path ([IO.Path]::GetTempPath()) ('framework-1.5.1-draft-' + [Guid]::NewGuid().ToString('N'))
...
$integrationRoot = Join-Path ([IO.Path]::GetTempPath()) ('framework-1.5.1-integration-' + [Guid]::NewGuid().ToString('N'))
...
try {
    ...
    $register151 = & powershell.exe -NoProfile -ExecutionPolicy Bypass -File $registerScript -ProjectId register-151 -DisplayName 'Register 151' -RepositoryPath $newRepo -FrameworkVersion 1.5.1 -WorkspaceRoot $isolatedWorkspace -Apply 2>&1
    ...
    Assert-True ($register151Code -eq 0 -and $null -ne $register151Config -and [string]$register151Config.frameworkVersion -ceq '1.5.1') 'register-explicit-1.5.1'
    ...
    $upgradePreview = & powershell.exe -NoProfile -ExecutionPolicy Bypass -File $upgradeScript -ProjectId upgrade-141-to-151 -RepositoryPath $upgradeRepo -ToVersion 1.5.1 -WorkspaceRoot $isolatedWorkspace 2>&1
    ...
    $upgradeApply = & powershell.exe -NoProfile -ExecutionPolicy Bypass -File $upgradeScript -ProjectId upgrade-141-to-151 -RepositoryPath $upgradeRepo -ToVersion 1.5.1 -WorkspaceRoot $isolatedWorkspace -Apply 2>&1
    ...
    Assert-True ($upgradeApplyCode -eq 0 -and [string]$upgradeConfig.frameworkVersion -ceq '1.5.1' -and
        $upgradedBootstrap.Contains('framework/versions/1.5.1/RECOVERY_CORE.md') -and
        $upgradedBootstrap.Contains('CUSTOM_PRESERVE_151') -and
        $upgradeHeadAfter -ceq $upgradeHeadBefore -and $upgradeCached.Count -eq 0) 'upgrade-1.4.1-to-1.5.1-preserves-custom-and-git'
    ...
    $patchUpgrade = & powershell.exe -NoProfile -ExecutionPolicy Bypass -File $upgradeScript -ProjectId upgrade-150-to-151 -RepositoryPath $patchUpgradeRepo -ToVersion 1.5.1 -WorkspaceRoot $isolatedWorkspace -Apply 2>&1
    ...
    Assert-True ($patchUpgradeCode -eq 0 -and [string]$patchConfig.frameworkVersion -ceq '1.5.1' -and
        $patchBootstrap.Contains('framework/versions/1.5.1/RECOVERY_CORE.md') -and
        $patchHeadAfter -ceq $patchHeadBefore -and $patchCached.Count -eq 0) 'upgrade-1.5.0-to-1.5.1-preserves-git'
    ...
    $conflictUpgrade = & powershell.exe -NoProfile -ExecutionPolicy Bypass -File $upgradeScript -ProjectId upgrade-conflict -RepositoryPath $conflictRepo -ToVersion 1.5.1 -WorkspaceRoot $isolatedWorkspace -Apply 2>&1
