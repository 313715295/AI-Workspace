...
function Seal-ReleaseFixture([string]$VersionRoot,[string]$Integration) {
    ...
    $manifest.fileCount=$facts.FileCount;$manifest.totalBytes=$facts.TotalBytes;$manifest.canonical=$facts.Canonical;$manifest.sourceReview='APPROVED';$manifest.releaseIntegration=$Integration
...
function Invoke-Ps([string]$Script,[string[]]$Arguments,[string]$WorkingDirectory='') {
    ...
    try {
        ...
        try { $output = @(& powershell.exe -NoProfile -NonInteractive -ExecutionPolicy Bypass -File $Script @Arguments 2>&1 | ForEach-Object { [string]$_ }); $code = $LASTEXITCODE }
...
function Invoke-PsHost([string]$Executable,[string]$Script,[string[]]$Arguments,[string]$WorkingDirectory='') {
    ...
    try {
        $old = $ErrorActionPreference
        $ErrorActionPreference = 'Continue'
        try { $output = @(& $Executable -NoProfile -NonInteractive -ExecutionPolicy Bypass -File $Script @Arguments 2>&1 | ForEach-Object { [string]$_ }); $code = $LASTEXITCODE }
        finally { $ErrorActionPreference = $old }
        return [pscustomobject]@{ Code=$code; Output=$output; Text=($output -join "`n") }
    } finally {
        if ($WorkingDirectory) { Pop-Location }
...
function Render-MaintenanceStarter([string]$Starter,[string]$ControlRoot) {
    ...
    Get-ChildItem -LiteralPath $destination -Recurse -Force -File | ForEach-Object {
        ...
        $text = $text.Replace('{{FRAMEWORK_VERSION}}','1.10.0')
...
function New-AuthorizationPackage(
    ...
    $package = [ordered]@{
        schemaVersion=$Schema; frameworkVersion='1.10.0'; taskId='FIXTURE-001'; profile='STANDARD'; lifecycle='ACTIVE'
...
Assert-True ($actual.Count -eq 57 -and ($actual -join "`n") -ceq ($expected -join "`n")) 'inventory-exact57'
...
Assert-True ([string]$version.version -ceq '1.10.0' -and [string]$version.lifecycle -ceq 'STABLE' -and [string]$version.releaseClass -ceq 'MINOR' -and [bool]$version.consumable -and [string]$version.baseline -ceq '1.9.0' -and $null -eq $version.PSObject.Properties['currentEligible'] -and [bool]$version.projectPinEligible) 'version-stable-minor-baseline-1.9.0-no-global-selector-field'
...
Assert-True ($loadResolverText.Contains('LOAD_MANIFEST_NOT_STABLE_1_10_0') -and -not $loadResolverText.Contains('LOAD_MANIFEST_NOT_STABLE_1_7_0')) 'load-resolver-diagnostic-version-current'

$liveFrameworkRoot = [IO.Path]::GetFullPath((Join-Path $candidateRoot '..\..')).TrimEnd('\')
...
Assert-True ($selectorHits.Count -eq 0) 'version-selector-terminology-absent-from-live-root-and-1.9-runtime'
...
try {
    $null = $normalProjectTemplate.Replace('{{PROJECT_ID_JSON}}','"fixture"').Replace('{{DISPLAY_NAME_JSON}}','"Fixture"').Replace('{{FRAMEWORK_VERSION_JSON}}','"1.10.0"') | ConvertFrom-Json
...
foreach ($relative in $workflowEntryDocuments) {
    ...
    Assert-True ($entryText.Contains('scripts/resolve-workflow-route.ps1') -and $entryText.Contains('ephemeral') -and $entryText.Contains('fails closed')) ('workflow-live-entry-fail-closed|' + $relative)
}

$temp = Join-Path ([IO.Path]::GetTempPath()) ('aiw-framework-190-' + [guid]::NewGuid().ToString('N'))
...
'@
    ...
    Write-Utf8 $standardTaskPath "# TASK-STANDARD-001 - fixture`n`n- Task schema: 1.10.0`n- Owner: owner-fixture`n- Range summary: profile=STANDARD; lifecycle=ACTIVE; expected_paths=[]; actual_paths=[]`n"
    $standardTaskValid = Invoke-Ps $taskChecker @('-TaskPath',$standardTaskPath)
    Assert-True ($standardTaskValid.Code -eq 0) 'task-card-standard-no-machinery-proportionality-exempt'
    ...
    Copy-Item -LiteralPath $candidateRoot -Destination (Join-Path $target 'framework\versions\1.10.0') -Recurse
    ...
    $installedFramework = Join-Path $target 'framework\versions\1.10.0'
    ...
    Assert-True ($installedActual.Count -eq 57 -and ($installedActual -join "`n") -ceq ($actual -join "`n")) 'maintenance-target-full-candidate-installed'
    ...
    Assert-True ($missingPin.Code -ne 0 -and $missingPin.Text.Contains('TARGET_REQUIRED_FILE_MISSING|framework/versions/1.10.0/RECOVERY_CORE.md')) 'maintenance-resolver-pin-entry-missing-rejected'
...
'@
    ...
    $repoLocalConfig = [ordered]@{schemaVersion=3;id='repo-local-fixture';displayName='Repo Local Fixture';controlPlaneLayout='repo-local';repositoryRoot='..';frameworkVersion='1.10.0';routineExcludedPaths=@('private/secret.txt');frameworkCapabilities=[pscustomobject]@{KNOWLEDGE_REFERENCE=[pscustomobject]@{enabled=$false}}}
    ...
    $criticalReviewPackage = [ordered]@{
        schemaVersion=1;frameworkVersion='1.10.0';taskId='FIXTURE-REVIEW-001';profile='CRITICAL';lifecycle='ACTIVE'
    ...
    Copy-Item -LiteralPath $candidateRoot -Destination (Join-Path $correctionFrameworkRoot 'framework\versions\1.10.0') -Recurse
    $null=Seal-ReleaseFixture (Join-Path $correctionFrameworkRoot 'framework\versions\1.10.0') 'CORRECTION_TEST_FIXTURE'
    ...
    $sealedCoveragePath=Join-Path $correctionFrameworkRoot 'framework\versions\1.10.0\CORRECTION_COVERAGE.json';$sealedCoverageOriginal=Get-Content -Raw -Encoding utf8 -LiteralPath $sealedCoveragePath
    ...
    $nonCumulative=$sealedCoverageOriginal|ConvertFrom-Json;$entry110=@($nonCumulative.versions|Where-Object{[string]$_.version-ceq'1.10.0'})[0];$entry110.incorporatedCorrectionIds=@($entry110.incorporatedCorrectionIds|Where-Object{[string]$_-cne'COMPACT_NON_INTERRUPT_DELIVERY'});Write-Utf8 $sealedCoveragePath ($nonCumulative|ConvertTo-Json -Depth 20);$null=Seal-ReleaseFixture (Split-Path -Parent $sealedCoveragePath) 'NONCUMULATIVE_TEST_FIXTURE'
    $nonCumulativeRun=Invoke-Ps $correctionChecker @($correctionArgs[0..4]+@('1.10.0')+$correctionArgs[6..($correctionArgs.Count-1)]);$nonCumulativeValue=$nonCumulativeRun.Output[-1]|ConvertFrom-Json
    ...
    $overlapCoverage=$sealedCoverageOriginal|ConvertFrom-Json;$entry110=@($overlapCoverage.versions|Where-Object{[string]$_.version-ceq'1.10.0'})[0];$entry110.conflictingCorrectionIds=@('OWNER_FIRST_DIRECT_DOMAIN_ROUTE');Write-Utf8 $sealedCoveragePath ($overlapCoverage|ConvertTo-Json -Depth 20);$null=Seal-ReleaseFixture (Split-Path -Parent $sealedCoveragePath) 'OVERLAP_TEST_FIXTURE'
    $overlapRun=Invoke-Ps $correctionChecker @($correctionArgs[0..4]+@('1.10.0')+$correctionArgs[6..($correctionArgs.Count-1)]);$overlapValue=$overlapRun.Output[-1]|ConvertFrom-Json
    ...
    if (-not $SkipRootMigration) {
        ...
        Copy-Item -LiteralPath $candidateRoot -Destination (Join-Path $rootFlow 'framework\versions\1.10.0') -Recurse
        Assert-True (-not (Test-Path -LiteralPath (Join-Path $rootFlow 'framework\CURRENT'))) 'root-global-version-selector-absent'

        $fixtureManifestPath=Join-Path $rootFlow 'framework\versions\1.10.0\RELEASE_MANIFEST.json'
        ...
        $pendingRegister=Invoke-Ps $register @($baseRegisterArgs + @('-FrameworkVersion','1.10.0'))
        Assert-True ($pendingRegister.Code -ne 0 -and $pendingRegister.Text.Contains('FRAMEWORK_RELEASE_NOT_SEALED|1.10.0')) 'register-pending-release-rejected-before-project-write'
        ...
        $shadowUpgradeWithoutRepository=Invoke-Ps $upgrade @('-ProjectId','shadow-fixture','-ToVersion','1.10.0','-ControllerId','controller-shadow')
        ...
        $previewRegister=Invoke-Ps $register @($baseRegisterArgs + @('-FrameworkVersion','1.10.0'))
        if ($previewRegister.Code -ne 0 -or -not $previewRegister.Text.Contains('WHAT_IF')) { Write-Output ('DIAG|register-explicit-1.10.0-preview|' + $previewRegister.Code + '|' + $previewRegister.Text) }
        Assert-True ($previewRegister.Code -eq 0 -and $previewRegister.Text.Contains('WHAT_IF')) 'register-explicit-1.10.0-preview'
        $applyRegister=Invoke-Ps $register @($baseRegisterArgs + @('-FrameworkVersion','1.10.0','-Apply'))
        Assert-True ($applyRegister.Code -eq 0 -and $applyRegister.Text.Contains('CREATED')) 'register-explicit-1.10.0-apply'
        $repeatRegister=Invoke-Ps $register @($baseRegisterArgs + @('-FrameworkVersion','1.10.0'))
        Assert-True ($repeatRegister.Code -eq 0 -and $repeatRegister.Text.Contains('ALREADY_REGISTERED')) 'register-explicit-1.10.0-repeat'
        ...
        Assert-True ([string]$registeredConfig.frameworkVersion -ceq '1.10.0' -and $registeredFiles.Count -eq 10 -and $registeredBootstrap.Contains('scripts/resolve-workflow-route.ps1') -and $registeredBootstrap.Contains('PROJECT-CORRECTIONS:BEGIN') -and [string]$registeredCorrections.projectId -ceq 'explicit-fixture' -and @($registeredCorrections.corrections).Count -eq 0) 'register-materializes-selected-starter-exact10-with-corrections'

        $upgradeRepo=Join-Path $rootFlow 'consumer-upgrade'
        New-GitRepo $upgradeRepo
        $sourceRegisterArgs=@('-ProjectId','upgrade-fixture','-DisplayName','Upgrade Fixture','-RepositoryPath',$upgradeRepo,'-ControllerId','controller-upgrade','-FrameworkVersion','1.7.0','-Apply')
        $sourceRegister=Invoke-Ps $register $sourceRegisterArgs
        Assert-True ($sourceRegister.Code -eq 0 -and $sourceRegister.Text.Contains('CREATED')) 'upgrade-fixture-registers-explicit-1.7.0'
        ...
        $upgradeArgs=@('-ProjectId','upgrade-fixture','-ToVersion','1.10.0','-RepositoryPath',$upgradeRepo,'-ControllerId','controller-upgrade')
        ...
        $conflictEntry=@($conflictCoverage.versions|Where-Object{[string]$_.version-ceq'1.10.0'})[0]
        $conflictEntry.incorporatedCorrectionIds=@($conflictEntry.incorporatedCorrectionIds|Where-Object{[string]$_-cne'PROJECT_CORRECTION_LIFECYCLE'})
        $conflictEntry.conflictingCorrectionIds=@('PROJECT_CORRECTION_LIFECYCLE')
        ...
        $conflictBlocked=$conflictUpgrade.Code-ne0-and$conflictUpgrade.Text.Contains('PROJECT_CORRECTION_CONFLICT')-and$conflictVersion-ceq'1.7.0'-and$conflictCorrectionsIdentity-ceq$upgradeCorrectionsBefore
        ...
        Assert-True ($upgradePreview.Code -eq 0 -and $upgradePreview.Text.Contains('Preview only') -and $upgradePreview.Text.Contains('incorporated=2') -and $upgradePreview.Text.Contains('conflicts=0')) 'upgrade-schema3-to-1.10.0-preview-with-correction-report'
        $upgradeApply=Invoke-Ps $upgrade @($upgradeArgs+'-Apply')
        if($upgradeApply.Code-ne0-or-not$upgradeApply.Text.Contains('Updated:')){Write-Output ('DIAG|upgrade-schema3-to-1.10.0-apply|code='+$upgradeApply.Code+'|output='+$upgradeApply.Text)}
        Assert-True ($upgradeApply.Code -eq 0 -and $upgradeApply.Text.Contains('Updated:')) 'upgrade-schema3-to-1.10.0-apply'
        ...
        Assert-True ($downgradeApply.Code-eq0-and[string]$downgradedConfig.frameworkVersion-ceq'1.9.0'-and$downgradedBootstrap.Contains('PROJECT-CORRECTIONS:BEGIN')-and$downgradedBootstrap.Contains('framework/versions/1.10.0/scripts/check-project-corrections.ps1')-and(Get-Identity $upgradeCorrectionsPath)-ceq$upgradeCorrectionsBefore) 'downgrade-preserves-overlay-records-and-correction-bytes'
    }
    ...
    $knowledgeConfig=[ordered]@{schemaVersion=3;id='knowledge-fixture';displayName='Knowledge Fixture';controlPlaneLayout='repo-local';repositoryRoot='..';frameworkVersion='1.10.0';routineExcludedPaths=@();frameworkCapabilities=[ordered]@{KNOWLEDGE_REFERENCE=[ordered]@{enabled=$true;indexLocator='knowledge/index.json'}}}
    ...
    $ps51Discover=Invoke-PsHost 'powershell.exe' $knowledgeChecker @($knowledgeBaseArgs+@('-Operation','DISCOVER','-AsJson'))
    $discoverValue=@($ps51Discover.Output)[-1]|ConvertFrom-Json
    Assert-True ($ps51Discover.Code -eq 0 -and [string]$discoverValue.status -ceq 'KNOWLEDGE_CATALOG' -and @($discoverValue.entries).Count -eq 3 -and $null-eq$discoverValue.entries[0].PSObject.Properties['summary'] -and [int]$discoverValue.maxQueries -eq 3) 'knowledge-discover-compact-metadata-no-summary'
    $ps51Query=Invoke-PsHost 'powershell.exe' $knowledgeChecker @($knowledgeBaseArgs+@('-Operation','QUERY','-EntryId','REF-1','-AsJson'))
    $queryValue=@($ps51Query.Output)[-1]|ConvertFrom-Json
    Assert-True ($ps51Query.Code -eq 0 -and [string]$queryValue.entries[0].status -ceq 'AVAILABLE' -and $null-ne$queryValue.entries[0].PSObject.Properties['authorityDependencies']) 'knowledge-query-requested-current-entry-available'
    $multiQuery=Invoke-PsHostEntryIdArray 'powershell.exe' $knowledgeChecker $knowledgeBaseArgs @('REF-1','REF-2') $temp
    ...
    $isolatedQuery=Invoke-PsHost 'powershell.exe' $knowledgeChecker @($knowledgeBaseArgs+@('-Operation','QUERY','-EntryId','REF-1','-AsJson'))
    ...
    $staleOnly=Invoke-PsHost 'powershell.exe' $knowledgeChecker @($knowledgeBaseArgs+@('-Operation','QUERY','-EntryId','REF-2','-AsJson'))
    Assert-True ($staleOnly.Code -eq 0 -and $staleOnly.Text.Contains('UNAVAILABLE')) 'knowledge-query-requested-stale-entry-refused'
    Write-Utf8 (Join-Path $knowledgeRoot 'authority-2.md') 'authority-2'

    $noId=Invoke-PsHost 'powershell.exe' $knowledgeChecker @($knowledgeBaseArgs+@('-Operation','QUERY','-AsJson'))
    $unknownId=Invoke-PsHost 'powershell.exe' $knowledgeChecker @($knowledgeBaseArgs+@('-Operation','QUERY','-EntryId','UNKNOWN','-AsJson'))
    $duplicateId=Invoke-PsHostEntryIdArray 'powershell.exe' $knowledgeChecker $knowledgeBaseArgs @('REF-1','REF-1') $temp
    $overLimit=Invoke-PsHostEntryIdArray 'powershell.exe' $knowledgeChecker $knowledgeBaseArgs @('REF-1','REF-2','REF-HISTORY','UNKNOWN') $temp
    ...
    $directImpact=Invoke-PsHost 'powershell.exe' $knowledgeImpact @($impactBaseArgs+@('-ChangedAuthorityPath','authority-1.md'))
    ...
    $noneImpact=Invoke-PsHost 'powershell.exe' $knowledgeImpact @($impactBaseArgs+@('-ChangedAuthorityPath','unrelated.md'))
    ...
    $unknownImpact=Invoke-PsHost 'powershell.exe' $knowledgeImpact @($impactBaseArgs+@('-ChangedAuthorityPath','unrelated.md'))
    ...
    $malformedImpact=Invoke-PsHost 'powershell.exe' $knowledgeImpact $malformedImpactArgs
    Assert-True ($malformedImpact.Code -eq 3 -and $malformedImpact.Text.Contains('ENTRY_VALUES')) 'knowledge-impact-ps5.1-malformed-dependency-index-fails-closed'
    ...
    $duplicateImpact=Invoke-PsHost 'powershell.exe' $knowledgeImpact $duplicateImpactArgs
    Assert-True ($duplicateImpact.Code -eq 3 -and $duplicateImpact.Text.Contains('ENTRY_ID_DUPLICATE')) 'knowledge-impact-ps5.1-duplicate-id-index-fails-closed'
    ...
    $unknownFieldImpact=Invoke-PsHost 'powershell.exe' $knowledgeImpact $unknownFieldImpactArgs
    ...
    $duplicateFieldImpact=Invoke-PsHost 'powershell.exe' $knowledgeImpact $duplicateFieldImpactArgs
    ...
    $reparseImpact=Invoke-PsHost 'powershell.exe' $knowledgeImpact $reparseImpactArgs
    Assert-True ($reparseImpact.Code -eq 3 -and $reparseImpact.Text.Contains('LOCATOR_REPARSE|linked-authority/outside.md')) 'knowledge-impact-ps5.1-dependency-junction-fails-closed'
    ...
    $schema1Query=Invoke-PsHost 'powershell.exe' $knowledgeChecker $schema1Args
    ...
    $ps51Type=Invoke-PsHost 'powershell.exe' $knowledgeChecker $typeArgs
    Assert-True ($ps51Type.Code -eq 3 -and $ps51Type.Text.Contains('ENTRY_VALUES')) 'knowledge-ps5.1-rejects-timestamp-type-drift'
    ...
    $ps51Invalid=Invoke-PsHost 'powershell.exe' $knowledgeChecker $invalidArgs
    Assert-True ($ps51Invalid.Code -eq 3 -and $ps51Invalid.Text.Contains('ENTRY_VALUES')) 'knowledge-ps5.1-rejects-invalid-timestamp'
    ...
    if (-not $SkipBaseline) {
        ...
        $baselineRunner = Join-Path $frameworkRoot 'versions\1.9.0\tests\run-framework-tests.ps1'
        $baseline = Invoke-Ps $baselineRunner @('-SkipRootMigration')
        if ($baseline.Code -ne 0) { Write-Output ('DIAG|baseline-1.9.0|' + $baseline.Text) }
        Assert-True ($baseline.Code -eq 0 -and $baseline.Text.Contains('RESULT|')) 'baseline-1.9.0-regression-suite'
    }
...
$resultScope=if($resultSealed){'Framework-1.10.0-stable'}else{'Framework-1.10.0-candidate'}
