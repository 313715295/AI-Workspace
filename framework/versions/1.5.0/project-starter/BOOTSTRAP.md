...
1. 严格UTF-8读取同目录`project.json`；验证schema、repo-local布局、`repositoryRoot=..`、项目ID和pin。
...
1. 读取`.ai-workspace/project.json`和`<FW>/RECOVERY_CORE.md`。
...
6. 复证真实Git顶层、HEAD/index/actual、其他owner、protected visibility/read/hash/diff/index/write和current authorization package。
