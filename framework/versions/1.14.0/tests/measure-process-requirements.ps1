...
try{
  ...
  $frameworkRoot=Join-Path $temp 'framework-workspace';$sealedRoot=Join-Path $frameworkRoot 'framework\versions\1.14.0';New-Item -ItemType Directory -Path (Split-Path -Parent $sealedRoot) -Force|Out-Null;Copy-Item -LiteralPath $versionRoot -Destination $sealedRoot -Recurse
  ...
  foreach($fixture in @($fixtureContract.fixtures)){
    ...
    $project=[ordered]@{schemaVersion=4;id=('measure-'+[string]$fixture.fixtureId);displayName='Measurement Fixture';controlPlaneLayout='repo-local';repositoryRoot='..';frameworkVersion='1.14.0';frameworkToolBackend='powershell7';routineExcludedPaths=@();frameworkCapabilities=$fixtureCapabilities;processPolicy=[ordered]@{schemaVersion=1;locator='.ai-workspace/process-policy.json'}}
    ...
    $taskOwner=if([string]$fixture.role-ceq'REVIEWER'){'fixture-owner'}else{[string]$fixture.actor}
    $taskPath=Join-Path $projectRoot ('.ai-workspace\tasks\active\'+$taskId+'.md')
    Write-Utf8 $taskPath ("# "+$taskId+" - measurement fixture`n`n- Task schema: 1.14.0`n- Owner: "+$taskOwner+"`n- Work route: actor="+[string]$fixture.actor+"; role="+[string]$fixture.role+"; phase="+[string]$fixture.phase+"`n- Range summary: profile="+[string]$fixture.profile+"; lifecycle=ACTIVE; expected_paths=[]; actual_paths=[]`n")
    ...
    if([string]$fixture.actionKind-cne'NONE'){
      ...
      $package=[ordered]@{schemaVersion=1;frameworkVersion='1.14.0';taskId=$taskId;taskIdentity=(Get-Identity $taskPath);profile=[string]$fixture.profile;lifecycle='ACTIVE';owner=$taskOwner;issuer=$issuer;issuerRole=$issuerRole;grantee=[string]$fixture.actor;bundle='MEASUREMENT_LOCAL';decisionClass=$decisionClass;userConfirmation=$userDecision;reviewIndependence=$(if([string]$fixture.actionKind-ceq'REVIEW_EXECUTE'){'INDEPENDENT'}else{'NOT_APPLICABLE'});delegatedGitCloser=$false;actions=@([string]$fixture.actionKind);exactPaths=@($fixture.exactPaths);objectIdentities=$objectIdentities;projectConfigIdentity=(Get-Identity $projectPath);invalidatesOn=@('TASK_CHANGE','OWNER_CHANGE','GRANTEE_CHANGE','ACTION_CHANGE','PATHSET_CHANGE','OBJECT_DRIFT','USER_DECISION_CHANGE','PROJECT_CONFIG_DRIFT')}
    ...
    $initial=Invoke-Cold $resolver $discoverPath ([int]$fixture.expectedExitCodes.DISCOVER);$receipt=$initial.Output|ConvertFrom-Json;$receiptPath=Join-Path $projectRoot '.ai-workspace\measure-receipt.json';Write-Utf8 $receiptPath ($receipt|ConvertTo-Json -Depth 50)
    $requiredPrep=@($receipt.selectedRequirements|ForEach-Object{@($_.preparationRequirements)}|Sort-Object -Unique);$requiredResult=@($receipt.selectedRequirements|ForEach-Object{@($_.resultRequirements)}|Sort-Object -Unique)
    ...
    $results+=[ordered]@{fixtureId=[string]$fixture.fixtureId;fixtureIdentity=[string]$fixture.fixtureIdentity;selectedPackBytes=[int]$receipt.selectedPackBytes;selectedPackEstimatedTokens=[int]$receipt.selectedPackEstimatedTokens;selectedRequirementCount=@($receipt.selectedRequirements).Count;modes=$modeResults}
  }
  $output=[ordered]@{schemaVersion=1;frameworkVersion='1.14.0';host=([Environment]::OSVersion.VersionString+' / PowerShell '+$PSVersionTable.PSVersion+' / '+[Runtime.InteropServices.RuntimeInformation]::OSArchitecture+' / '+[Environment]::ProcessorCount+' logical processors');process='cold pwsh process';filesystem='warmed after fixture setup';warmups=$Warmups;measuredRuns=$MeasuredRuns;fixtures=$results}
