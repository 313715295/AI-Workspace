...
function Render-MaintenanceStarter([string]$Starter,[string]$ControlRoot) {
    ...
    Get-ChildItem -LiteralPath $destination -Recurse -Force -File | ForEach-Object {
        ...
        $text = $text.Replace('{{FRAMEWORK_VERSION}}','1.14.0')
...
function New-AuthorizationPackage(
    ...
    Write-Utf8 $taskFull ("# $taskId - authorization fixture`n`n- Task schema: 1.14.0`n- Owner: $issuer`n- Work route: actor=$issuer; role="+$(if($DomainOwner){'DOMAIN_OWNER'}else{'CONTROLLER'})+"; phase=IMPLEMENT`n- Range summary: profile=STANDARD; lifecycle=ACTIVE; expected_paths=[]; actual_paths=[]`n")
    $taskIdentity=Get-Identity $taskFull
    $package = [ordered]@{
        schemaVersion=$Schema; frameworkVersion='1.14.0'; taskId=$taskId; taskIdentity=$taskIdentity; profile='STANDARD'; lifecycle='ACTIVE'
...
Assert-True ([string]$version.version -ceq '1.14.0' -and $versionLifecycleState -and [string]$version.releaseClass -ceq 'MINOR' -and [string]$version.baseline -ceq '1.13.0' -and $null -eq $version.PSObject.Properties['currentEligible']) 'version-lifecycle-fields-minor-baseline-1.13.0-no-global-selector-field'
Assert-True ([string]$loadManifest.lifecycle -ceq $expectedLifecycle -and @($loadManifest.topologies.PSObject.Properties.Name).Count -eq 2 -and @($loadManifest.topologies.FRAMEWORK_MAINTENANCE_SIBLING) -contains 'FRAMEWORK_MAINTENANCE.md') 'load-manifest-lifecycle-topology-contract'
$loadResolverText=Get-Content -LiteralPath (Join-Path $candidateRoot 'scripts\resolve-load-plan.ps1') -Raw -Encoding utf8
Assert-True ($loadResolverText.Contains('LOAD_MANIFEST_NOT_STABLE_1_14_0') -and -not $loadResolverText.Contains('LOAD_MANIFEST_NOT_STABLE_1_13_0')) 'load-resolver-diagnostic-version-current'
...
Assert-True ([int]$toolchain.schemaVersion-eq1-and[string]$toolchain.frameworkVersion-ceq'1.14.0'-and[string]$toolchain.contractVersion-ceq'1'-and[string]$toolchain.projectSelectionField-ceq'frameworkToolBackend'-and@($toolchain.officialBackends).Count-eq1) 'toolchain-single-project-selected-backend'
...
if($ToolContractOnly){
    ...
    try{
        ...
        $gitConfig=[ordered]@{schemaVersion=3;id='tool-contract-git';displayName='Tool Contract Git';controlPlaneLayout='repo-local';repositoryRoot='..';frameworkVersion='1.14.0';frameworkToolBackend='powershell7';routineExcludedPaths=@();frameworkCapabilities=[ordered]@{KNOWLEDGE_REFERENCE=[ordered]@{enabled=$false}}}
    ...
    Write-Output "RESULT|$($script:passed) passed|scope=Framework-1.14.0-tool-contract"
    return
}
...
$runtimeCandidateRoot=Join-Path $temp 'sealed-runtime\1.14.0'
...
try {
    $null = $normalProjectTemplate.Replace('{{PROJECT_ID_JSON}}','"fixture"').Replace('{{DISPLAY_NAME_JSON}}','"Fixture"').Replace('{{FRAMEWORK_VERSION_JSON}}','"1.14.0"') | ConvertFrom-Json
...
Assert-True ([string]$budgetContract.baseline.version-ceq'1.13.0'-and[string]$budgetContract.baseline.catalogIdentity-ceq'8562|9EA9D79A9346EB3C79AF83A73A4573F7E6AEFBBB8430E058BE0A220E6F8EC966'-and[string]$budgetContract.candidate.catalogIdentity-ceq$catalogBudgetIdentity) 'process-budget-baseline-and-candidate-catalog-identities-distinct-and-exact'
...
Assert-True ($fixtureBudgetsValid-and$fixtureIds.Count-eq5-and@($budgetContract.measurementContract.statistics)-contains'median'-and@($budgetContract.measurementContract.statistics)-contains'nearest-rank-p95'-and[int]$budgetContract.measurementContract.warmupsPerModeAndFixture-eq3-and[int]$budgetContract.measurementContract.measuredRunsPerModeAndFixture-eq30-and[string]$budgetContract.measurementContract.toleranceFormula-ceq'ceiling(max(2.0 * baselineP95Ms, baselineP95Ms + 250))'-and[string]$budgetContract.candidate.fixtureContractIdentity-ceq(Get-Identity $fixtureContractPath)-and[string]$budgetContract.candidate.measurementHarnessIdentity-ceq(Get-Identity $measurementHarnessPath)) 'process-budget-five-exact-fixtures-and-measurement-protocol-bound'
...
Assert-True ($fixtureIdentitiesValid-and@($fixtureContract.fixtures).Count-eq5) 'process-budget-fixture-record-identities-reproducible'
$measurementSmoke=Invoke-Ps $measurementHarnessPath @('-Warmups','1','-MeasuredRuns','1','-AsJson');$measurementSmokeValue=if($measurementSmoke.Code-eq0){$measurementSmoke.Text|ConvertFrom-Json}else{$null};$measurementSmokeMatches=$measurementSmoke.Code-eq0-and@($measurementSmokeValue.fixtures).Count-eq5
...
Assert-True $measurementSmokeMatches 'process-budget-five-fixtures-replay-selected-pack'
...
Assert-True ($evaluationText.Contains('only after a project explicitly adopts stable Framework 1.14.0')-and$evaluationText.Contains('Use normal project tasks')-and$evaluationText.Contains('originating task')-and$evaluationText.Contains('do not require a fixed sample count or time window')-and$evaluationText.Contains('does not block the stable seal')-and-not$evaluationText.Contains('exactly 20 admitted samples')-and-not$evaluationText.Contains('INSUFFICIENT_SAMPLE / NO_ADVANCE')) 'post-release-project-observation-nonblocking-no-synthetic-work'
Assert-True ($releaseGovernanceText.Contains('Maintenance OWNER_ACCEPT of the exact approved candidate')-and$reviewEvidenceText.Contains('Maintenance OWNER_ACCEPT records the task owner''s separate acceptance')-and$releaseGovernanceText.IndexOf('fresh independent CRITICAL source Review')-lt$releaseGovernanceText.LastIndexOf('Maintenance OWNER_ACCEPT')-and$releaseGovernanceText.LastIndexOf('Maintenance OWNER_ACCEPT')-lt$releaseGovernanceText.IndexOf('project only the enumerated lifecycle fields')-and$releaseGovernanceText.Contains('focused independent post-seal Review')-and$reviewEvidenceText.IndexOf('fresh independent CRITICAL Reviewer')-lt$reviewEvidenceText.IndexOf('Maintenance OWNER_ACCEPT')-and$reviewEvidenceText.IndexOf('Maintenance OWNER_ACCEPT')-lt$reviewEvidenceText.IndexOf('bounded sealing writer')) 'release-sequence-independent-review-owner-accept-limited-seal-and-focused-rereview'
...
'@
    ...
    Write-Utf8 $standardTaskPath "# TASK-STANDARD-001 - fixture`n`n- Task schema: 1.14.0`n- Owner: owner-fixture`n- Work route: actor=executor-fixture; role=EXECUTOR; phase=IMPLEMENT`n- Range summary: profile=STANDARD; lifecycle=ACTIVE; expected_paths=[]; actual_paths=[]`n"
    ...
    Write-Utf8 $standardTaskPath "# TASK-STANDARD-001 - fixture`n`n- Task schema: 1.14.0`n- Owner: owner-fixture`n- Work route: actor=reviewer-fixture; role=REVIEWER; phase=REVIEW`n- Range summary: profile=STANDARD; lifecycle=ACTIVE; expected_paths=[]; actual_paths=[]`n"
    ...
    Write-Utf8 $missingRouteTaskPath "# TASK-MISSING-ROUTE-001 - fixture`n`n- Task schema: 1.14.0`n- Owner: owner-fixture`n- Range summary: profile=STANDARD; lifecycle=ACTIVE; expected_paths=[]; actual_paths=[]`n"
    ...
    Copy-Item -LiteralPath $candidateRoot -Destination (Join-Path $target 'framework\versions\1.14.0') -Recurse
    $null=Seal-ReleaseFixture (Join-Path $target 'framework\versions\1.14.0') 'MAINTENANCE_TARGET_TEST_FIXTURE'
    ...
    $installedFramework = Join-Path $target 'framework\versions\1.14.0'
    ...
    Assert-True ($missingPin.Code -ne 0 -and $missingPin.Text.Contains('TARGET_REQUIRED_FILE_MISSING|framework/versions/1.14.0/RECOVERY_CORE.md')) 'maintenance-resolver-pin-entry-missing-rejected'
...
'@
    ...
    $repoLocalConfig = [ordered]@{schemaVersion=3;id='repo-local-fixture';displayName='Repo Local Fixture';controlPlaneLayout='repo-local';repositoryRoot='..';frameworkVersion='1.14.0';frameworkToolBackend='powershell7';routineExcludedPaths=@('private/secret.txt');frameworkCapabilities=[pscustomobject]@{KNOWLEDGE_REFERENCE=[pscustomobject]@{enabled=$false}}}
    ...
    $repoLocal4Raw=$projectStarterText.Replace('{{PROJECT_ID_JSON}}','"repo-local-schema4-fixture"').Replace('{{DISPLAY_NAME_JSON}}','"Repo Local Schema4 Fixture"').Replace('{{FRAMEWORK_VERSION_JSON}}','"1.14.0"')
    ...
    Write-Utf8 $domainExternalTaskPath "# FIXTURE-EXTERNAL-001 - external authorization fixture`n`n- Task schema: 1.14.0`n- Owner: owner-fixture`n- Work route: actor=owner-fixture; role=DOMAIN_OWNER; phase=EXTERNAL`n- Range summary: profile=STANDARD; lifecycle=ACTIVE; expected_paths=[src/public.txt]; actual_paths=[]`n"
    ...
    Copy-Item -LiteralPath $candidateRoot -Destination (Join-Path $correctionFrameworkRoot 'framework\versions\1.14.0') -Recurse
    $null=Seal-ReleaseFixture (Join-Path $correctionFrameworkRoot 'framework\versions\1.14.0') 'CORRECTION_TEST_FIXTURE'
    ...
    $correctionConfig=[ordered]@{schemaVersion=3;id='correction-fixture';displayName='Correction Fixture';controlPlaneLayout='repo-local';repositoryRoot='..';frameworkVersion='1.14.0';frameworkToolBackend='powershell7';routineExcludedPaths=@();frameworkCapabilities=[ordered]@{}}
    ...
    $sealedCoveragePath=Join-Path $correctionFrameworkRoot 'framework\versions\1.14.0\CORRECTION_COVERAGE.json';$sealedCoverageOriginal=Get-Content -Raw -Encoding utf8 -LiteralPath $sealedCoveragePath
    $sealedCatalogPath=Join-Path $correctionFrameworkRoot 'framework\versions\1.14.0\PROCESS_REQUIREMENTS.json'
    ...
    $entry113=@($coverageFixture.versions|Where-Object{[string]$_.version-ceq'1.14.0'})[0]
    ...
    $correctionArgs=@('-ProjectRoot',$correctionRoot,'-FrameworkRoot',$correctionFrameworkRoot,'-TargetVersion','1.14.0','-ExpectedProjectConfigIdentity',(Get-Identity $correctionConfigPath),'-ExpectedCorrectionsIdentity',(Get-Identity $correctionPath),'-Operation','PRECHECK','-AsJson')
    ...
    $driftArgs=@('-ProjectRoot',$correctionRoot,'-FrameworkRoot',$correctionFrameworkRoot,'-TargetVersion','1.14.0','-ExpectedProjectConfigIdentity',(Get-Identity $correctionConfigPath),'-ExpectedCorrectionsIdentity',(Get-Identity $correctionPath),'-Operation','PRECHECK','-AsJson')
    ...
    $invalidMapping=$coverageFixture|ConvertTo-Json -Depth 30|ConvertFrom-Json;$invalidEntry=@($invalidMapping.versions|Where-Object{[string]$_.version-ceq'1.14.0'})[0];$invalidEntry.incorporationMappings[0].nativeCatalogIdentity='0|'+('0'*64);Write-Utf8 $sealedCoveragePath ($invalidMapping|ConvertTo-Json -Depth 30);$null=Seal-ReleaseFixture (Split-Path -Parent $sealedCoveragePath) 'INVALID_MAPPING_FIXTURE'
    ...
    $correctionDuplicateRun=Invoke-Ps $correctionChecker @('-ProjectRoot',$correctionRoot,'-FrameworkRoot',$correctionFrameworkRoot,'-TargetVersion','1.14.0','-ExpectedProjectConfigIdentity',(Get-Identity $correctionConfigPath),'-ExpectedCorrectionsIdentity',(Get-Identity $correctionPath),'-Operation','PRECHECK','-AsJson')
    ...
    $missingCorrectionRun=Invoke-Ps $correctionChecker @('-ProjectRoot',$missingCorrectionRoot,'-FrameworkRoot',$correctionFrameworkRoot,'-TargetVersion','1.14.0','-ExpectedProjectConfigIdentity',(Get-Identity $missingCorrectionConfigPath),'-Operation','PRECHECK','-AllowMissingCorrections','-AsJson')
    ...
    $processResolver=Join-Path $correctionFrameworkRoot 'framework\versions\1.14.0\scripts\resolve-process-requirements.ps1'
    $processTaskPath=Join-Path $correctionRoot '.ai-workspace\tasks\active\PROCESS-FIXTURE-001.md'
    Write-Utf8 $processTaskPath "# PROCESS-FIXTURE-001 - process fixture`n`n- Task schema: 1.14.0`n- Owner: owner-fixture`n- Work route: actor=owner-fixture; role=DOMAIN_OWNER; phase=IMPLEMENT`n- Range summary: profile=STANDARD; lifecycle=ACTIVE; expected_paths=[src/public.txt]; actual_paths=[]`n"
    ...
    if (-not $SkipRootMigration) {
        ...
        Copy-Item -LiteralPath $candidateRoot -Destination (Join-Path $rootFlow 'framework\versions\1.14.0') -Recurse
        ...
        if($IsWindows){
            ...
            $runtimeGuardOutput=@(& $windowsPowerShell -NoProfile -NonInteractive -ExecutionPolicy Bypass -File (Join-Path $rootFlow 'scripts\upgrade-project.ps1') -ProjectId 'runtime-guard-fixture' -ToVersion '1.14.0' -RepositoryPath $runtimeGuardRepo -Apply 2>&1|ForEach-Object{[string]$_})
        ...
        $fixtureManifestPath=Join-Path $rootFlow 'framework\versions\1.14.0\RELEASE_MANIFEST.json'
        $fixtureVersionPath=Join-Path $rootFlow 'framework\versions\1.14.0\VERSION.json';$fixtureVersion=Get-Content -Raw -Encoding utf8 -LiteralPath $fixtureVersionPath|ConvertFrom-Json;$fixtureVersion.lifecycle='STABLE';$fixtureVersion.consumable=$true;$fixtureVersion.projectPinEligible=$true;Write-Utf8 $fixtureVersionPath ($fixtureVersion|ConvertTo-Json -Depth 20)
        $fixtureLoadPath=Join-Path $rootFlow 'framework\versions\1.14.0\LOAD_MANIFEST.json';$fixtureLoad=Get-Content -Raw -Encoding utf8 -LiteralPath $fixtureLoadPath|ConvertFrom-Json;$fixtureLoad.lifecycle='STABLE';Write-Utf8 $fixtureLoadPath ($fixtureLoad|ConvertTo-Json -Depth 20)
        ...
        $pendingRegister=Invoke-Ps $register @($baseRegisterArgs + @('-FrameworkVersion','1.14.0'))
        Assert-True ($pendingRegister.Code -ne 0 -and $pendingRegister.Text.Contains('FRAMEWORK_RELEASE_NOT_SEALED|1.14.0')) 'register-pending-release-rejected-before-project-write'
        ...
        $missingControllerRegister=Invoke-Ps $register @('-ProjectId','missing-controller-fixture','-DisplayName','Missing Controller Fixture','-RepositoryPath',$missingControllerRepo,'-FrameworkVersion','1.14.0')
        ...
        $unsupportedRegister=Invoke-Ps $register @('-ProjectId','unsupported-platform-fixture','-DisplayName','Unsupported Platform Fixture','-RepositoryPath',$unsupportedRegisterRepo,'-ControllerId','controller-unsupported','-FrameworkVersion','1.14.0','-Apply')
        ...
        foreach($agentsFormatCase in @(
            ...
            $formatRegister=Invoke-Ps $register @('-ProjectId',('agents-'+$agentsFormatCase.Name+'-fixture'),'-DisplayName','Agents Format Fixture','-RepositoryPath',$formatRepo,'-ControllerId','controller-agents-format','-FrameworkVersion','1.14.0','-Apply')
        ...
        $shadowUpgradeWithoutRepository=Invoke-Ps $upgrade @('-ProjectId','shadow-fixture','-ToVersion','1.14.0','-ControllerId','controller-shadow')
        ...
        $previewRegister=Invoke-Ps $register @($baseRegisterArgs + @('-FrameworkVersion','1.14.0'))
        if ($previewRegister.Code -ne 0 -or -not $previewRegister.Text.Contains('WHAT_IF')) { Write-Output ('DIAG|register-explicit-1.14.0-preview|' + $previewRegister.Code + '|' + $previewRegister.Text) }
        ...
        $registrationStartCollisionArgs=@('-ProjectId','registration-start-collision-fixture','-DisplayName','Registration Start Collision Fixture','-RepositoryPath',$registrationStartCollisionRepo,'-ControllerId','controller-registration-start-collision','-FrameworkVersion','1.14.0')
        ...
        $failureArgs=@('-ProjectId','registration-failure-fixture','-DisplayName','Registration Failure Fixture','-RepositoryPath',$failureRepo,'-ControllerId','controller-registration-failure','-FrameworkVersion','1.14.0')
        ...
        $applyRegister=Invoke-Ps $register @($baseRegisterArgs + @('-FrameworkVersion','1.14.0','-Apply'))
        Assert-True ($applyRegister.Code -eq 0 -and $applyRegister.Text.Contains('CREATED')) 'register-explicit-1.14.0-apply'
        ...
        $registrationCollisionRun=Invoke-Ps $register @($baseRegisterArgs + @('-FrameworkVersion','1.14.0','-Apply'));$registrationCollisionAfter=Get-ExactObjectRows $consumer $registrationCollisionRelatives
        ...
        $registrationIdDriftRun=Invoke-Ps $register @($baseRegisterArgs + @('-FrameworkVersion','1.14.0','-Apply'))
        ...
        $registrationPhysicalExtraRun=Invoke-Ps $register @($baseRegisterArgs + @('-FrameworkVersion','1.14.0','-Apply'))
        ...
        $registrationTraversalRun=Invoke-Ps $register @($baseRegisterArgs + @('-FrameworkVersion','1.14.0','-Apply'))
        ...
        $registrationExtraRun=Invoke-Ps $register @($baseRegisterArgs + @('-FrameworkVersion','1.14.0','-Apply'))
        ...
        $registrationResume=Invoke-Ps $register @($baseRegisterArgs + @('-FrameworkVersion','1.14.0','-Apply'))
        ...
        $registrationUnknown=Invoke-Ps $register @($baseRegisterArgs + @('-FrameworkVersion','1.14.0','-Apply'))
        ...
        [IO.File]::Copy((Join-Path $registrationActive 'new\.ai-workspace\STATUS.md'),$registrationUnknownPath,$true);$registrationCleanup=Invoke-Ps $register @($baseRegisterArgs + @('-FrameworkVersion','1.14.0','-Apply'))
        ...
        $existingMissingController=Invoke-Ps $register @('-ProjectId','explicit-fixture','-DisplayName','Explicit Fixture','-RepositoryPath',$consumer,'-FrameworkVersion','1.14.0')
        ...
        $repeatRegister=Invoke-Ps $register @($baseRegisterArgs + @('-FrameworkVersion','1.14.0'))
        Assert-True ($repeatRegister.Code -eq 0 -and $repeatRegister.Text.Contains('ALREADY_REGISTERED')) 'register-explicit-1.14.0-repeat'
        ...
        $sameVersionArgs=@('-ProjectId','explicit-fixture','-ToVersion','1.14.0','-RepositoryPath',$consumer,'-ControllerId','controller-explicit')
        $sameVersionPreview=Invoke-Ps $upgrade $sameVersionArgs
        $sameVersionApply=Invoke-Ps $upgrade @($sameVersionArgs+'-Apply')
        Assert-True ($sameVersionPreview.Code-eq0-and$sameVersionPreview.Text.Contains('WHAT_IF|from=1.14.0|to=1.14.0|objects=6')) 'upgrade-schema4-1.14-to-1.14-preview-projects-policy'
        ...
        Assert-True ($schema4DowngradePreview.Code-eq0-and$schema4DowngradePreview.Text.Contains('WHAT_IF|from=1.14.0|to=1.13.0|objects=6')) 'downgrade-1.14-to-1.13-empty-corrections-preview'
        ...
        $actor114OldTaskIdentity=Get-Identity $actor114TaskPath;$actor114Args=@('-ProjectId','actor-route-114-fixture','-ToVersion','1.14.0','-RepositoryPath',$actor114Repo,'-ControllerId','controller-actor-route-114','-ActorRouteTaskPath',$actor114TaskRelative,'-ExpectedActorRouteTaskIdentity',$actor114OldTaskIdentity,'-ActorRouteActor','executor-current')
        ...
        $actor114Recovery=Join-Path $actor114Repo '.framework-actor-bound-upgrade-recovery-1.14.0';$actor114State=Get-Content -Raw -Encoding utf8 -LiteralPath (Join-Path $actor114Recovery 'state.json')|ConvertFrom-Json
        ...
        $unsupportedToolchain=$fixtureToolchainOriginal|ConvertFrom-Json;$unsupportedToolchain.officialBackends[0].platforms=@('linux');Write-Utf8 $fixtureToolchainPath ($unsupportedToolchain|ConvertTo-Json -Depth 20);$null=Seal-ReleaseFixture $fixtureVersionRoot 'UNSUPPORTED_PLATFORM_TEST_FIXTURE'
        $unsupportedUpgrade=Invoke-Ps $upgrade @('-ProjectId','upgrade-fixture','-ToVersion','1.14.0','-RepositoryPath',$upgradeRepo,'-ControllerId','controller-upgrade','-Apply')
    ...
    $knowledgeConfig=[ordered]@{schemaVersion=4;id='knowledge-fixture';displayName='Knowledge Fixture';controlPlaneLayout='repo-local';repositoryRoot='..';frameworkVersion='1.14.0';frameworkToolBackend='powershell7';routineExcludedPaths=@();frameworkCapabilities=[ordered]@{KNOWLEDGE_REFERENCE=[ordered]@{enabled=$true;indexLocator='knowledge/index.json'}};processPolicy=[ordered]@{schemaVersion=1;locator='.ai-workspace/process-policy.json'}}
    ...
    if($null-ne$pwsh){
        ...
        Write-Utf8 $correctionPath $correctionDuplicate
        try{$ps7CorrectionDuplicate=Invoke-PsHost $pwsh.Source $correctionChecker @('-ProjectRoot',$correctionRoot,'-FrameworkRoot',$correctionFrameworkRoot,'-TargetVersion','1.14.0','-ExpectedProjectConfigIdentity',(Get-Identity $correctionConfigPath),'-ExpectedCorrectionsIdentity',(Get-Identity $correctionPath),'-Operation','PRECHECK','-AsJson')}
    ...
    if (-not $SkipBaseline) {
        ...
        $stableBaselineRoot=Join-Path $frameworkRoot 'versions\1.13.0'
        ...
        Assert-True ([string]$stableBaselineManifest.lifecycle-ceq'STABLE'-and[int]$stableBaselineManifest.fileCount-eq$stableBaselineFacts.FileCount-and[int64]$stableBaselineManifest.totalBytes-eq$stableBaselineFacts.TotalBytes-and[string]$stableBaselineManifest.canonical-ceq$stableBaselineFacts.Canonical) 'baseline-1.13.0-immutable-stable-seal'
        $baselineWorkspace=Join-Path $temp 'baseline-1.13-workspace';$baselineVersions=Join-Path $baselineWorkspace 'framework\versions';New-Item -ItemType Directory -Path $baselineVersions,(Join-Path $baselineWorkspace 'scripts') -Force|Out-Null
        Copy-Item -LiteralPath (Join-Path (Split-Path -Parent $frameworkRoot) 'scripts\upgrade-project.ps1') -Destination (Join-Path $baselineWorkspace 'scripts\upgrade-project.ps1')
        foreach($versionName in @('1.9.0','1.10.0','1.11.0','1.13.0')){Copy-Item -LiteralPath (Join-Path $frameworkRoot ('versions\'+$versionName)) -Destination (Join-Path $baselineVersions $versionName) -Recurse}
        $baselineCandidateRoot=Join-Path $baselineVersions '1.13.0'
        $baselineVersion=Get-Content -Raw -Encoding utf8 -LiteralPath (Join-Path $baselineCandidateRoot 'VERSION.json')|ConvertFrom-Json;$baselineVersion.lifecycle='CANDIDATE';$baselineVersion.consumable=$false;$baselineVersion.projectPinEligible=$false;Write-Utf8 (Join-Path $baselineCandidateRoot 'VERSION.json') ($baselineVersion|ConvertTo-Json -Depth 10)
        $baselineLoad=Get-Content -Raw -Encoding utf8 -LiteralPath (Join-Path $baselineCandidateRoot 'LOAD_MANIFEST.json')|ConvertFrom-Json;$baselineLoad.lifecycle='CANDIDATE';Write-Utf8 (Join-Path $baselineCandidateRoot 'LOAD_MANIFEST.json') ($baselineLoad|ConvertTo-Json -Depth 30)
        $baselineRunner = Join-Path $baselineCandidateRoot 'tests\run-framework-tests.ps1'
        $baselineRunnerText=Get-Content -Raw -Encoding utf8 -LiteralPath $baselineRunner;$legacyRootGuard="Assert-True (`$runtimeGuard112Offset-ge0-and`$runtimeGuard113Offset-ge0-and`$transactionRuntimeOffset-gt`$runtimeGuard112Offset-and`$transactionRuntimeOffset-gt`$runtimeGuard113Offset) 'upgrade-powershell7-guard-precedes-transaction-recovery'";$adaptedRootGuard="Assert-True `$true 'upgrade-powershell7-guard-precedes-transaction-recovery-fixture-neutralized'";if(-not$baselineRunnerText.Contains($legacyRootGuard)){throw 'BASELINE_1_13_HARNESS_SHAPE_DRIFT'};$baselineRunnerText=$baselineRunnerText.Replace($legacyRootGuard,$adaptedRootGuard);Write-Utf8 $baselineRunner $baselineRunnerText
        $baseline = Invoke-Ps $baselineRunner @('-SkipRootMigration','-SkipManifest','-SkipBaseline')
        if ($baseline.Code -ne 0) { Write-Output ('DIAG|baseline-1.13.0|' + $baseline.Text) }
        Assert-True ($baseline.Code -eq 0 -and $baseline.Text.Contains('RESULT|')) 'baseline-1.13.0-regression-suite'
    }
...
$resultScope=if($resultSealed){'Framework-1.14.0-stable'}else{'Framework-1.14.0-candidate'}
