...
if ($schemaMatches.Count -ne 1 -or $taskSchema -notin @('1.5','1.5.2','1.9.0','1.10.0','1.11.0','1.12.0','1.13.0','1.14.0')) { Add-Reason $reasons 'TASK_SCHEMA' }
...
$workRouteLegacyMatches = [regex]::Matches($text, '(?m)^- Work route:\s*role=(?<role>CONTROLLER|DOMAIN_OWNER|EXECUTOR|REVIEWER|FRAMEWORK_MAINTAINER);\s*phase=(?<phase>DISCOVER|PLAN|IMPLEMENT|VERIFY|REVIEW|GIT|EXTERNAL|RECOVER)\s*$')
$workRouteCurrentMatches = [regex]::Matches($text, '(?m)^- Work route:\s*actor=(?<actor>[^;\s]+);\s*role=(?<role>CONTROLLER|DOMAIN_OWNER|EXECUTOR|REVIEWER|FRAMEWORK_MAINTAINER);\s*phase=(?<phase>DISCOVER|PLAN|IMPLEMENT|VERIFY|REVIEW|GIT|EXTERNAL|RECOVER)\s*$')
if ($taskSchema -in @('1.13.0','1.14.0')) {
    if ($workRouteLines.Count -ne 1 -or $workRouteCurrentMatches.Count -ne 1) { Add-Reason $reasons 'WORK_ROUTE_ACTOR_FIELD' }
    elseif ($workRouteCurrentMatches[0].Groups['actor'].Value -in @('UNKNOWN','CONTROLLER','DOMAIN_OWNER','EXECUTOR','REVIEWER','FRAMEWORK_MAINTAINER')) { Add-Reason $reasons 'WORK_ROUTE_ACTOR_VALUE' }
} elseif ($taskSchema -in @('1.11.0','1.12.0')) {
    if ($workRouteLines.Count -ne 1 -or $workRouteLegacyMatches.Count -ne 1) { Add-Reason $reasons 'WORK_ROUTE_FIELD' }
} elseif ($workRouteLines.Count -ne 0 -or $workRouteLegacyMatches.Count -ne 0 -or $workRouteCurrentMatches.Count -ne 0) {
    if ($workRouteLines.Count -ne 1 -or ($workRouteLegacyMatches.Count + $workRouteCurrentMatches.Count) -ne 1) { Add-Reason $reasons 'WORK_ROUTE_FIELD' }
}
$effectiveRoute = if($workRouteCurrentMatches.Count -eq 1){$workRouteCurrentMatches[0]}elseif($workRouteLegacyMatches.Count -eq 1){$workRouteLegacyMatches[0]}else{$null}
$workActor = if($workRouteCurrentMatches.Count -eq 1){$workRouteCurrentMatches[0].Groups['actor'].Value}else{''}
$workRole = if ($null-ne$effectiveRoute) { $effectiveRoute.Groups['role'].Value } else { '' }
$workPhase = if ($null-ne$effectiveRoute) { $effectiveRoute.Groups['phase'].Value } else { '' }
...
if ($taskSchema -in @('1.9.0','1.10.0','1.11.0','1.12.0','1.13.0','1.14.0') -and $profile -ceq 'CRITICAL') {
    ...
    if ($proportionalityMatches.Count -ne 1) {
        ...
        $value = $proportionalityMatches[0].Groups['value'].Value
        $notApplicable = [regex]::Match($value, '^NOT_APPLICABLE;\s*reason=(?<reason>.+)$')
        if ($notApplicable.Success) {
            if (-not (Test-ResolvedField $notApplicable.Groups['reason'].Value)) { Add-Reason $reasons 'PROPORTIONALITY_NOT_APPLICABLE' }
        } else {
            $assessment = [regex]::Match($value, '^existing=(?<existing>sufficient|partial|insufficient);\s*classification=(?<classification>execution_deviation|project_gap|framework_gap|mixed);\s*minimum_sufficient_fix=(?<fix>[^;]+);\s*added_machinery=(?<machinery>[^;]+);\s*escalation_trigger=(?<trigger>.+)$')
            if (-not $assessment.Success -or -not (Test-ResolvedField $assessment.Groups['fix'].Value) -or -not (Test-ResolvedField $assessment.Groups['trigger'].Value)) {
                Add-Reason $reasons 'PROPORTIONALITY_FORMAT'
            } else {
                $machinery = $assessment.Groups['machinery'].Value
                $hasNonzero = $false
                if ($machinery -cne 'NONE') {
                    $machineNames = New-Object 'System.Collections.Generic.HashSet[string]' ([System.StringComparer]::Ordinal)
                    foreach ($item in $machinery.Split(',')) {
                        $part = $item.Trim()
                        $machineMatch = [regex]::Match($part, '^(?<name>[a-z][a-z0-9_-]*):(?<count>\d+)$')
                        if (-not $machineMatch.Success -or -not $machineNames.Add($machineMatch.Groups['name'].Value)) {
                            Add-Reason $reasons 'PROPORTIONALITY_MACHINERY'
                            continue
                        }
                        if ([int64]$machineMatch.Groups['count'].Value -gt 0) { $hasNonzero = $true }
                    }
                }
                if ($assessment.Groups['existing'].Value -ceq 'sufficient' -and $hasNonzero) { Add-Reason $reasons 'PROPORTIONALITY_CONTRADICTION' }
            }
        }
    }
}

if ($taskSchema -in @('1.5.2','1.9.0','1.10.0','1.11.0','1.12.0','1.13.0','1.14.0') -and $profile -ceq 'CRITICAL') {
