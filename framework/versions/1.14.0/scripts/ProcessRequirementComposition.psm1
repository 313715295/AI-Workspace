...
function Invoke-ProcessRequirementComposition {
    ...
    foreach ($requirement in @($catalog.requirements)) {
        Assert-AiwExactFields $requirement @('requirementId','legacyAliases','title','description','ownerModule','selectors','fullText','preparationRequirements','resultRequirements') 'NATIVE_REQUIREMENT'
        ...
        foreach($field in @('title','description','ownerModule','fullText')){if(-not($requirement.$field-is[string])-or[string]::IsNullOrWhiteSpace([string]$requirement.$field)){throw 'NATIVE_REQUIREMENT_TEXT'}}
    ...
    if ($null -ne $correctionsPath) {
        ...
        if ([int]$corrections.schemaVersion -eq 2 -and [string]$corrections.contractVersion -cne '1.14.0') { throw 'CORRECTIONS_CONTRACT' }
    ...
    foreach ($requirement in @($catalog.requirements)) {
        ...
        if ($match.Match) {
            ...
            $selected += [pscustomobject]@{requirementId=$id;source='FRAMEWORK';ownerModule=[string]$requirement.ownerModule;sourceAliases=@($origins);semanticApplicability=$match.Semantic;fullText=[string]$requirement.fullText;preparationRequirements=@($requirement.preparationRequirements);resultRequirements=@($requirement.resultRequirements)}
        }
    }
    foreach ($item in $legacyEffective) {
        if ([int]$item.sourceSchemaVersion -eq 1) {
            ...
            $match=Test-AiwRuleMatch $record.selectors $Profile $Role $Phase $ActionKind $ResultKind $ExactPaths $Capabilities $Objective
            if ($match.Match) {
