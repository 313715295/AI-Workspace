...
try {
    ...
    if ([string]$version.version -cne '1.14.0') { throw 'VERSION_MISMATCH' }
    ...
    foreach ($entry in @($manifest.requirementFragments)) {
        ...
        if ([int]$fragment.schemaVersion -ne 1 -or [string]$fragment.frameworkVersion -cne '1.14.0' -or [string]$fragment.ownerModule -cne $owner -or -not ($fragment.requirements -is [Array])) { throw 'REQUIREMENT_FRAGMENT_VALUES' }
    ...
    $projection = [ordered]@{schemaVersion=1;frameworkVersion='1.14.0';catalogVersion='2';requirements=@($orderedRequirements)}
