...
After loading, resolve `PROCESS_REQUIREMENTS_RESOLVE`. Use `DISCOVER` for the current task/source context and progressive selection, `ADMIT_ACTION` at each distinct governed action boundary and `FINALIZE_OUTPUT` immediately before the final result. Persist no receipt: the host may hold the exact DISCOVER result only for the healthy task/context. Objective/action/result/scope drift rebuilds the selection/boundary decision while an unchanged source composition keeps the same source identity; the host must not claim that the backend avoided a physical reread unless it observed that behavior.
