...
try {
    ...
    if (-not (Test-JsonInteger $config.schemaVersion) -or [int]$config.schemaVersion -ne 4 -or
        ...
        -not ($config.frameworkVersion -is [string]) -or [string]$config.frameworkVersion -cne '1.15.0' -or
    ...
    foreach ($required in @('README.md','framework/versions/1.15.0/RECOVERY_CORE.md','framework/versions/1.15.0/LOAD_MANIFEST.json','framework/versions/1.15.0/TOOLCHAIN.json','framework/versions/1.15.0/FRAMEWORK_MAINTENANCE.md','AGENTS.md')) {
