...
function Get-Receipt([string]$Path){
  ...
  if(-not(Test-JsonInteger $r.schemaVersion)-or[int]$r.schemaVersion-ne1-or-not($r.authorityGranted-is[bool])-or-not($r.semanticCorrectnessProven-is[bool])-or[string]$r.receiptType-cne'PROCESS_REQUIREMENTS_DISCOVER'-or[string]$r.mode-cne'DISCOVER'-or[string]$r.status-cnotin@('PASS','EVALUATION_ONLY')-or[string]$r.frameworkVersion-cne'1.15.0'-or[bool]$r.authorityGranted-or[bool]$r.semanticCorrectnessProven){throw 'DISCOVER_RECEIPT_TYPE'}
...
try{
  ...
  if($mode-ceq'DISCOVER'){
    ...
    $currentBindings=Get-AiwProcessBindingSnapshot -ProjectRoot ([string]$receipt.sourceLocators.projectRoot) -FrameworkRoot ([string]$receipt.sourceLocators.frameworkRoot) -TargetVersion '1.15.0' -TaskRelativePath ([string]$receipt.sourceLocators.taskRelativePath)
