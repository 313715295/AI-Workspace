...
try{
  ...
  $frameworkRoot=Join-Path $temp 'framework-workspace';$sealedRoot=Join-Path $frameworkRoot 'framework\versions\1.15.0';New-Item -ItemType Directory -Path (Split-Path -Parent $sealedRoot) -Force|Out-Null;Copy-Item -LiteralPath $versionRoot -Destination $sealedRoot -Recurse
  ...
  foreach($fixture in @($fixtureContract.fixtures)){
    ...
    $project=[ordered]@{schemaVersion=4;id=('measure-'+[string]$fixture.fixtureId);displayName='Measurement Fixture';controlPlaneLayout='repo-local';repositoryRoot='..';frameworkVersion='1.15.0';frameworkToolBackend='powershell7';routineExcludedPaths=@();frameworkCapabilities=$fixtureCapabilities;processPolicy=[ordered]@{schemaVersion=1;locator='.ai-workspace/process-policy.json'}}
    ...
    Write-Utf8 $taskPath ("# "+$taskId+" - measurement fixture`n`n- Task schema: 1.15.0`n- Owner: "+$taskOwner+"`n- Work route: actor="+[string]$fixture.actor+"; role="+[string]$fixture.role+"; phase="+[string]$fixture.phase+"`n- Range summary: profile="+[string]$fixture.profile+"; lifecycle=ACTIVE; expected_paths=[]; actual_paths=[]`n")
    ...
    if([string]$fixture.actionKind-cne'NONE'){
      ...
      $package=[ordered]@{schemaVersion=1;frameworkVersion='1.15.0';taskId=$taskId;taskIdentity=(Get-Identity $taskPath);profile=[string]$fixture.profile;lifecycle='ACTIVE';owner=$taskOwner;issuer=$issuer;issuerRole=$issuerRole;grantee=[string]$fixture.actor;bundle='MEASUREMENT_LOCAL';decisionClass=$decisionClass;userConfirmation=$userDecision;reviewIndependence=$(if([string]$fixture.actionKind-ceq'REVIEW_EXECUTE'){'INDEPENDENT'}else{'NOT_APPLICABLE'});delegatedGitCloser=$false;actions=@([string]$fixture.actionKind);exactPaths=@($fixture.exactPaths);objectIdentities=$objectIdentities;projectConfigIdentity=(Get-Identity $projectPath);invalidatesOn=@('TASK_CHANGE','OWNER_CHANGE','GRANTEE_CHANGE','ACTION_CHANGE','PATHSET_CHANGE','OBJECT_DRIFT','USER_DECISION_CHANGE','PROJECT_CONFIG_DRIFT')}
