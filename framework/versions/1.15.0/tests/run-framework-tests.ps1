...
function Render-MaintenanceStarter([string]$Starter,[string]$ControlRoot) {
    ...
    Get-ChildItem -LiteralPath $destination -Recurse -Force -File | ForEach-Object {
        ...
        $text = $text.Replace('{{FRAMEWORK_VERSION}}','1.15.0')
...
function New-AuthorizationPackage(
    ...
    Write-Utf8 $taskFull ("# $taskId - authorization fixture`n`n- Task schema: 1.15.0`n- Owner: $issuer`n- Work route: actor=$issuer; role="+$(if($DomainOwner){'DOMAIN_OWNER'}else{'CONTROLLER'})+"; phase=IMPLEMENT`n- Range summary: profile=STANDARD; lifecycle=ACTIVE; expected_paths=[]; actual_paths=[]`n")
    $taskIdentity=Get-Identity $taskFull
    $package = [ordered]@{
        schemaVersion=$Schema; frameworkVersion='1.15.0'; taskId=$taskId; taskIdentity=$taskIdentity; profile='STANDARD'; lifecycle='ACTIVE'
...
Assert-True ([int]$inventory.schemaVersion-eq1-and[string]$inventory.frameworkVersion-ceq'1.15.0'-and$inventoryFields.Count-eq6-and$declaredArray.Count-eq$declaredUnique.Count-and$actual.Count-eq88-and($actual-join"`n")-ceq($declaredArray-join"`n")) 'normative-surface-inventory-exact88-classified-once'
...
Assert-True ([string]$version.version -ceq '1.15.0' -and $versionLifecycleState -and [string]$version.releaseClass -ceq 'MINOR' -and [string]$version.baseline -ceq '1.14.1' -and $null -eq $version.PSObject.Properties['currentEligible']) 'version-lifecycle-fields-minor-baseline-1.14.1-no-global-selector-field'
...
Assert-True ($loadResolverText.Contains('LOAD_MANIFEST_NOT_STABLE_1_15_0') -and -not $loadResolverText.Contains('LOAD_MANIFEST_NOT_STABLE_1_14_1')) 'load-resolver-diagnostic-version-current'
...
Assert-True ([int]$toolchain.schemaVersion-eq1-and[string]$toolchain.frameworkVersion-ceq'1.15.0'-and[string]$toolchain.contractVersion-ceq'1'-and[string]$toolchain.projectSelectionField-ceq'frameworkToolBackend'-and@($toolchain.officialBackends).Count-eq1) 'toolchain-single-project-selected-backend'
...
if($ToolContractOnly){
    ...
    try{
        ...
        $gitConfig=[ordered]@{schemaVersion=3;id='tool-contract-git';displayName='Tool Contract Git';controlPlaneLayout='repo-local';repositoryRoot='..';frameworkVersion='1.15.0';frameworkToolBackend='powershell7';routineExcludedPaths=@();frameworkCapabilities=[ordered]@{KNOWLEDGE_REFERENCE=[ordered]@{enabled=$false}}}
    ...
    Write-Output "RESULT|$($script:passed) passed|scope=Framework-1.15.0-tool-contract"
    return
}
...
$runtimeCandidateRoot=Join-Path $temp 'sealed-runtime\1.15.0'
...
try {
    $null = $normalProjectTemplate.Replace('{{PROJECT_ID_JSON}}','"fixture"').Replace('{{DISPLAY_NAME_JSON}}','"Fixture"').Replace('{{FRAMEWORK_VERSION_JSON}}','"1.15.0"') | ConvertFrom-Json
...
Assert-True ([string]$budgetContract.baseline.version-ceq'1.14.1'-and[string]$budgetContract.baseline.catalogIdentity-ceq'17826|DC9FF91E54F579E80E08CD787023C9BF7BEE1D8C96AED315BE6D3EC41A2249C4'-and[string]$budgetContract.candidate.catalogIdentity-ceq$catalogBudgetIdentity) 'process-budget-minor-baseline-and-candidate-catalog-identities-exact'
...
$canonicalRouterPath=Join-Path $candidateRoot 'host\skills\ai-workspace-router\SKILL.md';$rootRouterPath=Join-Path $liveRepositoryRoot 'skills\ai-workspace-router\SKILL.md';$routerBytes=[int64](Get-Item -LiteralPath $canonicalRouterPath).Length
foreach($context in @($budgetContract.endToEndContexts)){
    ...
    $total=$routerBytes+[int64]$fixture[0].selectedPackBytes
    if($fixture.Count-ne1-or@($context.loadPlanModules).Count-ne0-or[int64]$context.routerSkillBytes-ne$routerBytes-or[int64]$context.loadPlanBytes-ne0-or[int64]$context.selectedPackBytes-ne[int64]$fixture[0].selectedPackBytes-or[int64]$context.frameworkControlledBytes-ne$total-or[int]$context.frameworkControlledEstimatedTokens-ne[int][math]::Ceiling($total/4.0)-or[bool]$context.catalogModelLoaded-or-not[bool]$context.projectTaskFactsExcluded-or$total-gt[int64]$budgetContract.ceilings.frameworkControlledContextBytes-or[int]$context.frameworkControlledEstimatedTokens-gt[int]$budgetContract.ceilings.frameworkControlledContextEstimatedTokens){$contextBudgetsValid=$false}
}
Assert-True ($contextBudgetsValid-and@($budgetContract.endToEndContexts).Count-eq3-and[string]$budgetContract.evidenceCeiling-like'*project/task facts*excluded*') 'process-budget-normal-path-router-plus-selected-blocks-without-load-plan-modules'
Assert-True ((Get-Identity $canonicalRouterPath)-ceq(Get-Identity $rootRouterPath)-and-not(Test-Path -LiteralPath (Join-Path $candidateRoot 'project-starter\.agents\skills\ai-workspace-router'))-and-not(Test-Path -LiteralPath (Join-Path $candidateRoot 'framework-maintenance-starter\.agents\skills\ai-workspace-router'))) 'router-skill-canonical-root-projection-and-no-starter-copy'
foreach($starterName in @('project-starter','framework-maintenance-starter')){
    ...
    Assert-True ($routerBytes-le[int]$budgetContract.ceilings.routerSkillBytes-and$routerSkillText.Contains('relevant user prompt')-and$routerSkillText.Contains('Do not activate for every tool call')-and$routerSkillText.Contains('BOOTSTRAP.md')-and$routerSkillText.Contains('FINALIZE_OUTPUT')) ('host-global-router-skill-bounded-natural-reactivation-no-per-tool|'+$starterName)
...
Assert-True ($evaluationText.Contains('only after a project explicitly adopts stable Framework 1.15.0')-and$evaluationText.Contains('Use normal project tasks')-and$evaluationText.Contains('originating task')-and$evaluationText.Contains('do not require a fixed sample count or time window')-and$evaluationText.Contains('does not block the stable seal')-and-not$evaluationText.Contains('exactly 20 admitted samples')-and-not$evaluationText.Contains('INSUFFICIENT_SAMPLE / NO_ADVANCE')) 'post-release-project-observation-nonblocking-no-synthetic-work'
...
'@
    ...
    Write-Utf8 $standardTaskPath "# TASK-STANDARD-001 - fixture`n`n- Task schema: 1.15.0`n- Owner: owner-fixture`n- Work route: actor=executor-fixture; role=EXECUTOR; phase=IMPLEMENT`n- Range summary: profile=STANDARD; lifecycle=ACTIVE; expected_paths=[]; actual_paths=[]`n"
    ...
    Write-Utf8 $standardTaskPath "# TASK-STANDARD-001 - fixture`n`n- Task schema: 1.15.0`n- Owner: owner-fixture`n- Work route: actor=reviewer-fixture; role=REVIEWER; phase=REVIEW`n- Range summary: profile=STANDARD; lifecycle=ACTIVE; expected_paths=[]; actual_paths=[]`n"
    ...
    Write-Utf8 $missingRouteTaskPath "# TASK-MISSING-ROUTE-001 - fixture`n`n- Task schema: 1.15.0`n- Owner: owner-fixture`n- Range summary: profile=STANDARD; lifecycle=ACTIVE; expected_paths=[]; actual_paths=[]`n"
    ...
    Copy-Item -LiteralPath $candidateRoot -Destination (Join-Path $target 'framework\versions\1.15.0') -Recurse
    $null=Seal-ReleaseFixture (Join-Path $target 'framework\versions\1.15.0') 'MAINTENANCE_TARGET_TEST_FIXTURE'
    ...
    $installedFramework = Join-Path $target 'framework\versions\1.15.0'
    ...
    Assert-True ($missingPin.Code -ne 0 -and $missingPin.Text.Contains('TARGET_REQUIRED_FILE_MISSING|framework/versions/1.15.0/RECOVERY_CORE.md')) 'maintenance-resolver-pin-entry-missing-rejected'
...
'@
    ...
    $repoLocalConfig = [ordered]@{schemaVersion=3;id='repo-local-fixture';displayName='Repo Local Fixture';controlPlaneLayout='repo-local';repositoryRoot='..';frameworkVersion='1.15.0';frameworkToolBackend='powershell7';routineExcludedPaths=@('private/secret.txt');frameworkCapabilities=[pscustomobject]@{KNOWLEDGE_REFERENCE=[pscustomobject]@{enabled=$false}}}
    ...
    $repoLocal4Raw=$projectStarterText.Replace('{{PROJECT_ID_JSON}}','"repo-local-schema4-fixture"').Replace('{{DISPLAY_NAME_JSON}}','"Repo Local Schema4 Fixture"').Replace('{{FRAMEWORK_VERSION_JSON}}','"1.15.0"')
    ...
    Write-Utf8 $domainExternalTaskPath "# FIXTURE-EXTERNAL-001 - external authorization fixture`n`n- Task schema: 1.15.0`n- Owner: owner-fixture`n- Work route: actor=owner-fixture; role=DOMAIN_OWNER; phase=EXTERNAL`n- Range summary: profile=STANDARD; lifecycle=ACTIVE; expected_paths=[src/public.txt]; actual_paths=[]`n"
    ...
    Copy-Item -LiteralPath $candidateRoot -Destination (Join-Path $correctionFrameworkRoot 'framework\versions\1.15.0') -Recurse
    $null=Seal-ReleaseFixture (Join-Path $correctionFrameworkRoot 'framework\versions\1.15.0') 'CORRECTION_TEST_FIXTURE'
    ...
    $correctionConfig=[ordered]@{schemaVersion=3;id='correction-fixture';displayName='Correction Fixture';controlPlaneLayout='repo-local';repositoryRoot='..';frameworkVersion='1.15.0';frameworkToolBackend='powershell7';routineExcludedPaths=@();frameworkCapabilities=[ordered]@{}}
    ...
    $sealedCoveragePath=Join-Path $correctionFrameworkRoot 'framework\versions\1.15.0\CORRECTION_COVERAGE.json';$sealedCoverageOriginal=Get-Content -Raw -Encoding utf8 -LiteralPath $sealedCoveragePath
    $sealedCatalogPath=Join-Path $correctionFrameworkRoot 'framework\versions\1.15.0\PROCESS_REQUIREMENTS.json'
    ...
    $entry113=@($coverageFixture.versions|Where-Object{[string]$_.version-ceq'1.15.0'})[0]
    ...
    $correctionArgs=@('-ProjectRoot',$correctionRoot,'-FrameworkRoot',$correctionFrameworkRoot,'-TargetVersion','1.15.0','-ExpectedProjectConfigIdentity',(Get-Identity $correctionConfigPath),'-ExpectedCorrectionsIdentity',(Get-Identity $correctionPath),'-Operation','PRECHECK','-AsJson')
    ...
    $driftArgs=@('-ProjectRoot',$correctionRoot,'-FrameworkRoot',$correctionFrameworkRoot,'-TargetVersion','1.15.0','-ExpectedProjectConfigIdentity',(Get-Identity $correctionConfigPath),'-ExpectedCorrectionsIdentity',(Get-Identity $correctionPath),'-Operation','PRECHECK','-AsJson')
    ...
    $invalidMapping=$coverageFixture|ConvertTo-Json -Depth 30|ConvertFrom-Json;$invalidEntry=@($invalidMapping.versions|Where-Object{[string]$_.version-ceq'1.15.0'})[0];$invalidEntry.incorporationMappings[0].nativeCatalogIdentity='0|'+('0'*64);Write-Utf8 $sealedCoveragePath ($invalidMapping|ConvertTo-Json -Depth 30);$null=Seal-ReleaseFixture (Split-Path -Parent $sealedCoveragePath) 'INVALID_MAPPING_FIXTURE'
    ...
    $correctionDuplicateRun=Invoke-Ps $correctionChecker @('-ProjectRoot',$correctionRoot,'-FrameworkRoot',$correctionFrameworkRoot,'-TargetVersion','1.15.0','-ExpectedProjectConfigIdentity',(Get-Identity $correctionConfigPath),'-ExpectedCorrectionsIdentity',(Get-Identity $correctionPath),'-Operation','PRECHECK','-AsJson')
    ...
    $missingCorrectionRun=Invoke-Ps $correctionChecker @('-ProjectRoot',$missingCorrectionRoot,'-FrameworkRoot',$correctionFrameworkRoot,'-TargetVersion','1.15.0','-ExpectedProjectConfigIdentity',(Get-Identity $missingCorrectionConfigPath),'-Operation','PRECHECK','-AllowMissingCorrections','-AsJson')
    ...
    $processResolver=Join-Path $correctionFrameworkRoot 'framework\versions\1.15.0\scripts\resolve-process-requirements.ps1'
    $processTaskPath=Join-Path $correctionRoot '.ai-workspace\tasks\active\PROCESS-FIXTURE-001.md'
    Write-Utf8 $processTaskPath "# PROCESS-FIXTURE-001 - process fixture`n`n- Task schema: 1.15.0`n- Owner: owner-fixture`n- Work route: actor=owner-fixture; role=DOMAIN_OWNER; phase=IMPLEMENT`n- Range summary: profile=STANDARD; lifecycle=ACTIVE; expected_paths=[src/public.txt]; actual_paths=[]`n"
    ...
    $runtimeGuard115Offset=$liveUpgradeText.IndexOf("if(`$ToVersion-ceq'1.15.0'",[StringComparison]::Ordinal)
    ...
    if (-not $SkipRootMigration) {
        ...
        Copy-Item -LiteralPath $candidateRoot -Destination (Join-Path $rootFlow 'framework\versions\1.15.0') -Recurse
        ...
        if($IsWindows){
            ...
            $runtimeGuardOutput=@(& $windowsPowerShell -NoProfile -NonInteractive -ExecutionPolicy Bypass -File (Join-Path $rootFlow 'scripts\upgrade-project.ps1') -ProjectId 'runtime-guard-fixture' -ToVersion '1.15.0' -RepositoryPath $runtimeGuardRepo -Apply 2>&1|ForEach-Object{[string]$_})
        ...
        $fixtureManifestPath=Join-Path $rootFlow 'framework\versions\1.15.0\RELEASE_MANIFEST.json'
        $fixtureVersionPath=Join-Path $rootFlow 'framework\versions\1.15.0\VERSION.json';$fixtureVersion=Get-Content -Raw -Encoding utf8 -LiteralPath $fixtureVersionPath|ConvertFrom-Json;$fixtureVersion.lifecycle='STABLE';$fixtureVersion.consumable=$true;$fixtureVersion.projectPinEligible=$true;Write-Utf8 $fixtureVersionPath ($fixtureVersion|ConvertTo-Json -Depth 20)
        $fixtureLoadPath=Join-Path $rootFlow 'framework\versions\1.15.0\LOAD_MANIFEST.json';$fixtureLoad=Get-Content -Raw -Encoding utf8 -LiteralPath $fixtureLoadPath|ConvertFrom-Json;$fixtureLoad.lifecycle='STABLE';Write-Utf8 $fixtureLoadPath ($fixtureLoad|ConvertTo-Json -Depth 20)
        ...
        $pendingRegister=Invoke-Ps $register @($baseRegisterArgs + @('-FrameworkVersion','1.15.0'))
        Assert-True ($pendingRegister.Code -ne 0 -and $pendingRegister.Text.Contains('FRAMEWORK_RELEASE_NOT_SEALED|1.15.0')) 'register-pending-release-rejected-before-project-write'
        ...
        $missingControllerRegister=Invoke-Ps $register @('-ProjectId','missing-controller-fixture','-DisplayName','Missing Controller Fixture','-RepositoryPath',$missingControllerRepo,'-FrameworkVersion','1.15.0')
        ...
        $unsupportedRegister=Invoke-Ps $register @('-ProjectId','unsupported-platform-fixture','-DisplayName','Unsupported Platform Fixture','-RepositoryPath',$unsupportedRegisterRepo,'-ControllerId','controller-unsupported','-FrameworkVersion','1.15.0','-Apply')
        ...
        foreach($agentsFormatCase in @(
            ...
            $formatRegister=Invoke-Ps $register @('-ProjectId',('agents-'+$agentsFormatCase.Name+'-fixture'),'-DisplayName','Agents Format Fixture','-RepositoryPath',$formatRepo,'-ControllerId','controller-agents-format','-FrameworkVersion','1.15.0','-Apply')
        ...
        $shadowUpgradeWithoutRepository=Invoke-Ps $upgrade @('-ProjectId','shadow-fixture','-ToVersion','1.15.0','-ControllerId','controller-shadow')
        ...
        $previewRegister=Invoke-Ps $register @($baseRegisterArgs + @('-FrameworkVersion','1.15.0'))
        if ($previewRegister.Code -ne 0 -or -not $previewRegister.Text.Contains('WHAT_IF')) { Write-Output ('DIAG|register-explicit-1.15.0-preview|' + $previewRegister.Code + '|' + $previewRegister.Text) }
        ...
        $registrationStartCollisionArgs=@('-ProjectId','registration-start-collision-fixture','-DisplayName','Registration Start Collision Fixture','-RepositoryPath',$registrationStartCollisionRepo,'-ControllerId','controller-registration-start-collision','-FrameworkVersion','1.15.0')
        ...
        $failureArgs=@('-ProjectId','registration-failure-fixture','-DisplayName','Registration Failure Fixture','-RepositoryPath',$failureRepo,'-ControllerId','controller-registration-failure','-FrameworkVersion','1.15.0')
        ...
        $applyRegister=Invoke-Ps $register @($baseRegisterArgs + @('-FrameworkVersion','1.15.0','-Apply'))
        Assert-True ($applyRegister.Code -eq 0 -and $applyRegister.Text.Contains('CREATED')) 'register-explicit-1.15.0-apply'
        ...
        $registrationCollisionRun=Invoke-Ps $register @($baseRegisterArgs + @('-FrameworkVersion','1.15.0','-Apply'));$registrationCollisionAfter=Get-ExactObjectRows $consumer $registrationCollisionRelatives
        ...
        $registrationIdDriftRun=Invoke-Ps $register @($baseRegisterArgs + @('-FrameworkVersion','1.15.0','-Apply'))
        ...
        $registrationPhysicalExtraRun=Invoke-Ps $register @($baseRegisterArgs + @('-FrameworkVersion','1.15.0','-Apply'))
        ...
        $registrationTraversalRun=Invoke-Ps $register @($baseRegisterArgs + @('-FrameworkVersion','1.15.0','-Apply'))
        ...
        $registrationExtraRun=Invoke-Ps $register @($baseRegisterArgs + @('-FrameworkVersion','1.15.0','-Apply'))
        ...
        $registrationResume=Invoke-Ps $register @($baseRegisterArgs + @('-FrameworkVersion','1.15.0','-Apply'))
        ...
        $registrationUnknown=Invoke-Ps $register @($baseRegisterArgs + @('-FrameworkVersion','1.15.0','-Apply'))
        ...
        [IO.File]::Copy((Join-Path $registrationActive 'new\.ai-workspace\STATUS.md'),$registrationUnknownPath,$true);$registrationCleanup=Invoke-Ps $register @($baseRegisterArgs + @('-FrameworkVersion','1.15.0','-Apply'))
        ...
        $existingMissingController=Invoke-Ps $register @('-ProjectId','explicit-fixture','-DisplayName','Explicit Fixture','-RepositoryPath',$consumer,'-FrameworkVersion','1.15.0')
        ...
        $repeatRegister=Invoke-Ps $register @($baseRegisterArgs + @('-FrameworkVersion','1.15.0'))
        Assert-True ($repeatRegister.Code -eq 0 -and $repeatRegister.Text.Contains('ALREADY_REGISTERED')) 'register-explicit-1.15.0-repeat'
        ...
        $sameVersionArgs=@('-ProjectId','explicit-fixture','-ToVersion','1.15.0','-RepositoryPath',$consumer,'-ControllerId','controller-explicit')
        $sameVersionPreview=Invoke-Ps $upgrade $sameVersionArgs
        $sameVersionApply=Invoke-Ps $upgrade @($sameVersionArgs+'-Apply')
        Assert-True ($sameVersionPreview.Code-eq0-and$sameVersionPreview.Text.Contains('WHAT_IF|from=1.15.0|to=1.15.0|objects=0|transaction=none')) 'upgrade-schema4-1.15-to-1.15-preview-validates-noop'
        ...
        $actor114OldTaskIdentity=Get-Identity $actor114TaskPath;$actor114Args=@('-ProjectId','actor-route-114-fixture','-ToVersion','1.15.0','-RepositoryPath',$actor114Repo,'-ControllerId','controller-actor-route-114','-ActorRouteTaskPath',$actor114TaskRelative,'-ExpectedActorRouteTaskIdentity',$actor114OldTaskIdentity,'-ActorRouteActor','executor-current')
        $actor114Preview=Invoke-Ps $upgrade $actor114Args;$actor114WriteLines=@($actor114Preview.Output|Where-Object{$_-clike'UPGRADE_WRITESET|*'});if($actor114Preview.Code-ne0-or$actor114WriteLines.Count-ne1){throw ('ASSERT_FAIL|upgrade-actor-bound-1.14-to-1.15-preview|code='+$actor114Preview.Code+'|output='+$actor114Preview.Text)};$actor114WriteLine=$actor114WriteLines[0];$actor114WritePaths=@($actor114WriteLine.Split('|')[1..($actor114WriteLine.Split('|').Count-1)]);$actor114PreimageMap=@{};$actor114PostimageMap=@{}
        ...
        $actor114Recovery=Join-Path $actor114Repo '.ai-workspace/upgrade-recovery/1.15.0';$actor114State=Get-Content -Raw -Encoding utf8 -LiteralPath (Join-Path $actor114Recovery 'state.json')|ConvertFrom-Json
        ...
        $unsupportedToolchain=$fixtureToolchainOriginal|ConvertFrom-Json;$unsupportedToolchain.officialBackends[0].platforms=@('linux');Write-Utf8 $fixtureToolchainPath ($unsupportedToolchain|ConvertTo-Json -Depth 20);$null=Seal-ReleaseFixture $fixtureVersionRoot 'UNSUPPORTED_PLATFORM_TEST_FIXTURE'
        $unsupportedUpgrade=Invoke-Ps $upgrade @('-ProjectId','upgrade-fixture','-ToVersion','1.15.0','-RepositoryPath',$upgradeRepo,'-ControllerId','controller-upgrade','-Apply')
    ...
    $knowledgeConfig=[ordered]@{schemaVersion=4;id='knowledge-fixture';displayName='Knowledge Fixture';controlPlaneLayout='repo-local';repositoryRoot='..';frameworkVersion='1.15.0';frameworkToolBackend='powershell7';routineExcludedPaths=@();frameworkCapabilities=[ordered]@{KNOWLEDGE_REFERENCE=[ordered]@{enabled=$true;indexLocator='knowledge/index.json'}};processPolicy=[ordered]@{schemaVersion=1;locator='.ai-workspace/process-policy.json'}}
    ...
    if($null-ne$pwsh){
        ...
        Write-Utf8 $correctionPath $correctionDuplicate
        try{$ps7CorrectionDuplicate=Invoke-PsHost $pwsh.Source $correctionChecker @('-ProjectRoot',$correctionRoot,'-FrameworkRoot',$correctionFrameworkRoot,'-TargetVersion','1.15.0','-ExpectedProjectConfigIdentity',(Get-Identity $correctionConfigPath),'-ExpectedCorrectionsIdentity',(Get-Identity $correctionPath),'-Operation','PRECHECK','-AsJson')}
    ...
    if (-not $SkipBaseline) {
        ...
        $stableBaselineRoot=Join-Path $frameworkRoot 'versions\1.14.1'
        ...
        Assert-True ([string]$stableBaselineManifest.lifecycle-ceq'STABLE'-and[int]$stableBaselineManifest.fileCount-eq$stableBaselineFacts.FileCount-and[int64]$stableBaselineManifest.totalBytes-eq$stableBaselineFacts.TotalBytes-and[string]$stableBaselineManifest.canonical-ceq$stableBaselineFacts.Canonical-and[string]$stableBaselineManifest.canonical-ceq'547422DE8C0FE8214CA7B1B4980B97FEF065061B8146CA913650613F241693A6') 'baseline-1.14.1-immutable-stable-seal'
    }
...
$resultScope=if($resultSealed){'Framework-1.15.0-stable'}else{'Framework-1.15.0-candidate'}
