...
Schema1 indexes remain readable: their single `authorityLocator`/`authorityIdentity` pair is one declared dependency. Schema2 may declare tags and multiple exact authority dependencies. The official PowerShell 7 backend uses `ConvertFrom-Json -DateKind String` when available so ISO UTC timestamps remain strings; immutable older releases retain their own runtime behavior.
