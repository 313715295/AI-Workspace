...
The official `powershell7` backend is invoked with `pwsh -NoProfile -NonInteractive -File <entrypoint> ...`. Each 1.15.0 backend entrypoint independently rejects a non-Core runtime or PowerShell major version below 7.
...
The contract permits later platform admission, but each released toolchain lists only platforms it actually supports. 1.15.0 lists Windows only. Platform-specific evidence requirements remain explicit:
