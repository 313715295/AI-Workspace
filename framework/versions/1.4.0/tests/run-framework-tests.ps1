...
$testRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("ai-workspace-1.4.0-tests-{0}" -f [Guid]::NewGuid().ToString('N'))
$script:Passed = 0
...
function Copy-StarterWorkspace {
    ...
    foreach ($version in @('1.3.0', '1.3.2', '1.4.0')) {
...
'@
    ...
    $stableVersionNames = @(Get-ChildItem -LiteralPath $versionsRoot -Directory | Where-Object { $_.Name -ne '1.4.0' } | Sort-Object -Property Name | ForEach-Object { $_.Name })
    ...
    $registerPreview = Invoke-IsolatedPowerShell @('-NoProfile', '-ExecutionPolicy', 'Bypass', '-File', $registerScript, '-ProjectId', 'new-project', '-DisplayName', 'New Project', '-RepositoryPath', $registerRepo, '-FrameworkVersion', '1.4.0', '-WorkspaceRoot', $registerWorkspace)
    ...
    $registerApply = Invoke-IsolatedPowerShell @('-NoProfile', '-ExecutionPolicy', 'Bypass', '-File', $registerScript, '-ProjectId', 'new-project', '-DisplayName', 'New Project', '-RepositoryPath', $registerRepo, '-FrameworkVersion', '1.4.0', '-WorkspaceRoot', $registerWorkspace, '-Apply')
    ...
    Assert-True ($registerApply.ExitCode -eq 0 -and $registerApply.Output -match 'CREATED' -and $registeredConfig.schemaVersion -eq 2 -and $registeredConfig.controlPlaneLayout -ceq 'repo-local' -and $registeredConfig.repositoryRoot -ceq '..' -and $registeredConfig.frameworkVersion -ceq '1.4.0' -and $strictRegisteredFiles -eq $registeredFiles.Count -and (Test-Path -LiteralPath (Join-Path $registeredRoot '.gitattributes') -PathType Leaf)) 'register-explicit-1.4.0-apply-schema2-strict-lf' $registerApply.Output

    $registerAgain = Invoke-IsolatedPowerShell @('-NoProfile', '-ExecutionPolicy', 'Bypass', '-File', $registerScript, '-ProjectId', 'new-project', '-DisplayName', 'New Project', '-RepositoryPath', $registerRepo, '-FrameworkVersion', '1.4.0', '-WorkspaceRoot', $registerWorkspace, '-Apply')
    ...
    Assert-True ($registerAgainWithoutVersion.ExitCode -eq 0 -and $registerAgainWithoutVersion.Output -match 'ALREADY_REGISTERED' -and $registerAgainWithoutVersion.Output -match '1\.4\.0') 'register-existing-repo-local-pin-precedes-current-default' $registerAgainWithoutVersion.Output

    $registerExplicitPinConflict = Invoke-IsolatedPowerShell @('-NoProfile', '-ExecutionPolicy', 'Bypass', '-File', $registerScript, '-ProjectId', 'new-project', '-DisplayName', 'New Project', '-RepositoryPath', $registerRepo, '-FrameworkVersion', '1.4.1', '-WorkspaceRoot', $registerWorkspace, '-Apply')
    ...
    $managedRegisterCreate = Invoke-IsolatedPowerShell @('-NoProfile', '-ExecutionPolicy', 'Bypass', '-File', $registerScript, '-ProjectId', 'managed-register', '-DisplayName', 'Managed Register', '-RepositoryPath', $managedRegisterRepo, '-FrameworkVersion', '1.4.0', '-WorkspaceRoot', $registerWorkspace, '-Apply')
    ...
    $managedRegisterAgain = Invoke-IsolatedPowerShell @('-NoProfile', '-ExecutionPolicy', 'Bypass', '-File', $registerScript, '-ProjectId', 'managed-register', '-DisplayName', 'Managed Register', '-RepositoryPath', $managedRegisterRepo, '-FrameworkVersion', '1.4.0', '-WorkspaceRoot', $registerWorkspace, '-Apply')
    ...
    foreach ($markerCase in @(
        ...
        $markerCreate = Invoke-IsolatedPowerShell @('-NoProfile', '-ExecutionPolicy', 'Bypass', '-File', $registerScript, '-ProjectId', $markerCase.Id, '-DisplayName', "Register Custom $($markerCase.Name)", '-RepositoryPath', $markerRepo, '-FrameworkVersion', '1.4.0', '-WorkspaceRoot', $registerWorkspace, '-Apply')
        ...
        $markerAgain = Invoke-IsolatedPowerShell @('-NoProfile', '-ExecutionPolicy', 'Bypass', '-File', $registerScript, '-ProjectId', $markerCase.Id, '-DisplayName', "Register Custom $($markerCase.Name)", '-RepositoryPath', $markerRepo, '-FrameworkVersion', '1.4.0', '-WorkspaceRoot', $registerWorkspace, '-Apply')
    ...
    $registerIncompleteIdentity = Invoke-IsolatedPowerShell @('-NoProfile', '-ExecutionPolicy', 'Bypass', '-File', $registerScript, '-ProjectId', 'new-project', '-DisplayName', 'New Project', '-RepositoryPath', $registerRepo, '-FrameworkVersion', '1.4.0', '-WorkspaceRoot', $registerWorkspace, '-Apply')
    ...
    Copy-StarterWorkspace $registerCurrentWorkspace '1.4.0'
    ...
    Assert-True ($defaultNew.ExitCode -eq 0 -and $defaultNew.Output -match 'WHAT_IF' -and $defaultNew.Output -match '1\.4\.0') 'register-current-default-when-current-is-repo-local-capable' $defaultNew.Output
    ...
    $dirtyRegister = Invoke-IsolatedPowerShell @('-NoProfile', '-ExecutionPolicy', 'Bypass', '-File', $registerScript, '-ProjectId', 'dirty-project', '-DisplayName', 'Dirty Project', '-RepositoryPath', $dirtyRepo, '-FrameworkVersion', '1.4.0', '-WorkspaceRoot', $registerWorkspace, '-Apply')
    ...
    $nonGitRegister = Invoke-IsolatedPowerShell @('-NoProfile', '-ExecutionPolicy', 'Bypass', '-File', $registerScript, '-ProjectId', 'nongit-project', '-DisplayName', 'NonGit Project', '-RepositoryPath', $nonGitRepo, '-FrameworkVersion', '1.4.0', '-WorkspaceRoot', $registerWorkspace, '-Apply')
    ...
    $nestedRegister = Invoke-IsolatedPowerShell @('-NoProfile', '-ExecutionPolicy', 'Bypass', '-File', $registerScript, '-ProjectId', 'nested-project', '-DisplayName', 'Nested Project', '-RepositoryPath', $nestedRegisterPath, '-FrameworkVersion', '1.4.0', '-WorkspaceRoot', $registerWorkspace, '-Apply')
    ...
    Copy-StarterWorkspace $brokenRegisterWorkspace '1.4.0'
    $brokenTemplate = Join-Path $brokenRegisterWorkspace 'framework\versions\1.4.0\project-starter\PROJECT.md'
    ...
    $brokenRegister = Invoke-IsolatedPowerShell @('-NoProfile', '-ExecutionPolicy', 'Bypass', '-File', (Join-Path $brokenRegisterWorkspace 'scripts\register-project.ps1'), '-ProjectId', 'broken-project', '-DisplayName', 'Broken Project', '-RepositoryPath', $brokenRegisterRepo, '-FrameworkVersion', '1.4.0', '-WorkspaceRoot', $brokenRegisterWorkspace, '-Apply')
    ...
    $partialRegister = Invoke-IsolatedPowerShell @('-NoProfile', '-ExecutionPolicy', 'Bypass', '-File', $registerScript, '-ProjectId', 'partial-project', '-DisplayName', 'Partial Project', '-RepositoryPath', $partialRepo, '-FrameworkVersion', '1.4.0', '-WorkspaceRoot', $registerWorkspace, '-Apply')
    ...
    $registerFile = Invoke-IsolatedPowerShell @('-NoProfile', '-ExecutionPolicy', 'Bypass', '-File', $registerScript, '-ProjectId', 'file-project', '-DisplayName', 'File Project', '-RepositoryPath', $registerFileRepo, '-FrameworkVersion', '1.4.0', '-WorkspaceRoot', $registerWorkspace, '-Apply')
    ...
    $registerReparse = Invoke-IsolatedPowerShell @('-NoProfile', '-ExecutionPolicy', 'Bypass', '-File', $registerScript, '-ProjectId', 'reparse-project', '-DisplayName', 'Reparse Project', '-RepositoryPath', $registerReparseRepo, '-FrameworkVersion', '1.4.0', '-WorkspaceRoot', $registerWorkspace, '-Apply')
    ...
    $registerIdentityCreate = Invoke-IsolatedPowerShell @('-NoProfile', '-ExecutionPolicy', 'Bypass', '-File', $registerScript, '-ProjectId', 'identity-one', '-DisplayName', 'Identity One', '-RepositoryPath', $registerIdentityRepo, '-FrameworkVersion', '1.4.0', '-WorkspaceRoot', $registerWorkspace, '-Apply')
    $registerIdentityConflict = Invoke-IsolatedPowerShell @('-NoProfile', '-ExecutionPolicy', 'Bypass', '-File', $registerScript, '-ProjectId', 'identity-two', '-DisplayName', 'Identity Two', '-RepositoryPath', $registerIdentityRepo, '-FrameworkVersion', '1.4.0', '-WorkspaceRoot', $registerWorkspace, '-Apply')
    ...
    foreach ($centralCollisionCase in @(
        ...
        $centralCollisionCreate = Invoke-IsolatedPowerShell @('-NoProfile', '-ExecutionPolicy', 'Bypass', '-File', $registerScript, '-ProjectId', $centralCollisionCase.Id, '-DisplayName', "Central $($centralCollisionCase.Kind)", '-RepositoryPath', $centralCollisionRepo, '-FrameworkVersion', '1.4.0', '-WorkspaceRoot', $registerWorkspace, '-Apply')
        ...
        $centralCollisionAgain = Invoke-IsolatedPowerShell @('-NoProfile', '-ExecutionPolicy', 'Bypass', '-File', $registerScript, '-ProjectId', $centralCollisionCase.Id, '-DisplayName', "Central $($centralCollisionCase.Kind)", '-RepositoryPath', $centralCollisionRepo, '-FrameworkVersion', '1.4.0', '-WorkspaceRoot', $registerWorkspace, '-Apply')
    ...
    $legacyConflict = Invoke-IsolatedPowerShell @('-NoProfile', '-ExecutionPolicy', 'Bypass', '-File', $registerScript, '-ProjectId', 'legacy-conflict', '-DisplayName', 'Legacy Conflict', '-RepositoryPath', $legacyConflictRepo, '-FrameworkVersion', '1.4.0', '-WorkspaceRoot', $registerWorkspace, '-Apply')
    ...
    $centralTopology = Invoke-IsolatedPowerShell @('-NoProfile', '-ExecutionPolicy', 'Bypass', '-File', $upgradeScript, '-ProjectId', 'central-topology', '-ToVersion', '1.4.0', '-WorkspaceRoot', $upgradeWorkspace, '-Apply')
    ...
    Copy-StarterWorkspace $repoUpgradeWorkspace '1.4.0'
    Copy-Item -LiteralPath (Join-Path $repoUpgradeWorkspace 'framework\versions\1.4.0') -Destination (Join-Path $repoUpgradeWorkspace 'framework\versions\1.4.1') -Recurse

    $dualAuthorityRepo = Join-Path $testRoot 'dual-authority-repository'
    Initialize-GitRepository $dualAuthorityRepo
    $dualRegister = Invoke-IsolatedPowerShell @('-NoProfile', '-ExecutionPolicy', 'Bypass', '-File', (Join-Path $repoUpgradeWorkspace 'scripts\register-project.ps1'), '-ProjectId', 'dual-authority', '-DisplayName', 'Dual Authority', '-RepositoryPath', $dualAuthorityRepo, '-FrameworkVersion', '1.4.0', '-WorkspaceRoot', $repoUpgradeWorkspace, '-Apply')
    ...
    $dualRepoLocalUpgrade = Invoke-IsolatedPowerShell @('-NoProfile', '-ExecutionPolicy', 'Bypass', '-File', (Join-Path $repoUpgradeWorkspace 'scripts\upgrade-project.ps1'), '-ProjectId', 'dual-authority', '-ToVersion', '1.4.1', '-WorkspaceRoot', $repoUpgradeWorkspace, '-Apply')
    $dualRepoLocalConfig = Read-StrictUtf8NoBom (Join-Path $dualRepoLocalRoot 'project.json') | ConvertFrom-Json
    Assert-True ($dualRepoLocalUpgrade.ExitCode -eq 0 -and $dualRepoLocalConfig.frameworkVersion -ceq '1.4.1' -and (Get-DirectoryHashManifest $dualCentralRoot) -ceq $dualCentralBefore) 'upgrade-omitted-repository-valid-dual-layout-selects-repo-local-authority' $dualRepoLocalUpgrade.Output
    ...
    foreach ($upgradeCentralCollisionCase in @(
        ...
        $upgradeCentralCollisionResult = Invoke-IsolatedPowerShell @('-NoProfile', '-ExecutionPolicy', 'Bypass', '-File', (Join-Path $repoUpgradeWorkspace 'scripts\upgrade-project.ps1'), '-ProjectId', $upgradeCentralCollisionCase.Id, '-RepositoryPath', $upgradeCentralCollisionRepo, '-ToVersion', '1.4.1', '-WorkspaceRoot', $repoUpgradeWorkspace, '-Apply')
    ...
    foreach ($upgradeMarkerCase in @(
        ...
        $upgradeMarkerRegister = Invoke-IsolatedPowerShell @('-NoProfile', '-ExecutionPolicy', 'Bypass', '-File', (Join-Path $repoUpgradeWorkspace 'scripts\register-project.ps1'), '-ProjectId', $upgradeMarkerCase.Id, '-DisplayName', "Upgrade Custom $($upgradeMarkerCase.Name)", '-RepositoryPath', $upgradeMarkerRepo, '-FrameworkVersion', '1.4.0', '-WorkspaceRoot', $repoUpgradeWorkspace, '-Apply')
        ...
        $upgradeMarkerResult = Invoke-IsolatedPowerShell @('-NoProfile', '-ExecutionPolicy', 'Bypass', '-File', (Join-Path $repoUpgradeWorkspace 'scripts\upgrade-project.ps1'), '-ProjectId', $upgradeMarkerCase.Id, '-RepositoryPath', $upgradeMarkerRepo, '-ToVersion', '1.4.0', '-WorkspaceRoot', $repoUpgradeWorkspace, '-Apply')
    ...
    $repoToCentralRegister = Invoke-IsolatedPowerShell @('-NoProfile', '-ExecutionPolicy', 'Bypass', '-File', (Join-Path $repoUpgradeWorkspace 'scripts\register-project.ps1'), '-ProjectId', 'repo-to-central', '-DisplayName', 'Repo To Central', '-RepositoryPath', $repoToCentralRepo, '-FrameworkVersion', '1.4.0', '-WorkspaceRoot', $repoUpgradeWorkspace, '-Apply')
    ...
    $repoRegister = Invoke-IsolatedPowerShell @('-NoProfile', '-ExecutionPolicy', 'Bypass', '-File', (Join-Path $repoUpgradeWorkspace 'scripts\register-project.ps1'), '-ProjectId', 'repo-upgrade', '-DisplayName', 'Repo Upgrade', '-RepositoryPath', $repoUpgradeRepo, '-FrameworkVersion', '1.4.0', '-WorkspaceRoot', $repoUpgradeWorkspace, '-Apply')
    ...
    $repoUpgrade = Invoke-IsolatedPowerShell @('-NoProfile', '-ExecutionPolicy', 'Bypass', '-File', (Join-Path $repoUpgradeWorkspace 'scripts\upgrade-project.ps1'), '-ProjectId', 'repo-upgrade', '-RepositoryPath', $repoUpgradeRepo, '-ToVersion', '1.4.1', '-WorkspaceRoot', $repoUpgradeWorkspace, '-Apply')
    ...
    Assert-True ($repoRegister.ExitCode -eq 0 -and $repoUpgrade.ExitCode -eq 0 -and $repoUpgradedConfig.frameworkVersion -ceq '1.4.1' -and $customSuffixAfter -ceq $customSuffixBefore -and $repoUpgradedBootstrap -match 'framework/versions/1\.4\.1/scripts/check-task-card\.ps1' -and (Get-UpgradeResidue (Join-Path $repoUpgradeRepo '.ai-workspace')).Count -eq 0) 'upgrade-repo-local-managed-block-preserves-custom-suffix-byte-for-byte' ($repoRegister.Output + "`n" + $repoUpgrade.Output)

    & git -C $repoUpgradeRepo add -- .ai-workspace
    & git -C $repoUpgradeRepo commit --quiet -m 'upgrade result'
    $repoUpgradeAgain = Invoke-IsolatedPowerShell @('-NoProfile', '-ExecutionPolicy', 'Bypass', '-File', (Join-Path $repoUpgradeWorkspace 'scripts\upgrade-project.ps1'), '-ProjectId', 'repo-upgrade', '-RepositoryPath', $repoUpgradeRepo, '-ToVersion', '1.4.1', '-WorkspaceRoot', $repoUpgradeWorkspace, '-Apply')
    ...
    $managedRegistered = Invoke-IsolatedPowerShell @('-NoProfile', '-ExecutionPolicy', 'Bypass', '-File', (Join-Path $repoUpgradeWorkspace 'scripts\register-project.ps1'), '-ProjectId', 'managed-conflict', '-DisplayName', 'Managed Conflict', '-RepositoryPath', $managedConflictRepo, '-FrameworkVersion', '1.4.0', '-WorkspaceRoot', $repoUpgradeWorkspace, '-Apply')
    $managedConflictPath = Join-Path $managedConflictRepo '.ai-workspace\BOOTSTRAP.md'
    $managedConflictText = (Read-StrictUtf8NoBom $managedConflictPath).Replace('framework/versions/1.4.0/scripts/check-task-card.ps1', 'framework/versions/1.4.0/scripts/check-task-card.changed.ps1')
    ...
    $managedConflict = Invoke-IsolatedPowerShell @('-NoProfile', '-ExecutionPolicy', 'Bypass', '-File', (Join-Path $repoUpgradeWorkspace 'scripts\upgrade-project.ps1'), '-ProjectId', 'managed-conflict', '-RepositoryPath', $managedConflictRepo, '-ToVersion', '1.4.1', '-WorkspaceRoot', $repoUpgradeWorkspace, '-Apply')
    ...
    Assert-True ($stableManifestAfter -ceq $stableManifestBefore) 'framework-1.3.2-and-earlier-zero-drift-during-suite'
    ...
    if ($resolvedTest.StartsWith($resolvedTemp, [System.StringComparison]::OrdinalIgnoreCase) -and
        $resolvedTest -match 'ai-workspace-1\.4\.0-tests-[0-9a-f]{32}$' -and
