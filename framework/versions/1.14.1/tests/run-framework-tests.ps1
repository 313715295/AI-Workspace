...
function Render-MaintenanceStarter([string]$Starter,[string]$ControlRoot) {
    ...
    Get-ChildItem -LiteralPath $destination -Recurse -Force -File | ForEach-Object {
        ...
        $text = $text.Replace('{{FRAMEWORK_VERSION}}','1.14.1')
...
function New-AuthorizationPackage(
    ...
    Write-Utf8 $taskFull ("# $taskId - authorization fixture`n`n- Task schema: 1.14.1`n- Owner: $issuer`n- Work route: actor=$issuer; role="+$(if($DomainOwner){'DOMAIN_OWNER'}else{'CONTROLLER'})+"; phase=IMPLEMENT`n- Range summary: profile=STANDARD; lifecycle=ACTIVE; expected_paths=[]; actual_paths=[]`n")
    $taskIdentity=Get-Identity $taskFull
    $package = [ordered]@{
        schemaVersion=$Schema; frameworkVersion='1.14.1'; taskId=$taskId; taskIdentity=$taskIdentity; profile='STANDARD'; lifecycle='ACTIVE'
...
$expected = @(
    'AUTHORITY_CONTEXT_SCHEMA.json','AUTHORIZATION_MODEL.md','CHANGELOG.md','CONTROLLER_SCHEMA.json','CORRECTION_COVERAGE.json','EXAMPLES.md','FRAMEWORK_MAINTENANCE_CONFIG_SCHEMA.json',
    'FRAMEWORK_MAINTENANCE.md','FRAMEWORK_RELEASE.md','framework-maintenance-starter/.agents/skills/ai-workspace-router/SKILL.md','framework-maintenance-starter/.gitattributes','framework-maintenance-starter/AGENTS.md','framework-maintenance-starter/BOOTSTRAP.md',
    ...
    'INTENT_ENVELOPE_SCHEMA.json','LOAD_MANIFEST.json','MIGRATION_MATRIX.md','PERSPECTIVE_LENSES.md','PROCESS_POLICY_SCHEMA.json','PROCESS_REQUIREMENTS.json','PROCESS_REQUIREMENTS_EVALUATION.md','PROJECT_CONFIG_SCHEMA.json','PROJECT_CONTROL.md','PROJECT_CORRECTIONS_SCHEMA.json',
    'project-starter/.agents/skills/ai-workspace-router/SKILL.md','project-starter/.gitattributes','project-starter/AGENTS.md','project-starter/BOOTSTRAP.md','project-starter/controller.json','project-starter/corrections.json','project-starter/process-policy.json','project-starter/project.json',
    ...
    'requirements/fragments/AUTHORIZATION_MODEL.json','requirements/fragments/GIT_AND_EXTERNAL.json','requirements/fragments/HOST_CODEX.json','requirements/fragments/PERSPECTIVE_LENSES.json','requirements/fragments/PROJECT_CONTROL.json','requirements/fragments/RECOVERY_CORE.json','requirements/fragments/REVIEW_AND_EVIDENCE.json','requirements/fragments/TASK_AND_SCOPE.json',
    'scripts/build-process-requirements.ps1','scripts/check-authorization.ps1','scripts/check-knowledge-entry.ps1','scripts/check-knowledge-impact.ps1','scripts/check-project-corrections.ps1','scripts/check-task-card.ps1',
    ...
    'STATIC_COMPARISON.md','TASK_AND_SCOPE.md','TASK_TEMPLATE.md','tests/canonical-identity-reference.mjs','tests/measure-process-requirements.ps1','tests/PROCESS_REQUIREMENTS_BUDGETS.json','tests/PROCESS_REQUIREMENTS_FIXTURES.json','tests/run-framework-tests.ps1','TOOLCHAIN.json','TOOL_CONTRACT.md','VERSION.json'
)
...
Assert-True ($actual.Count -eq 85 -and ($actual -join "`n") -ceq ($expected -join "`n")) 'inventory-exact85'
...
Assert-True ([string]$version.version -ceq '1.14.1' -and $versionLifecycleState -and [string]$version.releaseClass -ceq 'PATCH' -and [string]$version.baseline -ceq '1.14.0' -and $null -eq $version.PSObject.Properties['currentEligible']) 'version-lifecycle-fields-patch-baseline-1.14.0-no-global-selector-field'
Assert-True ([string]$loadManifest.lifecycle -ceq $expectedLifecycle -and @($loadManifest.topologies.PSObject.Properties.Name).Count -eq 2 -and @($loadManifest.topologies.FRAMEWORK_MAINTENANCE_SIBLING) -contains 'FRAMEWORK_MAINTENANCE.md' -and @($loadManifest.core)-cnotcontains'PROCESS_REQUIREMENTS.json' -and @($loadManifest.order)-cnotcontains'PROCESS_REQUIREMENTS.json') 'load-manifest-lifecycle-topology-and-no-catalog-preload-contract'
$loadResolverText=Get-Content -LiteralPath (Join-Path $candidateRoot 'scripts\resolve-load-plan.ps1') -Raw -Encoding utf8
Assert-True ($loadResolverText.Contains('LOAD_MANIFEST_NOT_STABLE_1_14_1') -and -not $loadResolverText.Contains('LOAD_MANIFEST_NOT_STABLE_1_14_0')) 'load-resolver-diagnostic-version-current'
...
Assert-True ([int]$toolchain.schemaVersion-eq1-and[string]$toolchain.frameworkVersion-ceq'1.14.1'-and[string]$toolchain.contractVersion-ceq'1'-and[string]$toolchain.projectSelectionField-ceq'frameworkToolBackend'-and@($toolchain.officialBackends).Count-eq1) 'toolchain-single-project-selected-backend'
...
Assert-True (-not(Test-Path -LiteralPath (Join-Path $liveRepositoryRoot '.github\workflows\framework-toolchain-conformance.yml'))) 'three-platform-ci-not-selected-for-windows-only-1.14-scope'
...
if($ToolContractOnly){
    ...
    try{
        ...
        $gitConfig=[ordered]@{schemaVersion=3;id='tool-contract-git';displayName='Tool Contract Git';controlPlaneLayout='repo-local';repositoryRoot='..';frameworkVersion='1.14.1';frameworkToolBackend='powershell7';routineExcludedPaths=@();frameworkCapabilities=[ordered]@{KNOWLEDGE_REFERENCE=[ordered]@{enabled=$false}}}
    ...
    Write-Output "RESULT|$($script:passed) passed|scope=Framework-1.14.1-tool-contract"
    return
}
...
$runtimeCandidateRoot=Join-Path $temp 'sealed-runtime\1.14.1'
...
try {
    $null = $normalProjectTemplate.Replace('{{PROJECT_ID_JSON}}','"fixture"').Replace('{{DISPLAY_NAME_JSON}}','"Fixture"').Replace('{{FRAMEWORK_VERSION_JSON}}','"1.14.1"') | ConvertFrom-Json
...
Assert-True ($starterActual.Count -eq 13 -and $starterActual -contains 'corrections.json' -and $starterActual -contains 'process-policy.json' -and $starterActual -contains 'AGENTS.md' -and $starterActual -contains '.agents/skills/ai-workspace-router/SKILL.md') 'maintenance-starter-exact13-with-router-corrections-and-policy'
$budgetContract=Get-Content -Raw -Encoding utf8 -LiteralPath (Join-Path $candidateRoot 'tests\PROCESS_REQUIREMENTS_BUDGETS.json')|ConvertFrom-Json
...
Assert-True ([string]$budgetContract.baseline.version-ceq'1.14.0'-and[string]$budgetContract.baseline.catalogIdentity-ceq'17826|A0C5CB3166074F10B2154E335240D39F778A62528122DD50AEBFE622255AA424'-and[string]$budgetContract.candidate.catalogIdentity-ceq$catalogBudgetIdentity) 'process-budget-patch-baseline-and-candidate-catalog-identities-exact'
...
Assert-True ($fixtureBudgetsValid-and$fixtureIds.Count-eq6-and@($budgetContract.measurementContract.statistics)-contains'median'-and@($budgetContract.measurementContract.statistics)-contains'nearest-rank-p95'-and[int]$budgetContract.measurementContract.warmupsPerModeAndFixture-eq3-and[int]$budgetContract.measurementContract.measuredRunsPerModeAndFixture-eq30-and[string]$budgetContract.measurementContract.toleranceFormula-ceq'ceiling(max(2.0 * baselineP95Ms, baselineP95Ms + 250))'-and[string]$budgetContract.candidate.fixtureContractIdentity-ceq(Get-Identity $fixtureContractPath)-and[string]$budgetContract.candidate.measurementHarnessIdentity-ceq(Get-Identity $measurementHarnessPath)) 'process-budget-six-exact-fixtures-and-measurement-protocol-bound'
...
$ordinaryContextLoad=Invoke-Ps $loader @('-Role','DOMAIN_OWNER','-Profile','STANDARD','-Phase','IMPLEMENT','-IncludeRecovery','-HostName','CODEX','-AsJson')
$criticalContextLoad=Invoke-Ps $loader @('-Role','REVIEWER','-Profile','CRITICAL','-Phase','REVIEW','-IncludeRecovery','-HostName','CODEX','-AsJson')
$maintenanceContextLoad=Invoke-Ps $loader @('-Role','FRAMEWORK_MAINTAINER','-Profile','CRITICAL','-Phase','REVIEW','-IncludeRecovery','-HostName','CODEX','-Topology','FRAMEWORK_MAINTENANCE_SIBLING','-AsJson')
$contextRuns=[ordered]@{ordinary_source=$ordinaryContextLoad;critical_review=$criticalContextLoad;framework_maintenance_review=$maintenanceContextLoad};$contextBudgetsValid=$true
$projectRouterPath=Join-Path $candidateRoot 'project-starter\.agents\skills\ai-workspace-router\SKILL.md';$projectRouterBytes=(Get-Item -LiteralPath $projectRouterPath).Length
$maintenanceRouterPath=Join-Path $candidateRoot 'framework-maintenance-starter\.agents\skills\ai-workspace-router\SKILL.md';$maintenanceRouterBytes=(Get-Item -LiteralPath $maintenanceRouterPath).Length
$contextRouterBytes=[ordered]@{ordinary_source=$projectRouterBytes;critical_review=$projectRouterBytes;framework_maintenance_review=$maintenanceRouterBytes}
foreach($context in @($budgetContract.endToEndContexts)){
    $run=$contextRuns[[string]$context.caseId];if($null-eq$run-or$run.Code-ne0){$contextBudgetsValid=$false;continue}
    try{$load=$run.Text|ConvertFrom-Json}catch{$contextBudgetsValid=$false;continue}
    $actualModules=@($load.modules|ForEach-Object{[string]$_.path});$declaredModules=@($context.loadPlanModules|ForEach-Object{[string]$_})
    $fixture=@($budgetContract.fixtures|Where-Object{[string]$_.fixtureId-ceq[string]$context.caseId})
    $routerBytes=[int64]$contextRouterBytes[[string]$context.caseId];$total=$routerBytes+[int64]$load.totalBytes+[int64]$fixture[0].selectedPackBytes
    if($fixture.Count-ne1-or$actualModules.Count-ne$declaredModules.Count-or($actualModules-join"`n")-cne($declaredModules-join"`n")-or$actualModules-contains'PROCESS_REQUIREMENTS.json'-or[int64]$context.routerSkillBytes-ne$routerBytes-or[int64]$context.loadPlanBytes-ne[int64]$load.totalBytes-or[int64]$context.selectedPackBytes-ne[int64]$fixture[0].selectedPackBytes-or[int64]$context.frameworkControlledBytes-ne$total-or[int]$context.frameworkControlledEstimatedTokens-ne[int][math]::Ceiling($total/4.0)-or[bool]$context.catalogModelLoaded-or-not[bool]$context.projectTaskFactsExcluded-or$total-gt[int64]$budgetContract.ceilings.frameworkControlledContextBytes-or[int]$context.frameworkControlledEstimatedTokens-gt[int]$budgetContract.ceilings.frameworkControlledContextEstimatedTokens){$contextBudgetsValid=$false}
}
Assert-True ($contextBudgetsValid-and@($budgetContract.endToEndContexts).Count-eq3-and[string]$budgetContract.evidenceCeiling-like'*project/task facts*excluded*') 'process-budget-end-to-end-project-and-maintenance-router-load-plan-selected-pack-with-variable-project-facts-excluded'
foreach($starterName in @('project-starter','framework-maintenance-starter')){
    $routerSkillPath=Join-Path $candidateRoot ($starterName+'\.agents\skills\ai-workspace-router\SKILL.md');$routerSkillText=Get-Content -Raw -Encoding utf8 -LiteralPath $routerSkillPath;$routerSkillBytes=[IO.File]::ReadAllBytes($routerSkillPath).Length
    $agentsText=Get-Content -Raw -Encoding utf8 -LiteralPath (Join-Path $candidateRoot ($starterName+'\AGENTS.md'))
    Assert-True ($routerSkillBytes-le[int]$budgetContract.ceilings.routerSkillBytes-and$routerSkillText.Contains('natural prompt')-and($routerSkillText.Contains('Do not activate for every tool call')-or$routerSkillText.Contains('Do not invoke per tool call'))-and$routerSkillText.Contains('BOOTSTRAP.md')-and$routerSkillText.Contains('FINALIZE_OUTPUT')) ('router-skill-bounded-natural-reactivation-no-per-tool|'+$starterName)
...
Assert-True ($catalogCheck.Code-eq0-and$catalogCheck.Text.Contains('PASS|requirements=14|fragments=8')) 'process-requirements-canonical-fragments-match-generated-catalog'
...
Assert-True ($evaluationText.Contains('only after a project explicitly adopts stable Framework 1.14.1')-and$evaluationText.Contains('Use normal project tasks')-and$evaluationText.Contains('originating task')-and$evaluationText.Contains('do not require a fixed sample count or time window')-and$evaluationText.Contains('does not block the stable seal')-and-not$evaluationText.Contains('exactly 20 admitted samples')-and-not$evaluationText.Contains('INSUFFICIENT_SAMPLE / NO_ADVANCE')) 'post-release-project-observation-nonblocking-no-synthetic-work'
Assert-True ($releaseGovernanceText.Contains('At the final candidate freeze run the complete current-version suite once')-and$releaseGovernanceText.Contains('Re-run the baseline''s executable suite only when shared root tools, upgrade compatibility, Tool Contract or that baseline''s execution boundary is affected')-and$releaseGovernanceText.Contains('One independent CRITICAL Source Review')-and$releaseGovernanceText.Contains('Maintenance OWNER_ACCEPT separately accepts the exact approved candidate')-and$releaseGovernanceText.Contains('A second independent semantic post-seal Review is not required')-and$reviewEvidenceText.Contains('Same-scope repair receives a fresh writer package')-and$reviewEvidenceText.Contains('focused rereview')-and$reviewEvidenceText.Contains('OWNER_ACCEPT')-and$reviewEvidenceText.Contains('deterministic checks verify the exact allowlist')) 'release-sequence-proportional-source-review-owner-accept-and-deterministic-seal'
...
'@
    ...
    Write-Utf8 $standardTaskPath "# TASK-STANDARD-001 - fixture`n`n- Task schema: 1.14.1`n- Owner: owner-fixture`n- Work route: actor=executor-fixture; role=EXECUTOR; phase=IMPLEMENT`n- Range summary: profile=STANDARD; lifecycle=ACTIVE; expected_paths=[]; actual_paths=[]`n"
    ...
    Write-Utf8 $standardTaskPath "# TASK-STANDARD-001 - fixture`n`n- Task schema: 1.14.1`n- Owner: owner-fixture`n- Work route: actor=reviewer-fixture; role=REVIEWER; phase=REVIEW`n- Range summary: profile=STANDARD; lifecycle=ACTIVE; expected_paths=[]; actual_paths=[]`n"
    ...
    Write-Utf8 $missingRouteTaskPath "# TASK-MISSING-ROUTE-001 - fixture`n`n- Task schema: 1.14.1`n- Owner: owner-fixture`n- Range summary: profile=STANDARD; lifecycle=ACTIVE; expected_paths=[]; actual_paths=[]`n"
    ...
    Copy-Item -LiteralPath $candidateRoot -Destination (Join-Path $target 'framework\versions\1.14.1') -Recurse
    $null=Seal-ReleaseFixture (Join-Path $target 'framework\versions\1.14.1') 'MAINTENANCE_TARGET_TEST_FIXTURE'
    ...
    $installedFramework = Join-Path $target 'framework\versions\1.14.1'
    ...
    Assert-True ($installedActual.Count -eq 85 -and ($installedActual -join "`n") -ceq ($actual -join "`n")) 'maintenance-target-full-sealed-fixture-installed'
    ...
    Assert-True ($missingPin.Code -ne 0 -and $missingPin.Text.Contains('TARGET_REQUIRED_FILE_MISSING|framework/versions/1.14.1/RECOVERY_CORE.md')) 'maintenance-resolver-pin-entry-missing-rejected'
    ...
    Write-Utf8 $maintenanceProcessInputPath ($maintenanceProcessInput|ConvertTo-Json -Depth 30);$maintenanceProcessRun=Invoke-Ps $maintenanceProcessResolver @('-InputPath',$maintenanceProcessInputPath,'-AsJson');$maintenanceProcessValue=if($maintenanceProcessRun.Code-eq0){$maintenanceProcessRun.Output[-1]|ConvertFrom-Json}else{$null}
...
'@
    ...
    $repoLocalConfig = [ordered]@{schemaVersion=3;id='repo-local-fixture';displayName='Repo Local Fixture';controlPlaneLayout='repo-local';repositoryRoot='..';frameworkVersion='1.14.1';frameworkToolBackend='powershell7';routineExcludedPaths=@('private/secret.txt');frameworkCapabilities=[pscustomobject]@{KNOWLEDGE_REFERENCE=[pscustomobject]@{enabled=$false}}}
    ...
    $repoLocal4Raw=$projectStarterText.Replace('{{PROJECT_ID_JSON}}','"repo-local-schema4-fixture"').Replace('{{DISPLAY_NAME_JSON}}','"Repo Local Schema4 Fixture"').Replace('{{FRAMEWORK_VERSION_JSON}}','"1.14.1"')
    ...
    Write-Utf8 $domainExternalTaskPath "# FIXTURE-EXTERNAL-001 - external authorization fixture`n`n- Task schema: 1.14.1`n- Owner: owner-fixture`n- Work route: actor=owner-fixture; role=DOMAIN_OWNER; phase=EXTERNAL`n- Range summary: profile=STANDARD; lifecycle=ACTIVE; expected_paths=[src/public.txt]; actual_paths=[]`n"
    ...
    Copy-Item -LiteralPath $candidateRoot -Destination (Join-Path $correctionFrameworkRoot 'framework\versions\1.14.1') -Recurse
    $null=Seal-ReleaseFixture (Join-Path $correctionFrameworkRoot 'framework\versions\1.14.1') 'CORRECTION_TEST_FIXTURE'
    ...
    $correctionConfig=[ordered]@{schemaVersion=3;id='correction-fixture';displayName='Correction Fixture';controlPlaneLayout='repo-local';repositoryRoot='..';frameworkVersion='1.14.1';frameworkToolBackend='powershell7';routineExcludedPaths=@();frameworkCapabilities=[ordered]@{}}
    ...
    $sealedCoveragePath=Join-Path $correctionFrameworkRoot 'framework\versions\1.14.1\CORRECTION_COVERAGE.json';$sealedCoverageOriginal=Get-Content -Raw -Encoding utf8 -LiteralPath $sealedCoveragePath
    $sealedCatalogPath=Join-Path $correctionFrameworkRoot 'framework\versions\1.14.1\PROCESS_REQUIREMENTS.json'
    ...
    $entry113=@($coverageFixture.versions|Where-Object{[string]$_.version-ceq'1.14.1'})[0]
    ...
    $correctionArgs=@('-ProjectRoot',$correctionRoot,'-FrameworkRoot',$correctionFrameworkRoot,'-TargetVersion','1.14.1','-ExpectedProjectConfigIdentity',(Get-Identity $correctionConfigPath),'-ExpectedCorrectionsIdentity',(Get-Identity $correctionPath),'-Operation','PRECHECK','-AsJson')
    ...
    $driftArgs=@('-ProjectRoot',$correctionRoot,'-FrameworkRoot',$correctionFrameworkRoot,'-TargetVersion','1.14.1','-ExpectedProjectConfigIdentity',(Get-Identity $correctionConfigPath),'-ExpectedCorrectionsIdentity',(Get-Identity $correctionPath),'-Operation','PRECHECK','-AsJson')
    ...
    $invalidMapping=$coverageFixture|ConvertTo-Json -Depth 30|ConvertFrom-Json;$invalidEntry=@($invalidMapping.versions|Where-Object{[string]$_.version-ceq'1.14.1'})[0];$invalidEntry.incorporationMappings[0].nativeCatalogIdentity='0|'+('0'*64);Write-Utf8 $sealedCoveragePath ($invalidMapping|ConvertTo-Json -Depth 30);$null=Seal-ReleaseFixture (Split-Path -Parent $sealedCoveragePath) 'INVALID_MAPPING_FIXTURE'
    ...
    $correctionDuplicateRun=Invoke-Ps $correctionChecker @('-ProjectRoot',$correctionRoot,'-FrameworkRoot',$correctionFrameworkRoot,'-TargetVersion','1.14.1','-ExpectedProjectConfigIdentity',(Get-Identity $correctionConfigPath),'-ExpectedCorrectionsIdentity',(Get-Identity $correctionPath),'-Operation','PRECHECK','-AsJson')
    ...
    $missingCorrectionRun=Invoke-Ps $correctionChecker @('-ProjectRoot',$missingCorrectionRoot,'-FrameworkRoot',$correctionFrameworkRoot,'-TargetVersion','1.14.1','-ExpectedProjectConfigIdentity',(Get-Identity $missingCorrectionConfigPath),'-Operation','PRECHECK','-AllowMissingCorrections','-AsJson')
    ...
    $processResolver=Join-Path $correctionFrameworkRoot 'framework\versions\1.14.1\scripts\resolve-process-requirements.ps1'
    $processTaskPath=Join-Path $correctionRoot '.ai-workspace\tasks\active\PROCESS-FIXTURE-001.md'
    Write-Utf8 $processTaskPath "# PROCESS-FIXTURE-001 - process fixture`n`n- Task schema: 1.14.1`n- Owner: owner-fixture`n- Work route: actor=owner-fixture; role=DOMAIN_OWNER; phase=IMPLEMENT`n- Range summary: profile=STANDARD; lifecycle=ACTIVE; expected_paths=[src/public.txt]; actual_paths=[]`n"
    ...
    $semanticIdentityRun=Invoke-Ps $processResolver @('-InputPath',$discoverInputPath,'-AsJson');$semanticIdentityValue=$semanticIdentityRun.Output[-1]|ConvertFrom-Json
    ...
    $unknownMismatchRun=Invoke-Ps $processResolver @('-InputPath',$discoverInputPath,'-AsJson');$unknownMismatchValue=$unknownMismatchRun.Output[-1]|ConvertFrom-Json
    ...
    $v2Prep=@($discoverV2Value.selectedRequirements|ForEach-Object{@($_.preparationRequirements)}|Sort-Object -Unique)
    $v2Result=@($discoverV2Value.selectedRequirements|ForEach-Object{@($_.resultRequirements)}|Sort-Object -Unique)
    ...
    $ambiguousDiscover=Invoke-Ps $processResolver @('-InputPath',$discoverInputPath,'-AsJson');$ambiguousValue=$ambiguousDiscover.Output[-1]|ConvertFrom-Json
    ...
    $noAuthDiscover=Invoke-Ps $processResolver @('-InputPath',$discoverInputPath,'-AsJson');$noAuthValue=$noAuthDiscover.Output[-1]|ConvertFrom-Json
    ...
    $v2CorrectionDiscover=Invoke-Ps $processResolver @('-InputPath',$discoverInputPath,'-AsJson');$v2CorrectionValue=$v2CorrectionDiscover.Output[-1]|ConvertFrom-Json
    Assert-True ($v2CorrectionDiscover.Code-eq0-and@($v2CorrectionValue.selectedRequirements.requirementId)-contains'correction:correction-fixture:V2_SOURCE_ONLY'-and@($v2CorrectionValue.selectedRequirements.requirementId)-notcontains'correction:correction-fixture:V2_EXTERNAL_ONLY'-and[int]$v2CorrectionValue.legacyCorrectionsFullReadCount-eq0-and@($v2CorrectionValue.evidenceCeilings)-notcontains'LEGACY_CORRECTIONS_FULL_LOAD') 'corrections-v2-progressive-selection-without-legacy-full-load-claim'
    Import-Module (Join-Path $candidateRoot 'scripts\ProcessRequirementComposition.psm1') -Force
    ...
    Assert-True ($runtimeGuard112Offset-ge0-and$runtimeGuard113Offset-ge0-and$runtimeGuard1141Offset-ge0-and$transactionRuntimeOffset-gt$runtimeGuard112Offset-and$transactionRuntimeOffset-gt$runtimeGuard113Offset-and$transactionRuntimeOffset-gt$runtimeGuard1141Offset) 'upgrade-powershell7-guard-precedes-transaction-recovery'
    ...
    if (-not $SkipRootMigration) {
        ...
        Copy-Item -LiteralPath $candidateRoot -Destination (Join-Path $rootFlow 'framework\versions\1.14.1') -Recurse
        ...
        if($IsWindows){
            ...
            $runtimeGuardOutput=@(& $windowsPowerShell -NoProfile -NonInteractive -ExecutionPolicy Bypass -File (Join-Path $rootFlow 'scripts\upgrade-project.ps1') -ProjectId 'runtime-guard-fixture' -ToVersion '1.14.1' -RepositoryPath $runtimeGuardRepo -Apply 2>&1|ForEach-Object{[string]$_})
        ...
        $fixtureManifestPath=Join-Path $rootFlow 'framework\versions\1.14.1\RELEASE_MANIFEST.json'
        $fixtureVersionPath=Join-Path $rootFlow 'framework\versions\1.14.1\VERSION.json';$fixtureVersion=Get-Content -Raw -Encoding utf8 -LiteralPath $fixtureVersionPath|ConvertFrom-Json;$fixtureVersion.lifecycle='STABLE';$fixtureVersion.consumable=$true;$fixtureVersion.projectPinEligible=$true;Write-Utf8 $fixtureVersionPath ($fixtureVersion|ConvertTo-Json -Depth 20)
        $fixtureLoadPath=Join-Path $rootFlow 'framework\versions\1.14.1\LOAD_MANIFEST.json';$fixtureLoad=Get-Content -Raw -Encoding utf8 -LiteralPath $fixtureLoadPath|ConvertFrom-Json;$fixtureLoad.lifecycle='STABLE';Write-Utf8 $fixtureLoadPath ($fixtureLoad|ConvertTo-Json -Depth 20)
        ...
        $pendingRegister=Invoke-Ps $register @($baseRegisterArgs + @('-FrameworkVersion','1.14.1'))
        Assert-True ($pendingRegister.Code -ne 0 -and $pendingRegister.Text.Contains('FRAMEWORK_RELEASE_NOT_SEALED|1.14.1')) 'register-pending-release-rejected-before-project-write'
        ...
        $missingControllerRegister=Invoke-Ps $register @('-ProjectId','missing-controller-fixture','-DisplayName','Missing Controller Fixture','-RepositoryPath',$missingControllerRepo,'-FrameworkVersion','1.14.1')
        ...
        $unsupportedRegister=Invoke-Ps $register @('-ProjectId','unsupported-platform-fixture','-DisplayName','Unsupported Platform Fixture','-RepositoryPath',$unsupportedRegisterRepo,'-ControllerId','controller-unsupported','-FrameworkVersion','1.14.1','-Apply')
        ...
        foreach($agentsFormatCase in @(
            ...
            $formatRegister=Invoke-Ps $register @('-ProjectId',('agents-'+$agentsFormatCase.Name+'-fixture'),'-DisplayName','Agents Format Fixture','-RepositoryPath',$formatRepo,'-ControllerId','controller-agents-format','-FrameworkVersion','1.14.1','-Apply')
        ...
        $skillCollisionRegister=Invoke-Ps $register @('-ProjectId','skill-collision-fixture','-DisplayName','Skill Collision Fixture','-RepositoryPath',$skillCollisionRepo,'-ControllerId','controller-skill-collision','-FrameworkVersion','1.14.1','-Apply')
        ...
        try{$registerReparseRun=Invoke-Ps $register @('-ProjectId','register-reparse-fixture','-DisplayName','Register Reparse Fixture','-RepositoryPath',$registerReparseRepo,'-ControllerId','controller-register-reparse','-FrameworkVersion','1.14.1','-Apply')}
        ...
        try{$installReparseRun=Invoke-Ps $upgrade @('-ProjectId','upgrade-router-reparse-fixture','-ToVersion','1.14.1','-RepositoryPath',$installReparseRepo,'-ControllerId','controller-upgrade-reparse','-Apply')}
        ...
        $downgradeReparseRegister=Invoke-Ps $register @('-ProjectId','downgrade-router-reparse-fixture','-DisplayName','Downgrade Router Reparse Fixture','-RepositoryPath',$downgradeReparseRepo,'-ControllerId','controller-downgrade-reparse','-FrameworkVersion','1.14.1','-Apply')
        ...
        $shadowUpgradeWithoutRepository=Invoke-Ps $upgrade @('-ProjectId','shadow-fixture','-ToVersion','1.14.1','-ControllerId','controller-shadow')
        ...
        $previewRegister=Invoke-Ps $register @($baseRegisterArgs + @('-FrameworkVersion','1.14.1'))
        if ($previewRegister.Code -ne 0 -or -not $previewRegister.Text.Contains('WHAT_IF')) { Write-Output ('DIAG|register-explicit-1.14.1-preview|' + $previewRegister.Code + '|' + $previewRegister.Text) }
        $registrationWriteSet=@('AGENTS.md','.agents/skills/ai-workspace-router/SKILL.md','.ai-workspace/.gitattributes','.ai-workspace/project.json','.ai-workspace/BOOTSTRAP.md','.ai-workspace/PROJECT.md','.ai-workspace/REVIEW_PROFILE.md','.ai-workspace/RELATIONSHIPS.md','.ai-workspace/STATUS.md','.ai-workspace/tasks/README.md','.ai-workspace/controller.json','.ai-workspace/corrections.json','.ai-workspace/process-policy.json')
        ...
        $registrationStartCollisionArgs=@('-ProjectId','registration-start-collision-fixture','-DisplayName','Registration Start Collision Fixture','-RepositoryPath',$registrationStartCollisionRepo,'-ControllerId','controller-registration-start-collision','-FrameworkVersion','1.14.1')
        ...
        $failureArgs=@('-ProjectId','registration-failure-fixture','-DisplayName','Registration Failure Fixture','-RepositoryPath',$failureRepo,'-ControllerId','controller-registration-failure','-FrameworkVersion','1.14.1')
        ...
        $applyRegister=Invoke-Ps $register @($baseRegisterArgs + @('-FrameworkVersion','1.14.1','-Apply'))
        Assert-True ($applyRegister.Code -eq 0 -and $applyRegister.Text.Contains('CREATED')) 'register-explicit-1.14.1-apply'
        ...
        $registrationCollisionRun=Invoke-Ps $register @($baseRegisterArgs + @('-FrameworkVersion','1.14.1','-Apply'));$registrationCollisionAfter=Get-ExactObjectRows $consumer $registrationCollisionRelatives
        ...
        $registrationIdDriftRun=Invoke-Ps $register @($baseRegisterArgs + @('-FrameworkVersion','1.14.1','-Apply'))
        ...
        $registrationPhysicalExtraRun=Invoke-Ps $register @($baseRegisterArgs + @('-FrameworkVersion','1.14.1','-Apply'))
        ...
        $registrationTraversalRun=Invoke-Ps $register @($baseRegisterArgs + @('-FrameworkVersion','1.14.1','-Apply'))
        ...
        $registrationExtraRun=Invoke-Ps $register @($baseRegisterArgs + @('-FrameworkVersion','1.14.1','-Apply'))
        ...
        $registrationResume=Invoke-Ps $register @($baseRegisterArgs + @('-FrameworkVersion','1.14.1','-Apply'))
        ...
        $registrationUnknown=Invoke-Ps $register @($baseRegisterArgs + @('-FrameworkVersion','1.14.1','-Apply'))
        ...
        [IO.File]::Copy((Join-Path $registrationActive 'new\.ai-workspace\STATUS.md'),$registrationUnknownPath,$true);$registrationCleanup=Invoke-Ps $register @($baseRegisterArgs + @('-FrameworkVersion','1.14.1','-Apply'))
        ...
        $existingMissingController=Invoke-Ps $register @('-ProjectId','explicit-fixture','-DisplayName','Explicit Fixture','-RepositoryPath',$consumer,'-FrameworkVersion','1.14.1')
        ...
        $repeatRegister=Invoke-Ps $register @($baseRegisterArgs + @('-FrameworkVersion','1.14.1'))
        Assert-True ($repeatRegister.Code -eq 0 -and $repeatRegister.Text.Contains('ALREADY_REGISTERED')) 'register-explicit-1.14.1-repeat'
        ...
        $source112Upgrade=Invoke-Ps $upgrade @('-ProjectId','source-112-fixture','-ToVersion','1.14.1','-RepositoryPath',$source112Repo,'-ControllerId','controller-source-112','-Apply')
        ...
        Assert-True ($source112Register.Code-eq0-and$source112Upgrade.Code-eq0-and[int]$source112Projected.schemaVersion-eq4-and[string]$source112Projected.frameworkVersion-ceq'1.14.1'-and[string]$source112Projected.processPolicy.locator-ceq'.ai-workspace/process-policy.json') 'upgrade-healthy-1.12-schema3-to-1.14'
        ...
        $source113Schema3Upgrade=Invoke-Ps $upgrade @('-ProjectId','source-113-schema3-fixture','-ToVersion','1.14.1','-RepositoryPath',$source113Schema3Repo,'-ControllerId','controller-source-113-schema3','-Apply')
        ...
        $source113Schema4Upgrade=Invoke-Ps $upgrade @('-ProjectId','source-113-schema4-fixture','-ToVersion','1.14.1','-RepositoryPath',$source113Schema4Repo,'-ControllerId','controller-source-113-schema4','-Apply')
        ...
        foreach($upgradeAgentsFormatCase in @(
            ...
            $upgradeFormatRun=Invoke-Ps $upgrade @('-ProjectId',('upgrade-agents-'+$upgradeAgentsFormatCase.Name+'-fixture'),'-ToVersion','1.14.1','-RepositoryPath',$upgradeFormatRepo,'-ControllerId','controller-upgrade-agents','-Apply')
        ...
        $downgradeFormatRegister=Invoke-Ps $register @('-ProjectId','downgrade-agents-crlf-fixture','-DisplayName','Downgrade Agents Fixture','-RepositoryPath',$downgradeFormatRepo,'-ControllerId','controller-downgrade-agents','-FrameworkVersion','1.14.1','-Apply')
        ...
        $sameVersionArgs=@('-ProjectId','explicit-fixture','-ToVersion','1.14.1','-RepositoryPath',$consumer,'-ControllerId','controller-explicit')
        $sameVersionPreview=Invoke-Ps $upgrade $sameVersionArgs
        $sameVersionApply=Invoke-Ps $upgrade @($sameVersionArgs+'-Apply')
        Assert-True ($sameVersionPreview.Code-eq0-and$sameVersionPreview.Text.Contains('WHAT_IF|from=1.14.1|to=1.14.1|objects=6')) 'upgrade-schema4-1.14-to-1.14-preview-projects-policy'
        Assert-True ($sameVersionApply.Code-eq0-and$sameVersionApply.Text.Contains('ALREADY_UPGRADED|objects=6')) 'upgrade-schema4-1.14-to-1.14-apply-noop'
        ...
        Assert-True ($schema4DowngradePreview.Code-eq0-and$schema4DowngradePreview.Text.Contains('WHAT_IF|from=1.14.1|to=1.13.0|objects=6')) 'downgrade-1.14-to-1.13-empty-corrections-preview'
        ...
        $schema4DowngradedCorrections=Get-Content -Raw -Encoding utf8 -LiteralPath (Join-Path $consumer '.ai-workspace\corrections.json')|ConvertFrom-Json
        Assert-True ($schema4DowngradeApply.Code-eq0-and[int]$schema4Downgraded.schemaVersion-eq4-and[string]$schema4Downgraded.frameworkVersion-ceq'1.13.0'-and$null-ne$schema4Downgraded.PSObject.Properties['processPolicy']-and[string]$schema4Downgraded.processPolicy.locator-ceq'.ai-workspace/process-policy.json'-and[int]$schema4DowngradedCorrections.schemaVersion-eq1-and[string]$schema4DowngradedCorrections.contractVersion-ceq'1.10.0'-and-not(Test-Path -LiteralPath (Join-Path $consumer '.agents\skills\ai-workspace-router\SKILL.md'))-and-not((Get-Content -Raw -Encoding utf8 -LiteralPath (Join-Path $consumer 'AGENTS.md')).Contains('AI-WORKSPACE-FRAMEWORK:BEGIN'))) 'downgrade-1.14-to-1.13-reverses-empty-v2-and-router-projection'

        $activePolicyRepo=Join-Path $rootFlow 'consumer-active-policy';New-GitRepo $activePolicyRepo
        $activePolicyRegister=Invoke-Ps $register @('-ProjectId','active-policy-fixture','-DisplayName','Active Policy Fixture','-RepositoryPath',$activePolicyRepo,'-ControllerId','controller-active-policy','-FrameworkVersion','1.14.1','-Apply')
        ...
        Assert-True (-not$actorRouteDocs.Contains('FULL_COLD_THEN_UPGRADE')-and-not$actorRouteDocs.Contains('单对象路由迁移并停止')-and$actorRouteDocs.Contains('complete deterministic upgrade write set')-and$actorRouteDocs.Contains('完整升级写集')) 'upgrade-actor-bound-docs-match-combined-forward-transaction'
        ...
        $actor114Register=Invoke-Ps $register @('-ProjectId','actor-route-114-fixture','-DisplayName','Actor Route 1.14 Fixture','-RepositoryPath',$actor114Repo,'-ControllerId','controller-actor-route-114','-FrameworkVersion','1.11.0','-Apply')
        ...
        Write-Utf8 $actor114TaskPath "# ACTOR-ROUTE-114-001 - fixture`n`n- Task schema: 1.11.0`n- Owner: owner-fixture`n- Work route: role=EXECUTOR; phase=IMPLEMENT`n- Range summary: profile=STANDARD; lifecycle=ACTIVE; expected_paths=[]; actual_paths=[]`n"
        $actor114OldTaskIdentity=Get-Identity $actor114TaskPath;$actor114Args=@('-ProjectId','actor-route-114-fixture','-ToVersion','1.14.1','-RepositoryPath',$actor114Repo,'-ControllerId','controller-actor-route-114','-ActorRouteTaskPath',$actor114TaskRelative,'-ExpectedActorRouteTaskIdentity',$actor114OldTaskIdentity,'-ActorRouteActor','executor-current')
        $actor114Preview=Invoke-Ps $upgrade $actor114Args;$actor114WriteLine=@($actor114Preview.Output|Where-Object{$_-clike'UPGRADE_WRITESET|*'})[-1];$actor114WritePaths=@($actor114WriteLine.Split('|')[1..($actor114WriteLine.Split('|').Count-1)]);$actor114PreimageMap=@{}
        ...
        $actor114AuthPath=Join-Path $actor114Repo '.ai-workspace/tmp/actor-bound-114-authorization.json';$actor114Objects=@($actor114WritePaths|ForEach-Object{[ordered]@{path=$_;identity=[string]$actor114PreimageMap[$_]}});$actor114ControllerPath=Join-Path $actor114Repo '.ai-workspace/controller.json'
        $actor114Package=[ordered]@{schemaVersion=1;frameworkVersion='1.11.0';taskId='ACTOR-ROUTE-114-001';profile='STANDARD';lifecycle='ACTIVE';owner='owner-fixture';issuer='controller-actor-route-114';issuerRole='PROJECT_CONTROLLER';grantee='executor-current';bundle='PLAN_LOCAL';decisionClass='MAJOR_ARCHITECTURE';userConfirmation='USER_FIXTURE_ACTOR_BOUND_114_UPGRADE';reviewIndependence='NOT_APPLICABLE';delegatedGitCloser=$false;actions=@('CONTROL_WRITE');exactPaths=$actor114WritePaths;objectIdentities=$actor114Objects;invalidatesOn=@('TASK_CHANGE','OWNER_CHANGE','GRANTEE_CHANGE','ACTION_CHANGE','PATHSET_CHANGE','OBJECT_DRIFT','USER_DECISION_CHANGE','CONTROLLER_EPOCH_CHANGE');issuerControllerId='controller-actor-route-114';issuerControllerEpoch=1;controllerControlIdentity=Get-Identity $actor114ControllerPath}
        Write-Utf8 $actor114AuthPath ($actor114Package|ConvertTo-Json -Depth 20);$actor114AuthIdentity=Get-Identity $actor114AuthPath;$actor114Apply=Invoke-Ps $upgrade @($actor114Args+@('-AuthorizationPackagePath',$actor114AuthPath,'-ExpectedAuthorizationPackageIdentity',$actor114AuthIdentity,'-Apply'))
        $actor114Recovery=Join-Path $actor114Repo '.framework-actor-bound-upgrade-recovery-1.14.1';$actor114State=Get-Content -Raw -Encoding utf8 -LiteralPath (Join-Path $actor114Recovery 'state.json')|ConvertFrom-Json
        ...
        Assert-True ($actor114Register.Code-eq0-and$actor114Preview.Code-eq0-and$actor114Apply.Code-eq0-and$actor114DriftRun.Code-ne0-and$actor114DriftRun.Text.Contains('MANAGED_ROUTER_DESTINATION_REPARSE')-and(Get-Identity (Join-Path $actor114Repo '.ai-workspace/project.json'))-ceq$actor114ConfigPreimage-and(Get-Identity $actor114TaskPath)-ceq$actor114OldTaskIdentity-and-not(Test-Path -LiteralPath (Join-Path $actor114External 'skills\ai-workspace-router\SKILL.md'))-and(Test-Path -LiteralPath $actor114Recovery -PathType Container)) 'upgrade-actor-bound-router-reparse-drift-retains-recovery-zero-external-write'
        ...
        $unsupportedToolchain=$fixtureToolchainOriginal|ConvertFrom-Json;$unsupportedToolchain.officialBackends[0].platforms=@('linux');Write-Utf8 $fixtureToolchainPath ($unsupportedToolchain|ConvertTo-Json -Depth 20);$null=Seal-ReleaseFixture $fixtureVersionRoot 'UNSUPPORTED_PLATFORM_TEST_FIXTURE'
        $unsupportedUpgrade=Invoke-Ps $upgrade @('-ProjectId','upgrade-fixture','-ToVersion','1.14.1','-RepositoryPath',$upgradeRepo,'-ControllerId','controller-upgrade','-Apply')
        ...
        $upgradeArgs=@('-ProjectId','upgrade-fixture','-ToVersion','1.14.1','-RepositoryPath',$upgradeRepo,'-ControllerId','controller-upgrade')
        ...
        $conflictEntry=@($conflictCoverage.versions|Where-Object{[string]$_.version-ceq'1.14.1'})[0]
        ...
        Assert-True ($upgradePreview.Code -eq 0 -and $upgradePreview.Text.Contains('WHAT_IF|from=1.11.0|to=1.14.1|objects=6') -and $upgradePreview.Text.Contains('incorporated=0') -and $upgradePreview.Text.Contains('still-effective=3') -and $upgradePreview.Text.Contains('conflicts=0')) 'upgrade-1.11-to-1.14-preview-retains-unmapped-corrections'
        $upgradeObjectRelatives=@('.ai-workspace/project.json','.ai-workspace/BOOTSTRAP.md','.ai-workspace/corrections.json','.ai-workspace/process-policy.json','AGENTS.md','.agents/skills/ai-workspace-router/SKILL.md');$upgradeStartCollisionPath=Join-Path $upgradeRepo '.framework-1.14-upgrade-recovery-1.14.1';New-Item -ItemType Directory -Path $upgradeStartCollisionPath -Force|Out-Null;$upgradeStartCollisionMarker=Join-Path $upgradeStartCollisionPath 'project-owned.txt';Write-Utf8 $upgradeStartCollisionMarker 'preexisting recovery name'
        ...
        Assert-True ($downgradePreview.Code-eq0-and$downgradePreview.Text.Contains('WHAT_IF|from=1.14.1|to=1.13.0|objects=6')-and$downgradePreview.Text.Contains('STILL_EFFECTIVE PROJECT_CORRECTION_LIFECYCLE')-and$downgradePreview.Text.Contains('STILL_EFFECTIVE OWNER_FIRST_DIRECT_DOMAIN_ROUTE')) 'downgrade-1.14-to-1.13-re-evaluates-and-retains-v1-corrections'
        ...
        $downgradeCorrectionLocatorPresent=$downgradedBootstrap.Contains('framework/versions/1.14.1/scripts/check-project-corrections.ps1')
    ...
    $knowledgeConfig=[ordered]@{schemaVersion=4;id='knowledge-fixture';displayName='Knowledge Fixture';controlPlaneLayout='repo-local';repositoryRoot='..';frameworkVersion='1.14.1';frameworkToolBackend='powershell7';routineExcludedPaths=@();frameworkCapabilities=[ordered]@{KNOWLEDGE_REFERENCE=[ordered]@{enabled=$true;indexLocator='knowledge/index.json'}};processPolicy=[ordered]@{schemaVersion=1;locator='.ai-workspace/process-policy.json'}}
    ...
    if($null-ne$pwsh){
        ...
        Write-Utf8 $correctionPath $correctionDuplicate
        try{$ps7CorrectionDuplicate=Invoke-PsHost $pwsh.Source $correctionChecker @('-ProjectRoot',$correctionRoot,'-FrameworkRoot',$correctionFrameworkRoot,'-TargetVersion','1.14.1','-ExpectedProjectConfigIdentity',(Get-Identity $correctionConfigPath),'-ExpectedCorrectionsIdentity',(Get-Identity $correctionPath),'-Operation','PRECHECK','-AsJson')}
    ...
    if (-not $SkipBaseline) {
        ...
        $stableBaselineRoot=Join-Path $frameworkRoot 'versions\1.14.0'
        ...
        Assert-True ([string]$stableBaselineManifest.lifecycle-ceq'STABLE'-and[int]$stableBaselineManifest.fileCount-eq$stableBaselineFacts.FileCount-and[int64]$stableBaselineManifest.totalBytes-eq$stableBaselineFacts.TotalBytes-and[string]$stableBaselineManifest.canonical-ceq$stableBaselineFacts.Canonical-and[string]$stableBaselineManifest.canonical-ceq'D0E2AA2294150E6D3A23233EBA62592DDD72070E6E0DC19B8D89E684C65CAD57') 'baseline-1.14.0-immutable-stable-seal'
    }
...
$resultScope=if($resultSealed){'Framework-1.14.1-stable'}else{'Framework-1.14.1-candidate'}
