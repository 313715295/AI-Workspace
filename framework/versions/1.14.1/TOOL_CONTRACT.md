...
`PROCESS_REQUIREMENTS.json` is a sealed internal operation input, not a model-facing load-plan module. The host reads the selected complete rules returned by `DISCOVER`; it does not preload the complete catalog. Role, phase, recovery, host and topology modules returned by `LOAD_PLAN_RESOLVE` remain authoritative and are measured separately from project-owned task/control facts.
...
The official `powershell7` backend is invoked with `pwsh -NoProfile -NonInteractive -File <entrypoint> ...`. Each 1.14.1 backend entrypoint independently rejects a non-Core runtime or PowerShell major version below 7.
...
The contract permits later platform admission, but each released toolchain lists only platforms it actually supports. 1.14.1 lists Windows only. Platform-specific evidence requirements remain explicit:
