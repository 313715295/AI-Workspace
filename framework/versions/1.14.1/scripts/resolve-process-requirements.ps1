...
function Get-Receipt([string]$Path){
  ...
  if(-not(Test-JsonInteger $r.schemaVersion)-or[int]$r.schemaVersion-ne1-or-not($r.authorityGranted-is[bool])-or-not($r.semanticCorrectnessProven-is[bool])-or[string]$r.receiptType-cne'PROCESS_REQUIREMENTS_DISCOVER'-or[string]$r.mode-cne'DISCOVER'-or[string]$r.status-cnotin@('PASS','EVALUATION_ONLY')-or[string]$r.frameworkVersion-cne'1.14.1'-or[bool]$r.authorityGranted-or[bool]$r.semanticCorrectnessProven){throw 'DISCOVER_RECEIPT_TYPE'}
...
try{
  ...
  if($mode-ceq'DISCOVER'){
    ...
    $composition=Invoke-ProcessRequirementComposition -ProjectRoot ([string]$input.projectRoot) -FrameworkRoot ([string]$input.frameworkRoot) -TargetVersion '1.14.1' -ExpectedProjectConfigIdentity ([string]$input.expectedProjectConfigIdentity) -ExpectedCorrectionsIdentity ([string]$input.expectedCorrectionsIdentity) -Profile ([string]$profileMatch[0].Groups['profile'].Value) -Role ([string]$route[0].Groups['role'].Value) -Phase ([string]$route[0].Groups['phase'].Value) -Actor $actor -TaskIdentity $taskIdentity -Capabilities @($input.capabilities) -Objective $semanticObjective -ActionKind ([string]$input.actionKind) -ResultKind ([string]$input.resultKind) -ExactPaths @($input.exactPaths) -EvaluationOnly:([bool]$input.evaluationOnly)
    ...
    $currentBindings=Get-AiwProcessBindingSnapshot -ProjectRoot ([string]$receipt.sourceLocators.projectRoot) -FrameworkRoot ([string]$receipt.sourceLocators.frameworkRoot) -TargetVersion '1.14.1' -TaskRelativePath ([string]$receipt.sourceLocators.taskRelativePath)
