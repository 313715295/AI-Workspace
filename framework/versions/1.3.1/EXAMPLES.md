...
若实际挂载执行者和审核者，才在卡顶同时增加角色字段，并给`STANDARD`范围摘要追加actor/notifier尾段。没有多角色时不要填`OWNER / NONE`占位来模拟闭环。
...
- 执行载体与资源：独立任务会话；需要跨turn实现、返工与稳定审计对象
...
- 审核人：未参与方案和写入的认证审核者
- 直接审核闭环：允许exact内最多两轮窄返工；公共合同、权限、范围或同一发现复发立即回owner
- Git 收口者：认证领域owner
- Git 权限：独立APPROVED后允许owner逐项本地提交；push关闭
- 外部动作授权：部署、密钥轮换和发布均未授权
- 回传对象与机制：完成、阻断、范围门和最终结论回唯一认证领域owner
...
- 范围摘要：`profile=CRITICAL; lifecycle=ACTIVE_WRITE; current_actor=EXECUTOR; next_actor=EXECUTOR; current_exact=4; expected_paths=[docs/auth-contract.md|src/auth/token-store.js|src/auth/token-service.js|test/auth/token-lifecycle.test.js]; actual_paths=[]; review_start_notifier=EXECUTOR`
- 创建/更新：2026-01-15

## 目标

把令牌从无版本持久记录迁移为版本化记录，冻结创建、读取、刷新、撤销、过期和失败恢复合同，并保持旧记录只读迁移能力。
...
唯一下一动作：执行者在exact内形成候选和证据，通知owner Review开始后直交指定审核者并立即停写。
```
