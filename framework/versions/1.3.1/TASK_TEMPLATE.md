...
`STANDARD`默认不声明actor/notifier/direct loop。只有实际挂载执行者与审核者时，才同时增加下列字段，并把范围摘要替换为带角色尾段的格式：
...
- Review开始通知方：EXECUTOR / REVIEWER / NONE
- 审核人：<未参与实质写入的合格角色>
- 直接审核闭环：关闭 / <指定审核者、消息机制、允许返工、轮次、停止线和最终owner回传>
- Git 收口者：<会话或角色>
- Git 权限：关闭 / <条件预授权及精确门>
- 外部动作授权：无 / <逐项动作、范围与批准者>
- 回传对象与机制：<唯一owner；完成/阻断/新授权的消息机制>
- Exact路径：[<workspace/relative/a>|<workspace/relative/b>]
- Forbidden路径：[<workspace/relative/protected>]
- 范围摘要：`profile=CRITICAL; lifecycle=ACTIVE_WRITE; current_actor=<...>; next_actor=<...>; current_exact=2; expected_paths=[<workspace/relative/a>|<workspace/relative/b>]; actual_paths=[]; review_start_notifier=<...>`
- 创建/更新：YYYY-MM-DD
...
进入Review前冻结稳定内容对象/section identity、candidate与依赖manifest、验证上限和任务卡可变区；direct loop逐轮记录delta、受影响层、停止线和最终owner回传。
...
有持久卡时由AI调用项目固定版本的`scripts/check-task-card.ps1`。进入Review、Git收口或关闭前，从git status/diff或冻结manifest独立得到本任务actual，并以`-ObservedActualPath`逐卡传入。checker只核对UTF-8/结构、profile字段、范围摘要、expected/actual、观察路径、生命周期和已声明actor/notifier的一致性；它不扫描Git归属、不识别owner、不判断设计或风险选择，也不授予审核、Git、阶段或外部权限。
