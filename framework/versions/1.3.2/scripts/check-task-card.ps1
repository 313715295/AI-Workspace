...
function Test-TaskCard {
    ...
    $criticalSummary = [regex]::Match(
        $summaryText,
        '^profile=CRITICAL; lifecycle=(?<lifecycle>ACTIVE_WRITE|ACTIVE|REVIEW|CLOSED); current_exact=(?<exact>[^;]+); expected_paths=\[(?<expected>[^\]]*)\]; actual_paths=\[(?<actual>[^\]]*)\](?:; current_actor=(?<current>[A-Z][A-Z0-9_]*); next_actor=(?<next>[A-Z][A-Z0-9_]*); review_start_notifier=(?<notifier>[A-Z][A-Z0-9_]*))?$'
    )
    $criticalHostLegacySummary = [regex]::Match(
        $summaryText,
    ...
    elseif ($criticalHostLegacySummary.Success) {
        $summary = $criticalHostLegacySummary
        $profile = 'CRITICAL'
    }
    ...
    if ($null -ne $summary) {
        ...
        if ($profile -eq 'CRITICAL') {
            ...
            $roleLabels = [regex]::Matches(
                $text,
                '(?m)^-\s*(?:\u5f53\u524dactor|\u552f\u4e00\u4e0b\u4e00\u52a8\u4f5c\u6267\u884c\u8005|Review\u5f00\u59cb\u901a\u77e5\u65b9|\u76f4\u63a5\u5ba1\u6838\u95ed\u73af|\u56de\u4f20\u5bf9\u8c61\u4e0e\u673a\u5236)\s*\uFF1A'
            )
            $hasRoleSummary = $summary.Groups['current'].Success
            if (($roleLabels.Count -gt 0) -ne $hasRoleSummary) {
                Add-Reason $reasons 'CRITICAL_HOST_ROLE_BLOCK_SUMMARY_MISMATCH'
            }
            if ($hasRoleSummary) {
                $currentActorField = Get-LogicalField $text '\u5f53\u524dactor' 'CURRENT_ACTOR' $reasons
                $nextActorField = Get-LogicalField $text '\u552f\u4e00\u4e0b\u4e00\u52a8\u4f5c\u6267\u884c\u8005' 'NEXT_ACTOR' $reasons
