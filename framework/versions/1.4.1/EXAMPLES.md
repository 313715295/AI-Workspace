...
```markdown
# CONTROL-PLANE-UPGRADE-001 — 升级repo-local Framework pin

- 状态：ACTIVE_WRITE
- 任务档位：STANDARD
- 唯一owner：项目控制面owner
- 项目Git仓：项目Git顶层；`.ai-workspace`
- 当前Framework：1.4.1
- 目标Framework：1.4.2；目标starter仍为repo-local
- 修改对象：`project.json` pin与`BOOTSTRAP.md`唯一受管区
- 自定义区：逐字保留；受管区被改写时停止
- Git权限：关闭；不stage/commit
- Exact路径：[.ai-workspace/project.json|.ai-workspace/BOOTSTRAP.md]
- Forbidden路径：[产品源码|.git/index/ref|中央legacy目录]

## 验收

项目仓必须干净；目标版本完整存在且布局仍为repo-local。升级后pin正确、受管区来自目标starter、自定义区逐字一致；中断恢复沿用同仓两文件事务，未知恢复材料保留并停止。

## 交接/下一步

唯一下一动作：项目owner独立审核升级差异并决定是否提交；跨布局要求必须返回owner另建项目专属任务。
```
