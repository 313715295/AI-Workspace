...
$baselineRoot = Join-Path $frameworkRoot 'versions\1.5.1'
...
Assert-Hotfix ([string]$metadata.version -ceq '1.5.2' -and [string]$metadata.lifecycle -ceq 'STABLE' -and [string]$metadata.releaseClass -ceq 'PATCH_HOTFIX' -and
    [string]$metadata.baseline -ceq '1.5.1' -and [bool]$metadata.consumable -and [bool]$metadata.projectPinEligible) 'metadata-patch-stable'
...
$draftInventory = Get-RelativeInventory $versionRoot @('RELEASE_MANIFEST.json')
...
Assert-Hotfix ($corePlan.frameworkVersion -ceq '1.5.2' -and $corePlan.lifecycle -ceq 'STABLE' -and @($corePlan.modules).Count -eq 1 -and $corePlan.modules[0].path -ceq 'RECOVERY_CORE.md') 'core-only-load-plan'
...
$tempRoot = Join-Path ([IO.Path]::GetTempPath()) ('framework-hotfix-1.5.2-' + [Guid]::NewGuid().ToString('N'))
...
try {
    ...
    Write-Utf8NoBom $standardPath "# STANDARD-151 - no phase tax`n`n- Task schema: 1.5.2`n- Owner: owner-1`n- Range summary: profile=STANDARD; lifecycle=ACTIVE; expected_paths=[]; actual_paths=[]`n"
    ...
    Write-Utf8NoBom $phaseFalsePath "# CRITICAL-FALSE - no phase transition`n`n- Task schema: 1.5.2`n- Owner: controller-1`n- Range summary: profile=CRITICAL; lifecycle=ACTIVE; current_exact=candidate-1; expected_paths=[]; actual_paths=[]`n- Phase gate: FALSE`n"
...
"@
    ...
    $sha = ('A' * 64)
    $package = [ordered]@{
        schemaVersion=1; frameworkVersion='1.5.2'; taskId='FW-HOTFIX-AUTH'; profile='STANDARD'; lifecycle='ACTIVE';
    ...
    Assert-Hotfix ($LASTEXITCODE -eq 0 -and $authPass -like 'PASS|*') 'authorization-1.5.2-valid'

    $package.actions = @('GIT_COMMIT')
    $package.delegatedGitCloser = 'false'
    Write-Utf8NoBom $authPath ($package | ConvertTo-Json -Depth 8)
    $stringFalse = & $authChecker -PackagePath $authPath -ObservedActor executor-1 -ObservedTaskId FW-HOTFIX-AUTH -ObservedOwner owner-1 -ObservedAction GIT_COMMIT -ObservedPath src/a.js -ObservedIdentity "src/a.js=10|$sha" 2>$null
    Assert-Hotfix ($LASTEXITCODE -eq 2 -and $stringFalse -like '*FIELD_TYPE_delegatedGitCloser_BOOLEAN*') 'authorization-rejects-string-false-git-delegation'

    $package.delegatedGitCloser = $false
    $package.actions = 'SOURCE_WRITE'
    Write-Utf8NoBom $authPath ($package | ConvertTo-Json -Depth 8)
    $scalarActions = & $authChecker -PackagePath $authPath -ObservedActor executor-1 -ObservedTaskId FW-HOTFIX-AUTH -ObservedOwner owner-1 -ObservedAction SOURCE_WRITE -ObservedPath src/a.js -ObservedIdentity "src/a.js=10|$sha" 2>$null
    Assert-Hotfix ($LASTEXITCODE -eq 2 -and $scalarActions -like '*FIELD_TYPE_actions_ARRAY*') 'authorization-rejects-scalar-actions'

    $package.schemaVersion = '1'
    $package.actions = @('SOURCE_WRITE')
    Write-Utf8NoBom $authPath ($package | ConvertTo-Json -Depth 8)
    $stringSchema = & $authChecker -PackagePath $authPath -ObservedActor executor-1 -ObservedTaskId FW-HOTFIX-AUTH -ObservedOwner owner-1 -ObservedAction SOURCE_WRITE -ObservedPath src/a.js -ObservedIdentity "src/a.js=10|$sha" 2>$null
    Assert-Hotfix ($LASTEXITCODE -eq 2 -and $stringSchema -like '*FIELD_TYPE_schemaVersion_INTEGER*') 'authorization-rejects-string-schema-version'
    ...
    Write-Utf8NoBom $authPath ($package | ConvertTo-Json -Depth 8)
    $mixedReview = & $authChecker -PackagePath $authPath -ObservedActor executor-1 -ObservedTaskId FW-HOTFIX-AUTH -ObservedOwner owner-1 -ObservedAction REVIEW_EXECUTE -ObservedPath src/a.js -ObservedIdentity "src/a.js=10|$sha" 2>$null
    Assert-Hotfix ($LASTEXITCODE -eq 2 -and $mixedReview -like '*REVIEW_WRITE_ACTION_CONFLICT*') 'authorization-rejects-review-plus-candidate-write'

    $package.actions = @('REVIEW_EXECUTE')
    Write-Utf8NoBom $authPath ($package | ConvertTo-Json -Depth 8)
    $pureReview = & $authChecker -PackagePath $authPath -ObservedActor executor-1 -ObservedTaskId FW-HOTFIX-AUTH -ObservedOwner owner-1 -ObservedAction REVIEW_EXECUTE -ObservedPath src/a.js -ObservedIdentity "src/a.js=10|$sha"
    Assert-Hotfix ($LASTEXITCODE -eq 0 -and $pureReview -like 'PASS|*') 'authorization-allows-pure-independent-review'

    $package.profile = 'STANDARD'
    $package.actions = @('GIT_COMMIT')
    $package.reviewIndependence = 'NOT_APPLICABLE'
    $package.delegatedGitCloser = $true
    Write-Utf8NoBom $authPath ($package | ConvertTo-Json -Depth 8)
    $delegatedGit = & $authChecker -PackagePath $authPath -ObservedActor executor-1 -ObservedTaskId FW-HOTFIX-AUTH -ObservedOwner owner-1 -ObservedAction GIT_COMMIT -ObservedPath src/a.js -ObservedIdentity "src/a.js=10|$sha"
    Assert-Hotfix ($LASTEXITCODE -eq 0 -and $delegatedGit -like 'PASS|*') 'authorization-allows-real-boolean-git-delegation'
    ...
    Copy-Item -LiteralPath $baselineRoot -Destination (Join-Path $isolatedWorkspace 'framework\versions\1.5.1') -Recurse
    ...
    $register = & powershell.exe -NoProfile -ExecutionPolicy Bypass -File (Join-Path $isolatedWorkspace 'scripts\register-project.ps1') -ProjectId hotfix-smoke -DisplayName 'Hotfix Smoke' -RepositoryPath $projectRepo -FrameworkVersion 1.5.1 -WorkspaceRoot $isolatedWorkspace -Apply 2>&1
    ...
    $upgrade = & powershell.exe -NoProfile -ExecutionPolicy Bypass -File (Join-Path $isolatedWorkspace 'scripts\upgrade-project.ps1') -ProjectId hotfix-smoke -RepositoryPath $projectRepo -ToVersion 1.5.2 -WorkspaceRoot $isolatedWorkspace -Apply 2>&1
    ...
    Assert-Hotfix ($upgradeCode -eq 0 -and $null -ne $config -and [string]$config.frameworkVersion -ceq '1.5.2' -and $headAfter -ceq $headBefore -and $cached.Count -eq 0) 'upgrade-1.5.1-to-1.5.2-smoke'
} finally {
