...
6. Load every exact complete Markdown rule block returned once by `DISCOVER`; persist only its compact receipt for later boundary checks, then obtain any rule-required facts, evidence, schemas, templates or other action artifacts.
...
Run `ADMIT_ACTION` at each distinct governed action boundary and `FINALIZE_OUTPUT` immediately before a final user or consumer result. Both consume the exact compact `DISCOVER` receipt, never a second copy of the full rule blocks. Delete that receipt after finalization, invalidation or abort. UNKNOWN semantic applicability conservatively selects affected blocks. Missing or conflicting mapping first loads every mapped block in the affected module; only an incomplete module mapping falls back to that complete module, never the whole Framework.
