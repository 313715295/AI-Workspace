...
Knowledge 使用分两步。`DISCOVER` 校验 index，只返回 compact metadata：ID、title、可选 tags、lifecycle、reference locator 与声明的 authority locators；不得返回 summary 或 reference body。`RECOVER`/`PLAN` 随后按当前请求选择有界的显式 ID 集合；任务或 upstream 已声明的 ID list 只是 deterministic shortcut。`QUERY` 对这些精确选择逐项校验 reference 与 authority identities，并返回明确的 `AVAILABLE` 或 `UNAVAILABLE`。unknown、duplicate 或越出请求范围的选择必须 fail closed；无关 stale entry 不阻塞新的 requested entry。
...
discovery 只返回 metadata，query 只读取当前请求选择的显式 ID，不设统一数量配额。不增加 tokenizer accounting、summary budget service、semantic search、ranking cache、background indexing/refresh、common project registry、usage ledger 或 authority elevation。
