...
function Get-SafeCleanupInput([string]$Path){
  ...
  if(-not$full.StartsWith($temp+[IO.Path]::DirectorySeparatorChar,[StringComparison]::OrdinalIgnoreCase)-or[IO.Path]::GetExtension($full)-cne'.json'-or-not[IO.Path]::GetFileName($full).StartsWith('aiw-',[StringComparison]::Ordinal)){throw 'INPUT_CLEANUP_PATH_UNSAFE'}
...
function Get-AdoptionBudgets {
  $path=Join-Path (Split-Path -Parent $PSScriptRoot) 'ADOPTION_PROFILE.json';$raw=Read-Input $path
  Assert-Fields $raw @('schemaVersion','frameworkVersion','registrationEligible','directSourceVersions','projectControl','processBudgets')
  Assert-Fields $raw.projectControl @('schemaVersion','processCarrierContractVersion','frameworkToolBackend','navigationProjection','taskLastWriteRequired','capabilityBinding')
  Assert-Fields $raw.processBudgets @('ordinarySelectedPackBytes','absoluteSelectedPackBytes','legacySchema1CorrectionCompatibilityBytes')
  if(-not(Test-JsonInteger $raw.schemaVersion)-or[int]$raw.schemaVersion-ne1-or[string]$raw.frameworkVersion-cne'1.16.0'-or-not($raw.registrationEligible-is[bool])-or-not[bool]$raw.registrationEligible-or-not($raw.directSourceVersions-is[Array])-or[string]::Join('|',@($raw.directSourceVersions))-cne'1.14.1|1.15.0|1.15.1'-or-not(Test-JsonInteger $raw.processBudgets.ordinarySelectedPackBytes)-or[int]$raw.processBudgets.ordinarySelectedPackBytes-ne32768-or-not(Test-JsonInteger $raw.processBudgets.absoluteSelectedPackBytes)-or[int]$raw.processBudgets.absoluteSelectedPackBytes-ne65536-or-not(Test-JsonInteger $raw.processBudgets.legacySchema1CorrectionCompatibilityBytes)-or[int]$raw.processBudgets.legacySchema1CorrectionCompatibilityBytes-ne98304){throw 'ADOPTION_PROFILE_VALUES'}
  return $raw.processBudgets
}
...
function Get-Receipt([string]$Path){
  ...
  foreach($name in @('sourceCompositionIdentity','selectionIdentity','contextIdentity','projectId','taskIdentity','taskId','taskOwner','actor','role','phase','profile','objective','actionKind','resultKind','hostEnforcementGrade','invocationState')){Assert-InputString $r.$name $name}
...
$cleanupInput=$null
if($DeleteInputOnExit){try{$cleanupInput=Get-SafeCleanupInput $InputPath}catch{if($AsJson){[ordered]@{status='FAIL';reason=[string]$_.Exception.Message}|ConvertTo-Json -Compress}else{Write-Output ('FAIL|'+[string]$_.Exception.Message)};exit 2}}
try{
  ...
  if($mode-ceq'DISCOVER'){
    ...
    $composition=Invoke-ProcessRequirementComposition -ProjectRoot ([string]$input.projectRoot) -FrameworkRoot ([string]$input.frameworkRoot) -TargetVersion '1.16.0' -ExpectedProjectConfigIdentity ([string]$input.expectedProjectConfigIdentity) -ExpectedCorrectionsIdentity ([string]$input.expectedCorrectionsIdentity) -Profile ([string]$profileMatch[0].Groups['profile'].Value) -Role ([string]$route[0].Groups['role'].Value) -Phase ([string]$route[0].Groups['phase'].Value) -Actor $actor -TaskIdentity $taskIdentity -Capabilities @($input.capabilities) -Objective $semanticObjective -ActionKind ([string]$input.actionKind) -ResultKind ([string]$input.resultKind) -ExactPaths @($input.exactPaths) -SemanticApplicabilityUnknown:([string]$intent.ambiguityState-cne'CLEAR') -EvaluationOnly:([bool]$input.evaluationOnly)
    ...
    $budgets=Get-AdoptionBudgets
    $packJson=@($composition.selectedRequirements)|ConvertTo-Json -Depth 50 -Compress;$packBytes=$utf8.GetByteCount($packJson)
    $legacyCompatibilityEligible=[int]$composition.legacyCorrectionsFullReadCount-eq1-and[int]$composition.legacyProjectCustomFullReadCount-eq0-and[string]$intent.ambiguityState-ceq'CLEAR'-and@($composition.evidenceCeilings|Where-Object{[string]$_-ceq'SOURCE_RECORD_IDENTITY_MISMATCH_RETAINED'}).Count-eq0
    if($packBytes-gt[int]$budgets.ordinarySelectedPackBytes-and-not$legacyCompatibilityEligible){throw ('SELECTED_RULE_PACK_BUDGET_EXCEEDED|bytes='+$packBytes+'|ceiling='+[int]$budgets.ordinarySelectedPackBytes)}
    if($packBytes-gt[int]$budgets.legacySchema1CorrectionCompatibilityBytes){throw ('SELECTED_RULE_PACK_BUDGET_EXCEEDED|bytes='+$packBytes+'|ceiling='+[int]$budgets.legacySchema1CorrectionCompatibilityBytes)}
    ...
    $ceilings=@($composition.evidenceCeilings);if([string]$input.hostEnforcementGrade-ceq'INSTRUCTION_BOUND'-or$invocationState-ceq'UNPROVEN'){$ceilings+='INVOCATION_UNPROVEN'};if([bool]$input.evaluationOnly){$ceilings+='NON_AUTHORITY_EVALUATION_ONLY'};if([string]$intent.ambiguityState-cne'CLEAR'){$ceilings+='INTENT_AMBIGUITY_CONSERVATIVE_LOAD'};if($intentFactMismatches.Count-gt0){$ceilings+='INTENT_FACT_MISMATCH_CONSERVATIVE_LOAD'};if($authority.repositoryGitTop-ceq'UNPROVEN'){$ceilings+='REPOSITORY_GIT_TOP_UNPROVEN'};if($packBytes-gt[int]$budgets.ordinarySelectedPackBytes-and$legacyCompatibilityEligible){$ceilings+=('LEGACY_SCHEMA1_CORRECTION_COMPATIBILITY|bytes='+$packBytes+'|ceiling='+[int]$budgets.legacySchema1CorrectionCompatibilityBytes)}
    ...
    $result=[ordered]@{status=$status;mode=$mode;reason=$reason;sourceCompositionIdentity=$receipt.sourceCompositionIdentity;selectionIdentity=$receipt.selectionIdentity;decisionIdentity=$decision;selectedRequirementIds=@($receipt.selectedObligations.requirementId);missingPreparation=@($missingPrep);missingResult=@($missingResult);sourceBuildCount=$receipt.sourceBuildCount;decisionBuildCount=1;authorityGranted=$false;semanticCorrectnessProven=$false;hostInvocationProven=$false}
  }else{throw 'INPUT_MODE'}
  if($AsJson){$result|ConvertTo-Json -Depth 50 -Compress}else{$count=if($mode-ceq'DISCOVER'){@($result.selectedRuleBlocks).Count}else{@($result.selectedRequirementIds).Count};Write-Output ($result.status+'|'+$mode+'|requirements='+$count)}
