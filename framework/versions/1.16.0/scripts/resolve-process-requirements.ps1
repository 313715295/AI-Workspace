...
function Get-AuthorizationObservation([string]$Path,[string]$ExpectedIdentity,[string]$Actor,[string[]]$ExactPaths,[string]$UserDecision,[string]$ActionKind,[string]$ProjectRoot,[string]$TaskRelativePath,[string]$TaskIdentity,[string]$TaskId,[string]$TaskOwner,[string]$ProjectConfigIdentity,[string]$ContinuationReceiptPath='NOT_REQUIRED',[string]$ExpectedContinuationReceiptIdentity='NOT_REQUIRED'){
  if([string]$Path-ceq'NOT_REQUIRED'){
    ...
    return [pscustomobject]@{Actions=@();Identity='NOT_REQUIRED';UserDecision=$UserDecision;RepositoryId='CONTROL';RepositoryRoot=$controlRoot;HasContinuationPlan=$false;ContinuationReceiptPath='NOT_REQUIRED';ContinuationReceiptIdentity='NOT_REQUIRED';ContinuationStepIndex=-1}
  }
  ...
  return [pscustomobject]@{Actions=@($package.actions);Identity=$identity;UserDecision=[string]$package.userConfirmation;RepositoryId=$repositoryId;RepositoryRoot=$repositoryRoot;HasContinuationPlan=$hasContinuationPlan;ContinuationReceiptPath=$ContinuationReceiptPath;ContinuationReceiptIdentity=$ExpectedContinuationReceiptIdentity;ContinuationStepIndex=$continuationStep}
}
