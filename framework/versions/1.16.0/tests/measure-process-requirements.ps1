...
try{
  ...
  foreach($fixture in @($fixtureContract.fixtures)){
    ...
    Write-Utf8 (Join-Path $projectRoot '.ai-workspace\process-policy.json') (([ordered]@{schemaVersion=1;contractVersion='1.16.0';projectId=$project.id;selectedRulePackBytes=$fixturePackCeiling;rules=@()})|ConvertTo-Json -Depth 20)
    ...
    $admit=[ordered]@{schemaVersion=2;mode='ADMIT_ACTION';discoverReceiptPath=$receiptPath;expectedDiscoverReceiptIdentity=(Get-Identity $receiptPath);preparationReceipts=@($requiredPrep);resultReceipts=@();deliveryReceipts=@();publicDecisionIdentity='NOT_REQUIRED';protectionState='BOUND'}
    ...
    $results+=[ordered]@{fixtureId=[string]$fixture.fixtureId;fixtureIdentity=[string]$fixture.fixtureIdentity;selectedPackBytes=[int]$receipt.pack.bytes;selectedPackEstimatedTokens=[int]$receipt.pack.estimatedTokens;selectedRequirementCount=@($receipt.selectedObligations).Count;modes=$modeResults}
  }
