...
try{
  ...
  $continuationTestFinalize=$continuationTestBoundary|ConvertTo-Json -Depth 50|ConvertFrom-Json;$continuationTestFinalize.mode='FINALIZE_OUTPUT';$continuationTestFinalize.resultReceipts=@($continuationTestResults);$continuationTestFinalize.resultReceipts+=('TEST_PROCESS_EXIT|'+$continuationFirstCheckExit);$continuationTestFinalize.resultReceipts+=('OBJECT_POSTIMAGE|src/file.txt|'+(Get-Identity $continuationFixturePath));Write-Json $boundaryPath $continuationTestFinalize;$continuationTestFinalizeRun=Invoke-Resolver $resolver $boundaryPath;Assert-ResolverSuccess $continuationTestFinalizeRun 'continuation-test-finalize'
  ...
  Write-Utf8 $taskPath "# PROCESS-V2-001 — runtime contract fixture`n`n- Task schema: 1.16.0`n- Owner: owner-fixture`n- Work route: actor=owner-fixture; role=DOMAIN_OWNER; phase=IMPLEMENT`n- Range summary: profile=STANDARD; lifecycle=ACTIVE; expected_paths=[$taskRelative]; actual_paths=[$taskRelative]`n"
  $taskPackage=[ordered]@{schemaVersion=1;frameworkVersion='1.16.0';taskId='PROCESS-V2-001';profile='STANDARD';lifecycle='ACTIVE';owner='owner-fixture';issuer='owner-fixture';issuerRole='DOMAIN_OWNER';grantee='owner-fixture';bundle='PLAN_LOCAL';decisionClass='ROUTINE_LOCAL';userConfirmation='NOT_REQUIRED';reviewIndependence='NOT_APPLICABLE';delegatedGitCloser=$false;taskIdentity=Get-Identity $taskPath;actions=@('CONTROL_WRITE');exactPaths=@($taskRelative);objectIdentities=@([ordered]@{path=$taskRelative;identity=Get-Identity $taskPath});invalidatesOn=@('TASK_CHANGE','OWNER_CHANGE','GRANTEE_CHANGE','ACTION_CHANGE','PATHSET_CHANGE','OBJECT_DRIFT','USER_DECISION_CHANGE','PROJECT_CONFIG_DRIFT');projectConfigIdentity=Get-Identity (Join-Path $control 'project.json')}
  ...
  $taskBindingDrifts=@(
    ...
    [pscustomobject]@{Name='role';Text=$taskPostText-replace 'role=DOMAIN_OWNER','role=EXECUTOR'},
  ...
  $successorTaskText=$taskPostText-replace 'actor=owner-fixture; role=DOMAIN_OWNER','actor=successor-fixture; role=EXECUTOR';Write-Utf8 $taskPath $successorTaskText
