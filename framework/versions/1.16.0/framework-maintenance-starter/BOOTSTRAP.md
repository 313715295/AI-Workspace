...
1. Lightly read Maintenance `STATUS.md` and `tasks/README.md` only to locate the assigned task, then bind its current header, authenticated actor, `role + phase`, profile, objective/action/result, selected repository, exact scope and protection boundary.
2. Invoke `PROCESS_REQUIREMENTS_RESOLVE/DISCOVER` over the complete sealed metadata catalog plus current corrections and policy before loading normative modules. Read every exact complete Markdown block returned once, and retain only the separate compact receipt for later boundary checks.
3. Read only additional Maintenance facts, target evidence, schemas, templates or action artifacts required by the selected rules and task. Explicitly read `<TARGET>/AGENTS.md` when target work is in scope; sibling instructions are not inherited automatically.
4. Use `LOAD_PLAN_RESOLVE` only for 1.14 compatibility, non-rule support, Framework-wide explanation/maintenance or bounded affected-module fallback. It is not a pre-DISCOVER exclusion gate.
5. Reprove control and target HEAD/index/dirty separately. Use the maintenance-aware safe-Git entry with frozen config identity, one repository ID and bounded literal paths. One repository's VERIFIED result cannot fill the other repository's UNVERIFIED result.
6. Reprove the current authorization package. Schema2 packages bind `repositoryId + projectConfigIdentity`; schema3 is allowed only for the closed repo-local actor-bound project-upgrade bundle.
...
Rerun DISCOVER on task/actor/role/phase/profile/capability or rule-source drift. Objective/action/result/scope drift invalidates only the boundary decision. Do not reload per tool call.
