...
当前开发候选：精确选择与职责边界、项目纠正日常修订和完整历史、中央映射退出及显式采用处置、actor 收据存储、生命周期路径预览、随包消费者说明和统一材料生命周期。根级维护同时分离开发源码与 Maintenance 职责，移除开发根宿主入口并保留独立交付模板。当前验证/审核资格以 RELEASE_MANIFEST 为准，尚未完成本批独立 Review 与完整套件验收；本条不代表发行或项目采用。
