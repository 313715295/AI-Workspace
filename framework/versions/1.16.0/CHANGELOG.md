...
当前候选交付包含调用边界减负、纠正整组生命周期、默认委托来源统一，以及规则包大小改为观测。原实现已完成独立完整 Review 和 R1/R2 聚焦复审；本次发行准备仅对齐变更记录并重新验证最终冻结 payload。仍为 `1.16.0 CANDIDATE/MINOR`，拟分发 `1.16.0-snapshot.5`；当前 manifest 在最终证据核对前保持 PENDING，已采用的 snapshot.4 不变。
