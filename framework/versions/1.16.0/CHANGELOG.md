...
本次候选增量：新建任务创建提示一次交齐委派和限域待绑定材料，接收者依据实际初始委派及宿主自身 ID 派生精确包；已知身份与健康续作直接沿原绑定。后建 Reviewer 可继承原 Owner/issuer 与当前生产证据。正式 Review 直接装载启动、资源与工具规则；普通项目治理按项目根绑定，Git 身份只在 Git 动作或跨仓库拓扑时要求；本地候选的安装后像不冻结合法的当前 Bootstrap 和其他项目内容。成果负责人在任务明确授权、审核与接受证据精确绑定待推送提交、提交及远端均精确绑定时可完成单次普通 PUSH。修复严格 JSON 同字节身份和 DELIVERY 文本输出；五组重复 PowerShell 调用已去除，完整测试日志可由摘要入口另存并显示计数。各项仍须按当前源字节重新验证与审核。

临时 writer 的交审链保留原 Owner/issuer：原生产包可预授权稍后创建的独立可见 Reviewer，writer 完成生产与测试后把宿主创建结果、真实 Reviewer ID 和原 FINALIZE 绑定到派生纯 Review 包。finding 直接返回 writer 修复，APPROVED 携完整交付材料引用一次交原 Owner 接受；绑定和候选漂移仍由 checker 拒绝。此宿主关系证据保持 INSTRUCTION_BOUND，不宣称本地记录能独立证明宿主调用。
