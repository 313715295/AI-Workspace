...
Every new schema 1.16.0 card declares exactly one `Work route: actor=<HOST_TASK_ID>; role=<ROLE>; phase=<PHASE>`. The current Owner changes all three atomically under the normal task CONTROL_WRITE boundary. The task index remains a locator only. Actor/role/phase select context; action authorization remains independent and cannot create or override them.
