# Framework 1.13.0 examples

## New project

After 1.13.0 becomes stable, explicit registration copies its exact starter. The project receives its own pin, Controller, corrections object and empty process-policy. Framework stores no consumer record.

## Existing project

A schema3 1.12 project previews `ToVersion=1.13.0`. Upgrade preserves its corrections and current `PROJECT-CUSTOM`, adds the project-level backend field and does not invent policy rules. Existing two-field tasks remain readable; the owner binds the real actor only when a task reaches its first 1.13 substantive boundary.
...
`ADMIT_ACTION` blocks when a selected preparation receipt is absent. A structural PASS does not grant SOURCE_WRITE, so the separate exact authorization checker must also pass. `FINALIZE_OUTPUT` blocks when requested result or direct-delivery evidence is absent and still reports `semanticCorrectnessProven=false`.
