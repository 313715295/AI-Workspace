...
Every new schema 1.13.0 card declares exactly one `Work route: actor=<HOST_TASK_ID>; role=<ROLE>; phase=<PHASE>`. The current Owner changes all three atomically under the normal task CONTROL_WRITE boundary. The task index remains a locator only. Actor/role/phase select context; action authorization remains independent and cannot create or override them.
...
Legacy cards are not bulk rewritten for adoption. A 1.11/1.12 two-field card may recover with `LEGACY_ACTOR_CONTEXT_UNBOUND`, but no 1.13 substantive actor action passes until the current Owner writes the authenticated actor into the same card. Actor is never inferred from owner, package, prompt or host label.
