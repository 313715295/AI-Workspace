...
function Test-AiwRuleMatch {
    param($Selectors,[string]$Profile,[string]$Role,[string]$Phase,[string]$ActionKind,[string]$ResultKind,[string[]]$Paths,[string[]]$Capabilities,[string]$Objective)
    ...
    return [pscustomobject]@{Match=$true;Semantic='UNKNOWN_CONSERVATIVE_LOAD'}
}
...
function Invoke-ProcessRequirementComposition {
    ...
    if ([int]$catalog.schemaVersion -ne 1 -or [string]$catalog.frameworkVersion -cne $ComposerVersion -or -not ($catalog.requirements -is [Array])) { throw 'PROCESS_REQUIREMENTS_VALUES' }
    $nativeById = @{}
    $nativeByAlias = @{}
    foreach ($requirement in @($catalog.requirements)) {
        Assert-AiwExactFields $requirement @('requirementId','legacyAliases','title','description','selectors','fullText','preparationRequirements','resultRequirements') 'NATIVE_REQUIREMENT'
        ...
        foreach($field in @('title','description','fullText')){if(-not($requirement.$field-is[string])-or[string]::IsNullOrWhiteSpace([string]$requirement.$field)){throw 'NATIVE_REQUIREMENT_TEXT'}}
    ...
    if ($null -ne $correctionsPath) {
        ...
        if ([int]$corrections.schemaVersion -ne 1 -or [string]$corrections.projectId -cne $projectId -or -not ($corrections.corrections -is [Array])) { throw 'CORRECTIONS_VALUES' }
        $seenCorrection = [Collections.Generic.HashSet[string]]::new([StringComparer]::Ordinal)
        foreach ($record in @($corrections.corrections)) {
            $canonical = Get-AiwCanonicalCorrectionRecordIdentityV1 $record
            ...
            $records += [pscustomobject]@{ Record=$record; Alias=('correction:'+$projectId+':'+[string]$record.correctionId); SourceRecordIdentity=$canonical }
        }
    }
    ...
    try {
        ...
        if ([int]$coverage.schemaVersion -ne 2 -or [string]$coverage.releaseVersion -cne $ComposerVersion -or -not ($coverage.versions -is [Array])) { throw 'CORRECTION_COVERAGE_VALUES' }
        ...
        if ($entry.Count -eq 1) {
            ...
            foreach ($mapping in @($entry[0].incorporationMappings)) {
                Assert-AiwExactFields $mapping @('correctionId','legacyRequirementId','nativeRequirementId','coverageState','nativeCatalogIdentity','legacySourceRecordIdentity') 'INCORPORATION_MAPPING'
    ...
    foreach ($item in $records) {
        $view = [ordered]@{correctionId=[string]$item.Record.correctionId;requirementReason=[string]$item.Record.requirementReason;effectiveRule=[string]$item.Record.effectiveRule;applicability=[string]$item.Record.applicability;decisionLocator=[string]$item.Record.decisionLocator;legacyRequirementId=[string]$item.Alias;legacySourceRecordIdentity=[string]$item.SourceRecordIdentity}
        ...
        elseif ($mappedByAlias.ContainsKey([string]$item.Alias)) {
            ...
            if ([string]$mapping.legacySourceRecordIdentity -ceq [string]$item.SourceRecordIdentity) {
    ...
    if ($null -ne $config.PSObject.Properties['processPolicy']) {
        ...
        if ([int]$policy.schemaVersion -ne 1 -or [string]$policy.contractVersion -cne $ComposerVersion -or [string]$policy.projectId -cne $projectId -or -not ($policy.rules -is [Array])) { throw 'PROCESS_POLICY_VALUES' }
        $policyRules = @($policy.rules)
    }
    if ($custom.HasNormativeContent -and $policyRules.Count -gt 0) { throw 'CONFLICT_PROJECT_RULE_DOUBLE_CARRIER' }

    $selected = @(); $seenRequirement = [Collections.Generic.HashSet[string]]::new([StringComparer]::Ordinal)
    foreach ($requirement in @($catalog.requirements)) {
        $id='framework:'+[string]$requirement.requirementId
        if (-not $seenRequirement.Add($id)) { throw 'NATIVE_REQUIREMENT_ID' }
        $match=Test-AiwRuleMatch $requirement.selectors $Profile $Role $Phase $ActionKind $ResultKind $ExactPaths $Capabilities $Objective
        if ($match.Match) {
            ...
            $selected += [pscustomobject]@{requirementId=$id;source='FRAMEWORK';sourceAliases=@($origins);semanticApplicability=$match.Semantic;fullText=[string]$requirement.fullText;preparationRequirements=@($requirement.preparationRequirements);resultRequirements=@($requirement.resultRequirements)}
        }
    }
    foreach ($item in $legacyEffective) {
        $selected += [pscustomobject]@{requirementId=[string]$item.legacyRequirementId;source='PROJECT_CORRECTION';semanticApplicability='LEGACY_PROGRESSIVE_SELECTION_UNPROVEN';fullText=[string]$item.effectiveRule;preparationRequirements=@();resultRequirements=@()}
    }
    foreach ($rule in $policyRules) {
        ...
        $match=Test-AiwRuleMatch $rule.selectors $Profile $Role $Phase $ActionKind $ResultKind $ExactPaths $Capabilities $Objective
        if ($match.Match) { $selected += [pscustomobject]@{requirementId=$id;source='PROJECT_POLICY';semanticApplicability=$match.Semantic;fullText=[string]$rule.effectiveRule;preparationRequirements=@($rule.preparationRequirements);resultRequirements=@($rule.resultRequirements)} }
    }
    if ($custom.HasNormativeContent) {
        $selected += [pscustomobject]@{requirementId=('project-custom:'+$projectId);source='LEGACY_PROJECT_CUSTOM';semanticApplicability='LEGACY_PROGRESSIVE_SELECTION_UNPROVEN';fullText=$custom.Text;preparationRequirements=@();resultRequirements=@()}
    }
    ...
    $evidenceCeilings=@()
    if($legacyEffective.Count-gt0){$evidenceCeilings+='LEGACY_CORRECTIONS_FULL_LOAD'}
    ...
    return [pscustomobject]@{
        ...
        projectConfigIdentity=$configDoc.Identity; correctionsIdentity=$correctionsIdentity; policyIdentity=$policyIdentity; projectCustomIdentity=$custom.Identity
        ...
        sourceBuildCount=1; legacyCorrectionsFullReadCount=$(if($legacyEffective.Count -gt 0){1}else{0}); legacyProjectCustomFullReadCount=$(if($custom.HasNormativeContent){1}else{0})
    }
}

Export-ModuleMember -Function Invoke-ProcessRequirementComposition,Get-AiwCanonicalCorrectionRecordIdentityV1,Get-AiwFileIdentity,Get-AiwProcessBindingSnapshot
