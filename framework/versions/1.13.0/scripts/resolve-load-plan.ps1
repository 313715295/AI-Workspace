...
if (-not [string]::IsNullOrWhiteSpace($TaskPath)) {
    ...
    if ($routeLines.Count -eq 1 -and $currentRouteMatches.Count -eq 1) {
        if($taskSchema-cne'1.13.0'){throw 'LOAD_TASK_ACTOR_ROUTE_SCHEMA'}
        ...
        if ($taskSchema -in @('1.11.0','1.12.0','1.13.0')) { throw 'LOAD_TASK_WORK_ROUTE_REQUIRED' }
...
if ($manifest.frameworkVersion -cne '1.13.0' -or $manifest.lifecycle -cne 'STABLE') { throw 'LOAD_MANIFEST_NOT_STABLE_1_13_0' }
