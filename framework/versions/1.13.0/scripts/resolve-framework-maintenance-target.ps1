...
try {
    ...
    if (-not (Test-JsonInteger $config.schemaVersion) -or [int]$config.schemaVersion -ne 4 -or
        ...
        -not ($config.frameworkVersion -is [string]) -or [string]$config.frameworkVersion -cne '1.13.0' -or
    ...
    if(-not(Test-JsonInteger $policy.schemaVersion)-or[int]$policy.schemaVersion-ne1-or[string]$policy.contractVersion-cne'1.13.0'-or[string]$policy.projectId-cne[string]$config.id-or-not($policy.rules-is[Array])){throw 'PROCESS_POLICY_VALUES'}
    ...
    foreach ($required in @('README.md','framework/versions/1.13.0/RECOVERY_CORE.md','framework/versions/1.13.0/LOAD_MANIFEST.json','framework/versions/1.13.0/TOOLCHAIN.json','framework/versions/1.13.0/FRAMEWORK_MAINTENANCE.md','AGENTS.md')) {
