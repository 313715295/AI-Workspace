...
- `DISCOVER` strictly binds project, task whole identity, host-authenticated actor, role, phase, profile, project-declared capabilities, objective/action/result kind and normalized exact scope. Caller capability drift, unknown action/result kinds and unsafe relative paths fail closed. It selects and returns the full text of applicable sealed Framework requirements, still-effective corrections and permanent project rules, a source-composition identity and a separate selection identity.
- `ADMIT_ACTION` consumes an exact ephemeral DISCOVER receipt, revalidates every bound task/Framework/catalog/coverage/project/correction/policy/custom source identity and checks its selected preparation requirements before a distinct governed action. It never grants the action.
- `FINALIZE_OUTPUT` performs the same source revalidation and checks observable result/delivery completeness before the actual final output.

UNKNOWN semantic applicability conservatively loads the rule. Schema1 free-text corrections and legacy PROJECT-CUSTOM use full-source compatibility loading and unchanged-context reuse, with `LEGACY_PROGRESSIVE_SELECTION_UNPROVEN`; no compact-saving claim is made. Source composition, progressive selection and boundary-decision identities are distinct, with separate invalidators; a backend may reread unchanged bytes and must not claim a physical cache hit unless observed. Receipts are ephemeral, non-authoritative and never a repository ledger.
...
The official `powershell7` backend is invoked with `pwsh -NoProfile -NonInteractive -File <entrypoint> ...`. Each 1.13.0 backend entrypoint independently rejects a non-Core runtime or PowerShell major version below 7.
...
The contract permits later platform admission, but each released toolchain lists only platforms it actually supports. 1.13.0 lists Windows only. Platform-specific evidence requirements remain explicit:
