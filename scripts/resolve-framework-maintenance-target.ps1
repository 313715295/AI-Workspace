...
try{
    ...
    foreach($required in @('README.md','AGENTS.md','scripts/resolve-framework-maintenance-target.ps1','scripts/check-framework-maintenance-authorization.ps1','scripts/invoke-framework-maintenance-safe-git.ps1','scripts/resolve-framework-maintenance-process-requirements.ps1')){Assert-TargetPathWithoutReparse $runtimeRoot $required.Replace('\','/');if(-not(Test-Path -LiteralPath (Join-Path $runtimeRoot $required) -PathType Leaf)){throw ('TARGET_ROOT_FILE_MISSING|'+$required)}}
