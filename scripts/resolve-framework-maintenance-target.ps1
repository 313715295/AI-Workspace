...
try{
    ...
    $legacyPolicy=@($overlay.LegacySourceVersions)-ccontains[string]$config.frameworkVersion
    if($legacyPolicy){
        Assert-Fields $policy @('schemaVersion','contractVersion','projectId','rules') 'PROCESS_POLICY'
        if(-not(Test-Integer $policy.schemaVersion)-or[int]$policy.schemaVersion-ne1-or[string]$policy.contractVersion-cne'1.14.0'-or[string]$policy.projectId-cne[string]$config.id-or-not($policy.rules-is[Array])){throw 'PROCESS_POLICY_VALUES'}
    }else{
